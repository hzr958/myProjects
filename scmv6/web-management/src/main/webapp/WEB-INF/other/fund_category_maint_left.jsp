...
$(document).ready(function(){
 ...
 <div class="Retrieval">
        ...
        <input onclick="categoryMaint.search(1);" type="button" class="btn-search" />
      </div>
  <div class="sidebar-nav mtop10" style="height:500px;overflow-y:auto;">
    <ul>
      <li><a onclick="categoryMaint.searchByLeft(this);" typeId="" class="leftnav-hover" parentId=""><span class="Shear-head"></span><div class="Fleft text-overflow2"><strong>全部</strong></div></a></li>
       <s:iterator value="leftList" var="menu">
       		<li>
		      	<a onclick="categoryMaint.searchByLeft(this);"  typeId="${menu.id}" parentId=""><span class="Shear-head"></span><div class="Fleft text-overflow2"><i class="icon_starpic_rol"></i><strong>${menu.name}</strong></div></a>
		      	  <s:if test="#menu.list!=null && #menu.list.size()>0">
			      	<dl style="display:none">
			      			<s:iterator value="#menu.list" var="ls">
			      					 <dd><a onclick="categoryMaint.searchByLeft(this);" typeId="${ls.id}" parentId="${menu.id}" alt="1" ><div class="two_nav_name3" title="<s:property value="#ls.name" />"><s:property value="#ls.name" /> </div></a></dd>
			      			</s:iterator>
			         </dl>
		         </s:if>
		      </li>
