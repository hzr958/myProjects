...
import com.smate.web.management.model.analysis.sns.PsnJournal;
...
public class PsnJournalDao extends SnsHibernateDao<PsnJournal, Long> {
  ...
  public List<String> getPsnPubCopartnerByJnl(Long psnId) {
    String hql =
        "select distinct t1.issnTxt from PsnJournal t1,PsnKnowCopartner t2 where t1.psnId=t2.cptPsnId and t2.psnId=?";
  ...
  public List<String> getPsnJouralClassifyTopN(Long psnId, int n) {
    ...
    String hql =
        "select t1.classify from PsnJournal t,JournalAreaClassify t1 where t.psnId=? and t.issnTxt = t1.issnTxt  group by t1.classify order by sum(t.tf) desc";
