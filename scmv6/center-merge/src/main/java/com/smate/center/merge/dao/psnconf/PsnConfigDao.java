...
public class PsnConfigDao extends SnsHibernateDao<PsnConfig, Long> {
  ...
  public Long findOpenPubSum(Long psnId) {
    String sql = "select count(*) from v_pub_simple t, " + "(select pcp.pub_id from Psn_Config pc,Psn_Config_Pub pcp "
        + "where pc.cnf_Id=pcp.cnf_Id and pc.status=1 and pcp.any_User=7 and pc.psn_Id=? order by pcp.pub_id desc) p"
        + " where t.pub_id = p.pub_id  and t.status = 0 and t.simple_status in (0, 1) "
        + "and t.article_type=1 and t.owner_psn_id = ?";
  ...
  public Long findTotalPubSum(Long psnId) {
    String sql = "select count(*) from v_pub_simple t, " + "(select pcp.pub_id from Psn_Config pc,Psn_Config_Pub pcp "
        + "where pc.cnf_Id=pcp.cnf_Id and pc.status=1  and pc.psn_Id=? order by pcp.pub_id desc) p "
        + "where t.pub_id = p.pub_id  and t.status = 0 and t.simple_status in (0, 1, 99)"
        + " and t.article_type=1 and t.owner_psn_id = ?";
    return super.queryForLong(sql, new Object[] {psnId, psnId});
  }
}
