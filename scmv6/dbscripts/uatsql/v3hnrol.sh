...
#6
sqlplus uathnrol/$hnroluat <<EOF
@/home/oracle/dbscripts/$newdirname/test/v3hnrol.sql
exit;
EOF
sleep 2
...
#将/dev目录下的.sql文件内容追加一份到/test目录下的相应文件内
  sed -n '10,10000p' $sqlpath/test/v3hnrol.sql >> $sqlpath/uat/v3hnrol.sql
#将/dev/目录下的.sql文件恢复到默认内容
  cp -rf $exsqlpath/test/v3hnrol.sql $sqlpath/test/v3hnrol.sql
#done;
