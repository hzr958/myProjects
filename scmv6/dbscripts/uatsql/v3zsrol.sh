...
#执行SQL脚本
#13
sqlplus uatzsrol/$zsroluat <<EOF
@/home/oracle/dbscripts/$newdirname/test/v3zsrol.sql;
exit;
EOF
sleep 2
...
#将/dev目录下的.sql文件内容追加一份到/test目录下的相应文件内
  sed -n '10,10000p' $sqlpath/test/v3zsrol.sql >> $sqlpath/uat/v3zsrol.sql
#将/dev/目录下的.sql文件恢复到默认内容
  cp -rf $exsqlpath/test/v3zsrol.sql $sqlpath/test/v3zsrol.sql
#done;
