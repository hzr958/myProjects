...
#执行SQL脚本
#10
sqlplus uatrol/$roluat <<EOF
@/home/oracle/dbscripts/$newdirname/test/v3rol.sql
exit;
EOF
sleep 2
...
#将/dev目录下的.sql文件内容追加一份到/test目录下的相应文件内
  sed -n '10,10000p' $sqlpath/test/v3rol.sql >> $sqlpath/uat/v3rol.sql
#将/dev/目录下的.sql文件恢复到默认内容
  cp -rf $exsqlpath/test/v3rol.sql $sqlpath/test/v3rol.sql
#done;
