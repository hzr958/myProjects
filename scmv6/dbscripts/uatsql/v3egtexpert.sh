...
#3
sqlplus egtexpertuat/$expertuat <<EOF
@/home/oracle/dbscripts/$newdirname/test/v3egtexpert.sql
exit;
EOF
sleep 2
...
#将/dev目录下的.sql文件内容追加一份到/test目录下的相应文件内
  sed -n '10,10000p' $sqlpath/test/v3egtexpert.sql >> $sqlpath/uat/v3egtexpert.sql
#将/dev/目录下的.sql文件恢复到默认内容
  cp -rf $exsqlpath/test/v3egtexpert.sql $sqlpath/test/v3egtexpert.sql
#done;
