...
@/home/oracle/scmv6/dbscripts/$newdirname/run/v3sns.sql
exit;
EOF
sleep 2
#sql到此执行完毕
...
#将/dev目录下的.sql文件内容追加一份到/test目录下的相应文件内
  sed -n '10,10000p' $sqlpath/run/v3sns_inc.sql >> $sqlpath/run/v3sns.sql
#将/dev/目录下的.sql文件恢复到默认内容
  git rm $sqlpath/run/v3sns_inc.sql
#done;

#svn 进行内容提交
git add $sqlpath/run/v3sns.sql && sleep 3 && git commit -m "测试机执行生产增量sql.." && sleep 3 && git push
