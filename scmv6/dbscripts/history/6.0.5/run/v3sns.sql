...
--原因（分享表，添加索引） 2017-07-22 ajb start
...
create index IDX_MSG_RELATION_ID on V_PSN_FILE_SHARE_RECORD (msg_relation_id);
create index IDX_SHARE_BASE_ID on V_PSN_FILE_SHARE_RECORD (share_base_id);

commit  ;

--原因（分享表，添加索引） 2017-07-22 ajb end
...
values(48,'InitMsgTaskTrigger','0 18 14 28 7 ?',0,'msg 数据初始化');
commit;
--原因（消息中心初始化任务） 2017-6-29 zjh begin
...
insert into v_quartz_cron_expression t (t.id,t.cron_trigger_bean,t.cron_expression,t.status,t.description) 
values(59,'RecomMsgTaskTrigger','0 15 14 28 7 ?',0,'成果认领、全文认领-发消息-定时任务');
commit;
