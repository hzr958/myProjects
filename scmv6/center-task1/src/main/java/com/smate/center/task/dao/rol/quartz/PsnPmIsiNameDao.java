...
public class PsnPmIsiNameDao extends RolHibernateDao<PsnPmIsiName, Long> {
   ...
   * @return
   */
  public boolean isAddtNameExists(String initName, Long psnId) {
    String hql = "select count(id) from PsnPmIsiName where name =:name and psnId =:psnId ";
    Long count =
        (Long) super.createQuery(hql).setParameter("name", initName).setParameter("psnId", psnId).uniqueResult();
    if (count > 0) {
      return true;
    } else {
      return false;
