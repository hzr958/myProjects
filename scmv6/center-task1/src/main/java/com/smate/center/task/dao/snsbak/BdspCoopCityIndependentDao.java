...
import com.smate.center.task.model.snsbak.BdspCoopCityIndependent;
import com.smate.center.task.model.snsbak.PubProvinceInteriorKey;
import com.smate.core.base.utils.data.SnsbakHibernateDao;

@Repository
public class BdspCoopCityIndependentDao extends SnsbakHibernateDao<BdspCoopCityIndependent, Long> {
  ...
  public void saveRs(PubProvinceInteriorKey key, Integer count, Integer indexId) {
    ...
    String hql =
        "from BdspCoopCityIndependent t where t.pubYear=:pubYear and t.categoryId=:categoryId and t.cityId=:cityId and t.provinceId=:provinceId and t.indexId=:indexId order by t.id desc";
    List<BdspCoopCityIndependent> list = (List<BdspCoopCityIndependent>) super.createQuery(hql)
    ...
    if (list != null && list.size() != 0) {
      BdspCoopCityIndependent bcpi = list.get(0);
      count = count + bcpi.getCount();
      bcpi.setCount(count);
      bcpi.setUpdateTime(new Date());
      super.save(bcpi);
    } else {
      BdspCoopCityIndependent bcpi =
          new BdspCoopCityIndependent(indexId, pubYear, categoryId, cityId, provinceId, count, new Date());
      super.save(bcpi);
    }
