...
public class InitPdwhInsPsnDic {
  ...
  public void init() {
    ...
    File fileKws = new File(pathKws);
    if (fileKws.exists()) {
      Integer k = 0;
      try {
        ris = new FileInputStream(fileKws);
        isr = new InputStreamReader(ris, "UTF-8");
        br = new BufferedReader(isr);
        String fileStr1 = null;
        while (StringUtils.isNotBlank(fileStr1 = br.readLine())) {
          DicLibrary.insert(DicLibrary.DEFAULT, fileStr1.toLowerCase().trim(), "nsfc_kw_discipline",
              DicLibrary.DEFAULT_FREQ);
        }
      } catch (Exception e) {
        System.out.println("加载关键词词典出错: " + e.getMessage());
      } finally {
        try {
          ris.close();
          isr.close();
      ...
      System.out.println("加载关键词词典完成====time:" + new Date() + "====加载关键词条目数:" + k);
    } else {
