...
@Deprecated
public class PubToPubsimpleTask7 extends TaskAbstract {
  private final Logger logger = LoggerFactory.getLogger(getClass());

  private final static Integer SIZE = 5000; // 每次刷新获取的个数

  /*
   * private Long startPubId = 1000006000000L;
   * 
   * private Long endPubId = 1000007500000L;
   */
  ...
  public PubToPubsimpleTask7() {
    super();
  }

  public PubToPubsimpleTask7(String beanName) {
    super(beanName);
  }
  ...
  public void doRun() throws SingleTaskException {
    if (!super.isAllowExecution()) {
      logger.info("=========PubToPubSimpleTask7已关闭==========");
      return;
    }
    if (taskMarkerService.getApplicationQuartzSettingValue("PubToPubsimpleTask_removePubCache7") == 1) {
      cacheService.remove("PubToPubsimpleTask7", "last_pub_id");
    }

    try {
      Long lastPubId = (Long) cacheService.get("PubToPubsimpleTask7", "last_pub_id");
      if (lastPubId == null) {
        lastPubId = startPubId;
      }
      logger.debug("===========================================PubToPubSimpleTask7=========开始1");
      ...
      if (CollectionUtils.isEmpty(pubList)) {
        logger.info(
            "===========================================BatchPubToPubSimpleTask7  没有获取到pubSimple表数据!!!!============, time = "
                + new Date());
        return;
      }
      ...
      this.cacheService.put("PubToPubsimpleTask7", 60 * 60 * 24, "last_pub_id", lastId);
      ...
      for (Long pubId : pubList) {
        try {
          ...
          logger.debug("PubToPubSimpleTask7出错==============", e);
        }
      }

    } catch (Exception e) {
      logger.error("pubToPubsimpleTask7,运行异常", e);
    }
