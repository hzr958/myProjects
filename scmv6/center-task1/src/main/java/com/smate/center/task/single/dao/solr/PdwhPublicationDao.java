...
public class PdwhPublicationDao extends PdwhHibernateDao<PdwhPublication, Long> {
  ...
  public List<PdwhPublication> findListByIds(List<Long> pubIdList) {
    String hql = "select new PdwhPublication(t.pubId,t.dbId,t.jnlId,t.pubType,t.pubYear) from PdwhPublication t "
        + "where t.pubId in(:pubIdList)";
  ...
  public List<PdwhPublication> findPubByBatchSize(Long lastId, Integer batchSize) {
    String hql =
        "from PdwhPublication t where t.pubType in (1, 2, 3, 4, 7, 8, 10) and t.pubId>:lastId order by t.pubId asc";
  ...
  public Long getPubIdByPatNoandOpenNo(String patentNo, String patentOpenNo) {
    ...
    return (Long) super.createQuery(hql).setParameter("patentNo", patentNo).setParameter("patentOpenNo", patentOpenNo)
  ...
  public Long getPubIdByTitle(String title) {
    ...
    return (Long) super.createQuery(hql).setParameter("zhTitle", title).setParameter("enTitle", title).setMaxResults(1)
        .uniqueResult();
  }
  ...
  public List<PdwhPublication> getPdwhPubIds(Long lastPubId, int batchSize) {
    String hql =
        "select new PdwhPublication(t.pubId,t.dbId) from PdwhPublication t where t.dbId in (4,15,16,17) and t.pubId > :lastPubId order by t.pubId ";
  ...
  public PdwhPublication getPubDupInfoById(Long pubId) {
    String hql =
        "select new PdwhPublication(t.pubId,t.zhTitle,t.enTitle,t.pubType,t.pubYear) from PdwhPublication t where t.pubId =:pubId";
    return (PdwhPublication) super.createQuery(hql).setParameter("pubId", pubId).uniqueResult();
  }
  ...
  public PdwhPublication getPubAuthorKwsById(Long pubId) {
    String hql =
        "select new PdwhPublication(t.pubId,t.authorName,t.authorNameSpec,t.zhKeywords,t.enKeywords) from PdwhPublication t where t.pubId =:pubId";
    return (PdwhPublication) super.createQuery(hql).setParameter("pubId", pubId).uniqueResult();
  }

  @SuppressWarnings("unchecked")
  public List<PdwhPublication> getConfirmPubDetail(List<Long> pdwhPubIds) {
    String hql =
        "select new PdwhPublication(t.pubId,t.zhTitle,t.enTitle,t.authorName,t.authorNameSpec,t.zhBriefDesc,t.enBriefDesc,t.pubType) from PdwhPublication t where t.pubId in (:pubIds)";
    return super.createQuery(hql).setParameterList("pubIds", pdwhPubIds).list();

  }

  public PdwhPublication getConfirmPubTitle(Long pubId) {
    String hql =
        "select new PdwhPublication(t.pubId,t.zhTitle,t.enTitle,t.authorName,t.authorNameSpec,t.zhBriefDesc,t.enBriefDesc,t.pubType)  from PdwhPublication t where t.pubId = :pubId";
    return (PdwhPublication) super.createQuery(hql).setParameter("pubId", pubId).uniqueResult();
  }
}
