...
public class SaveFundInfoFromXMlTask1 extends TaskAbstract {
  private final Logger logger = LoggerFactory.getLogger(getClass());
  private int batchSize = 200;
  private Long pubId = 15415739L;
  @Autowired
  ...
  public static String PDWH_PUB_ID_CACHE1 = "pdwh_pub_id_cache1";

  public SaveFundInfoFromXMlTask1() {
    super();
  }

  public SaveFundInfoFromXMlTask1(String beanName) {
    super(beanName);
  }

  public void doRun() throws SingleTaskException {
    if (!super.isAllowExecution()) {
      logger.info("=========SaveFundInfoFromXMlTask1 已关闭==========");
      return;
    }
    // 是否移除psn_id缓存
    if (taskMarkerService.getApplicationQuartzSettingValue("SaveFundInfoFromXMlTask_removePubIdCache1") == 1) {
      cacheService.remove(PDWH_PUB_ID_CACHE1, "last_pub_id");
    }
    logger.error("===========lastPubId为=======" + cacheService.get(PDWH_PUB_ID_CACHE1, "last_pub_id"));
    try {
      Long lastPubId = (Long) cacheService.get(PDWH_PUB_ID_CACHE1, "last_pub_id");
      if (lastPubId == null) {
        lastPubId = 10090668L;
      }
      ...
      this.cacheService.put(PDWH_PUB_ID_CACHE1, 60 * 60 * 24, "last_pub_id", lastPubId);
