...
public class BaseJournalTitleDao extends PdwhHibernateDao<BaseJournalTitle, Long> {
  ...
  /**
   * 根据期刊id查询所有对应的title,同时按照title表中的主键id进行倒叙排序
   * 
   * @param jnlId
   * @return
   */
  @SuppressWarnings("unchecked")
  public List<BaseJournalTitle> findTitleByJnlId(Long jnlId) {
    String hql = "from BaseJournalTitle where journal.jouId=? order by jouTitleId desc";
