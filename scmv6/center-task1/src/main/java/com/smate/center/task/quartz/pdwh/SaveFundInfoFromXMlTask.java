...
public class SaveFundInfoFromXMlTask extends TaskAbstract {
  private final Logger logger = LoggerFactory.getLogger(getClass());
  private int batchSize = 200;
  private Long pubId = 10090668L;
  @Autowired
  ...
  public static String PDWH_PUB_ID_CACHE = "pdwh_pub_id_cache";

  public SaveFundInfoFromXMlTask() {
    super();
  }

  public SaveFundInfoFromXMlTask(String beanName) {
    super(beanName);
  }
  ...
  public void doRun() throws SingleTaskException {
    ...
    // 是否移除psn_id缓存
    if (taskMarkerService.getApplicationQuartzSettingValue("SaveFundInfoFromXMlTask_removePubIdCache") == 1) {
      cacheService.remove(PDWH_PUB_ID_CACHE, "last_pub_id");
    }
    logger.error("===========lastPubId为=======" + cacheService.get(PDWH_PUB_ID_CACHE, "last_pub_id"));
    try {
      Long lastPubId = (Long) cacheService.get(PDWH_PUB_ID_CACHE, "last_pub_id");
      if (lastPubId == null) {
        lastPubId = 0L;
      }
      ...
      this.cacheService.put(PDWH_PUB_ID_CACHE, 60 * 60 * 24, "last_pub_id", lastPubId);
