<li onmouseover="dynMsgUtil.showDynOperation(this)"  onmouseout="dynMsgUtil.hideDynOperation(this)">
    ...
    <div class="publish-nr">
      ...
      <div class="moving-contat"><a href="/scmwebsns/resume/psnView?des3PsnId=${des3PsnId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" class="Blue">${psnEnName}</a> endorsed <a href="/scmwebsns/resume/psnView?des3PsnId=${endorseObj_des3PsnId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" class="Blue">${endorseObj_psnEnName}</a>'s research area: <p style="margin-top:10px;"><#list endorserDetails as endorser>${endorser.discEnName}<#if endorser_has_next>; </#if></#list></p></div>
