<li onmouseover="dynMsgUtil.showDynOperation(this)"  onmouseout="dynMsgUtil.hideDynOperation(this)">
    ...
    <div class="publish-nr">
      <div class="moving-contat"><input type="hidden" value="${psnId?c}" id="cpsn_${dynId?c}"/>
        <a href="/scmwebsns/resume/psnView?des3PsnId=${des3PsnId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" class="Blue">${psnEnName}</a> updated work experience
      <#if insName!=''||unitName!=''||position!=''>
      <p style="margin-top:10px;" id="work_iris_${dynId?c}">${insName}<#if insName!='' && unitName!=''>, ${unitName}<#else>${unitName}</#if><#if (insName!='' || unitName!='') && position!=''>, ${position}<#else>${position}</#if></p>
      </#if>
      </div>
      ...
      <div class="appraisa-choose">
        ...
        <span class="f888 date01"> ${dynDateVar}</span>&nbsp;&nbsp;&nbsp;&nbsp;
		  <#if (opType?exists && opType==1)>
	        <div class="praise_box" style="margin-left:20px;">
		        
		        <#if (hasAward==0)>
		        <a onclick="dynMsgUtil.ajaxAward(this, '${resType}','${resNode}','${resId?c}','${permission}')" style="cursor:pointer" class="mright10 f888 award_link${resType}_${resNode}_${resId?c}"><i class="dt_icon zan_icon"></i>Like</a>
		        ...
		        <#else>
		        <a onclick="dynMsgUtil.ajaxAward(this, '${resType}','${resNode}','${resId?c}','${permission}')" style="cursor:pointer;display:none" class="mright10 f888 award_link${resType}_${resNode}_${resId?c}"><i class="dt_icon zan_icon"></i>Like</a>
		        ...
		        <a style="cursor:pointer" onclick="dynMsgUtil.showReplyBox('${resId?c}','${resType}','${resNode}','${dynId?c}',event, this)" class="mright10 f888"><i class="dt_icon review_icon"></i>Say congrats<span<#if (replyNum==0)> style="display:none"</#if>>(<label id="reply_num_label_${dynId?c}_${resId?c}" class="reply_num_label_${resType}_${resId?c}">${replyNum}</label>)</span></a>
		        <#else>
		        <#if (replyNum>0)><span class="fe6e6 mright10">|</span></#if>
		        <span<#if (replyNum==0)> style="display:none"</#if> class="f666" >Say congrats(<label id="reply_num_label_${dynId?c}_${resId?c}" class="reply_num_label_${resType}_${resId?c}">${replyNum}</label>)</span>
		        </#if>
    ...
    <#if (opType)?exists && opType==1>
	         ...
	         <#list resReplyDetails as resReply>
	          <dd class="reply_item_${resReply.recordId?c}">
	            ...
	            <div class="t_message2">
	              ...
	              <p class="en10">${resReply.replyDate}&nbsp;&nbsp;<span class="reply_operation" style="display:none;"><a style="cursor:pointer" receiverPsnName="${resReply.replyerEnName}" receiverPsnLink="/scmwebsns/resume/psnView?des3PsnId=${resReply.des3ReplyerId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" onclick="dynMsgUtil.replyToPsn('${resId?c}','${resType}','${resNode}','${dynId?c}','${resReply.des3ReplyerId}',event, this)" class="Blue"><i class="evaluate-icon"></i></a>&nbsp;<#if (isResOwner==1 || resReply.replyerId==currentPsnId)><a onclick="dynMsgUtil.deleteReply('${dynId?c}','${resReply.recordId?c}','${resId?c}','${resType}','${resNode}',event, this)" style="cursor:pointer" class="Blue" title="Delete"><i class="delete-icon"></i></a></#if></span></p>
	            </div>
	          </dd>
      ...
      <div class="comments_box">
        <div class="reply_enter_div" id="input_${dynId?c}_${resId?c}">
          <input type="text" class="inp_text f666" value="Say congrats……"  style="width:590px;cursor:pointer" onclick="dynMsgUtil.showReplyBox('${resId?c}','${resType}','${resNode}','${dynId?c}',event, this)"/>
        </div>
        <div style="display:none;" class="reply_content_div" id="textarea_${dynId?c}_${resId?c}">
          ...
          <div class="t_message3">
              ...
              <p>
                <a style="cursor:pointer" onclick="dynMsgUtil.ajaxReply('${resType}','${resNode}','${resId?c}','${dynId?c}','${permission}',1, this)" class="replyBtn uiButton uiButtonConfirm">Comment</a>&nbsp;&nbsp;
              </p> 
           </div>
        </div>
      </div>
