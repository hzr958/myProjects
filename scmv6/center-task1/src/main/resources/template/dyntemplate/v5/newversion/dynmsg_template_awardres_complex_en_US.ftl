<li onmouseover="dynMsgUtil.showDynOperation(this)"  onmouseout="dynMsgUtil.hideDynOperation(this)">
    ...
    <div class="publish-nr">
      ...
      <#list awarderDetails as awarder><#if (!awarder_has_next) && (awarder_index!=0)> and <#else>${splitChar}</#if><a href="/scmwebsns/resume/psnView?des3PsnId=${awarder.awarder_des3PsnId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" class="Blue">${awarder.awarder_psnEnName}</a><#if awarder_has_next><#assign splitChar>, </#assign></#if></#list> like <#if resOwner_des3PsnId?exists><a href="/scmwebsns/resume/psnView?des3PsnId=${resOwner_des3PsnId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" class="Blue">${resOwner_psnEnName }</a>'s </#if><#switch resType><#case 0>status<#break><#case 1>publication<#break><#case 2>reference<#break><#case 3>file<#break><#case 4>project<#break><#case 5>course material<#break><#case 8>group<#break><#case 9>resume<#break><#case 19>work experience<#break><#case 20>education<#break><#case 21>research areas<#break><#default></#switch>:
      </div>
         ...
         <#list resReplyDetails as resReply>
          <dd class="reply_item_${resReply.recordId?c}">
            ...
            <div class="t_message2">
              ...
              <p class="en10">${resReply.replyDate}&nbsp;&nbsp;<span class="reply_operation" style="display:none;"><a style="cursor:pointer" receiverPsnName="${resReply.replyerEnName}" receiverPsnLink="/scmwebsns/resume/psnView?des3PsnId=${resReply.des3ReplyerId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" onclick="dynMsgUtil.replyToPsn('${resId?c}','${resType}','${resNode}','${dynId?c}','${resReply.des3ReplyerId}',event, this)" class="Blue" title="Reply"><i class="evaluate-icon"></i></a>&nbsp;<#if (isResOwner==1 || resReply.replyerId==currentPsnId)><a onclick="dynMsgUtil.deleteReply('${dynId?c}','${resReply.recordId?c}','${resId?c}','${resType}','${resNode}',event, this)" style="cursor:pointer" class="Blue" title="Delete"><i class="delete-icon"></i></a></#if></span></p>
            </div>
          </dd>
    ...
    <div class="drop_down"><a style="cursor:pointer;display:none;" onmouseover="dynMsgUtil.extendDynOperation(this)" onmouseout="dynMsgUtil.shrinkDynOperation(this)" class="link_delete"></a>
        ...
        <div class="drop_jt_en_US"></div>
           <ul>
             <#if (isMine==1)>
               <li><a style="cursor:pointer; border-width:0;" onclick="dynMsgUtil.ajaxDeleteDyn('${dynId?c}')"><i class="img_nochoose"></i>Delete</a></li>
               <!--<li><a style="cursor:pointer;" onclick="dynMsgUtil.ajaxShieldDynByType('','${dynId?c}')"><i class="img_nochoose"></i>Shield such update</a></li>-->
             <#else>
               <li><a style="cursor:pointer; border-width:0;" onclick="dynMsgUtil.ajaxDeleteDyn('${dynId?c}')"><i class="img_nochoose"></i>Delete</a></li>
