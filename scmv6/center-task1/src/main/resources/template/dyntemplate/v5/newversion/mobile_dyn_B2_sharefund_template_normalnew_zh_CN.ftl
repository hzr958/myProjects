<input type="hidden" name="dynId" value="${DYN_ID?c }" resType="${RES_TYPE}" resId="${RES_ID?c}" dynType="${DYN_TYPE}" databaseType="${PDWH_URL ?if_exists}" parentDynId="${PARENT_DYN_ID?c}" dbId ='${DB_ID?if_exists}'/>
                  ...
                  <div class="pub-idx__base-info">
                    ...
                    <div class="pub-idx__main_box" onclick="dynamic.openDetail('${DES3_DYN_ID}','${DYN_TYPE}','${DES3_RES_ID}','${RES_TYPE}',event)">
                      <div class="pub-idx__main">
						<div class="pub-idx__main_title">${FUND_TITLE_ZH?if_exists }</div>
        ...
        <div class="dynamic-social__list">
             	...
             	<div class="dynamic-social__item dynamic-social__item-normal-size  dev_awardcount_${DYN_ID?c}"   onclick="dynamic.awardDyn( ${RES_TYPE},${RES_ID?c}, '${DYN_TYPE}', ${DYN_ID?c},${PARENT_DYN_ID?c},event)"><a>赞</a></div>
          	 
            	<!--  <div class="dynamic-social__item  dev_commentcount_${DYN_ID?c }"  onclick="dynamic.replyDyn(${RES_TYPE},'${RES_ID}', '${DYN_TYPE}', ${DYN_ID?c},event)"><a>评论</a></div> -->
                ...
                <div class="dynamic-social__item dynamic-social__item-normal-size collectCancel_${FUND_ID?c }" style="display:none;" id="collectCancel_${DYN_ID?c }"  parentDynId="${PARENT_DYN_ID?c}"   onclick="javascript:FundRecommend.dynCollectCoperation($(this), '${ENCODE_FUND_ID?if_exists}', 1, '${FUND_ID?c }');"><a>取消收藏</a></div>
          		<div class="dynamic-social__item dynamic-social__item-normal-size collect_${FUND_ID?c }" id="collect_${DYN_ID?c }"  parentDynId="${PARENT_DYN_ID?c}"   onclick="javascript:FundRecommend.dynCollectCoperation($(this), '${ENCODE_FUND_ID?if_exists}', 0, '${FUND_ID?c }');"><a>收藏</a></div>
        </div>