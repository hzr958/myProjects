<li onmouseover="dynMsgUtil.showDynOperation(this)"  onmouseout="dynMsgUtil.hideDynOperation(this)">
    ...
    <div class="publish-nr">
      <div class="moving-contat"><a href="/scmwebsns/resume/psnView?des3PsnId=${des3PsnId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" class="Blue">${psnEnName}</a> shared resume: ${sayContent}</div>
      <div class="reproduced">
        ...
        <div class="dyn_introduction">
         ...
         <#if (resumePdf?exists)>
		 <a href="${resumePdf}" class="Blue" target="_self">${resumeName}.pdf</a><br/>
		 </#if>
		  <#if (resumeDoc?exists)>
		 <a href="${resumeDoc}" class="Blue" target="_self">${resumeName}.doc</a>
		 </#if>
        </div>
