    ...
    <div class="publish-nr">
      <div class="moving-contat">
       <#assign splitChar></#assign>
        <#list replyerDetails as replyer><#if (!replyer_has_next) && (replyer_index!=0)> and <#else>${splitChar}</#if><a href="/scmwebsns/resume/psnView?des3PsnId=${replyer.replyer_des3PsnId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" class="Blue">${replyer.replyer_psnEnName}</a><#if replyer_has_next><#assign splitChar>, </#assign></#if></#list>&nbsp;commented&nbsp;<a  href="/scmwebsns/resume/psnView?des3PsnId=${replyObjPsn_des3PsnId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" class="Blue">${replyObjPsn_psnEnName}</a>:
      </div>
