        ...
        <div class="dynamic-social__list">
          <div class="dynamic-social__item dynamic-social__item_mobile dev_awardcount_${DYN_ID?c}" style="padding:0px!important; font-size: 12px;"  onclick="dynamic.awardDyn( ${RES_TYPE},${RES_ID?c}, '${DYN_TYPE}', ${DYN_ID?c},${PARENT_DYN_ID?c},event)"><a>Like</a></div>
          <div class="dynamic-social__item dynamic-social__item_mobile dev_commentcount_${DYN_ID?c }" style="padding:0px!important; font-size: 12px;" onclick="dynamic.replyDyn(${RES_TYPE},${RES_ID?c}, '${DYN_TYPE}', ${DYN_ID?c},event)"><a>Comment</a></div>
          ...
          <#if (FUND_ID?exists)>
			...
			<div class="dynamic-social__item dynamic-social__item_mobile collectCancel_${FUND_ID?c }" style="padding:0px!important; font-size: 12px;display:none;" id="collectCancel_${DYN_ID?c }"  parentDynId="${PARENT_DYN_ID?c}"   onclick="javascript:FundRecommend.dynCollectCoperation($(this), '${ENCODE_FUND_ID?if_exists}', 1, '${FUND_ID?c }');"><a>Cancel</a></div>
          	<div class="dynamic-social__item dynamic-social__item_mobile collect_${FUND_ID?c }" style="padding:0px!important; font-size: 12px;" id="collect_${DYN_ID?c }"  parentDynId="${PARENT_DYN_ID?c}"   onclick="javascript:FundRecommend.dynCollectCoperation($(this), '${ENCODE_FUND_ID?if_exists}', 0, '${FUND_ID?c }');"><a>Save</a></div>
          </#if> 
