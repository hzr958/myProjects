        ...
        <div class="dynamic-social__list">
          ...
          <div class="dynamic-social__item dynamic-social__item_mobile dev_commentcount_${DYN_ID?c }" style="padding:0px!important; font-size: 12px;" onclick="dynamic.replyDyn(${RES_TYPE},'${DES3_RES_ID}', '${DYN_TYPE}', '${DES3_DYN_ID}',event)"><a>评论</a></div>
           <#if (RES_TYPE!=0)>
          <div class="dynamic-social__item dynamic-social__item_mobile dev_sharecount_${DYN_ID?c }"  style="padding:0px!important; font-size: 12px;"  resType="${RES_TYPE}"  resId="${RES_ID?c}"  dynType="${DYN_TYPE}" dynId="${DYN_ID?c}" dbId ='${DB_ID?if_exists}' databaseType="${PDWH_URL ?if_exists}" parentDynId="${PARENT_DYN_ID?c}"  
          	 onclick="dynamic.shareDynMain('${DES3_DYN_ID}','${DYN_TYPE}','${DES3_RES_ID}','${RES_TYPE}',event);"
           ><a>分享</a></div>
           <div class="dynamic-social__item dynamic-social__item_mobile dev_save" onClick="dynamic.dynCollectPub('${DES3_DYN_ID}','${DYN_TYPE}','${DES3_RES_ID}','${RES_TYPE}',event);" style="padding:0px!important; font-size: 12px;"><a>收藏</a></div>
