 ...
 * @date May 24, 2019
 */
@Service("shareFundToFriendService")
@Transactional(rollbackFor = Exception.class)
public class ShareFundToFriendService extends ShareToSmateService {
  ...
  boolean checkData(SmateShareVO vo) {
    ...
    // 资源json格式信息
    if (StringUtils.isBlank(vo.getResInfoJson())) {
      vo.setErrorCode(SmateShareConstant.SHARE_ERROR_CODE_NULL_JSONINFO);
      vo.setErrorMsg(SmateShareConstant.SHARE_ERROR_MSG_NULL_JSONINFO);
    }
    return true;
  }
  ...
  Map<String, Object> shareRes(SmateShareVO vo) {
    ...
    Map<String, Object> resultMap =
        restTemplate.postForObject(vo.getDomainMobile() + SmateShareConstant.SHARE_FUND_TO_FRIENDS_URL,
  ...
  private MultiValueMap<String, String> buildShareParams(SmateShareVO vo) {
    ...
    params.add("des3FundIds", StringUtils.trimToEmpty(vo.getDes3ResId()));
    params.add("des3ReceiverIds", StringUtils.trimToEmpty(vo.getDes3FriendIds()));
    // params.add("fundNames", groupFile.getFileName());
