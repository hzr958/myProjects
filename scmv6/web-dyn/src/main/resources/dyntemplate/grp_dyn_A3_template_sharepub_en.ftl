<input type="hidden" name="dynId" value="${GROUP_DYN_ID?if_exists }" dynOwnerDes3Id="${DYN_OWNER_DES3_ID?if_exists}" resOwnerDes3Id="${RES_OWNER_DES3ID?if_exists}" resType="${RES_TYPE?if_exists}" dbId="${DB_ID ?if_exists}" des3ResId="${RES_ID?if_exists}" resId="${RES_ID?if_exists}" databaseType="${PDWH_URL ?if_exists}" dynType="${DYN_TYPE?if_exists}" />
								...
								<div class="pub-idx__main">
									<div class="pub-idx__main_title">
										<#if (PDWH_URL ?exists) && PDWH_URL==2>
											<a onclick='DiscussOpenDetail.openPdwhDetail("${RES_ID?if_exists}",event)'>
										<#else>
											<#if (PUB_INDEX_URL?exists)>
												<a onclick='DiscussOpenDetail.shortUrlOpenPubDetail("${PUB_INDEX_URL?if_exists}",event)'>
											<#else>
												<a onclick='DiscussOpenDetail.openPubDetail("${RES_ID?if_exists}",event)'>
											</#if>
										</#if>
