<input type="hidden" name="dynId" value="${GROUP_DYN_ID?if_exists }" dynOwnerDes3Id="${DYN_OWNER_DES3_ID?if_exists}" resType="${RES_TYPE?if_exists}" des3ResId="${RES_ID?if_exists}" resOwnerDes3Id="${RES_OWNER_DES3ID?if_exists}" resId="${RES_ID?if_exists}" dbId="${DB_ID ?if_exists}" databaseType="${PDWH_URL ?if_exists}" dynType="${DYN_TYPE?if_exists}" />
