        <input type="hidden" name="dynId" value="${DYN_ID?c }" dynOwnerDes3Id="${DYN_OWNER_DES3_ID?if_exists}" resOwnerDes3Id="${RES_OWNER_DES3ID?if_exists}" resType="${RES_TYPE}" des3ResId="${DES3_RES_ID?if_exists}"  des3DynId="${DES3_DYN_ID?if_exists}" resId="${RES_ID?c}" dynType="${DYN_TYPE}" parentDynId="${PARENT_DYN_ID?c}"/>
          ...
          <div class="dynamic-content__post" >
            ...
            <div class="dynamic-post__main" >
              <div class="dynamic-post__author" > 
              	<a class="dynamic-post__author_name"  title="${ PERSON_NAME_ZH?if_exists }" onclick="dynamic.openPsnDetail('${DES3_PRODUCER_PSN_ID}' ,event)">${ PERSON_NAME_ZH?if_exists }</a>
        ...
        <div class="dynamic-social__list">
          <div class="dynamic-social__item  dev_awardcount_${DYN_ID?c}"   onclick="dynamic.awardDyn(${RES_TYPE},'${DES3_RES_ID}','${DYN_TYPE}','${DES3_DYN_ID}','${DES3_PARENT_DYN_ID}',event)"><a>赞</a></div>
           ...
           <#if (RES_TYPE==26)> 
            <div class="dynamic-social__item menuList_list-container_none dev_viewcount_${DYN_ID?c }" onclick=""><a>查看</a></div>
            </#if>
