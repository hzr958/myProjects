<input type="hidden" name="dynId" value="${GROUP_DYN_ID?if_exists }" dynOwnerDes3Id="${DYN_OWNER_DES3_ID?if_exists}" des3ResId="${DES3_RES_ID?if_exists}" resOwnerDes3Id="${RES_OWNER_DES3ID?if_exists}" resType="${RES_TYPE?if_exists}" resId="${RES_ID?if_exists}" dynType="${DYN_TYPE?if_exists}" />
<div class="dynamic-content">
	...
	<div class="dynamic-content__main">
		<div class="dynamic-main__box">
			<div class="dyn_content">${GROUP_DYN_CONTENT?if_exists }</div>
			<#if (RES_ID?exists)>
								...
								<div class="pub-idx__main">
									<div class="pub-idx__main_title">
										<#if (PUB_INDEX_URL?exists)>
											<a onclick='DiscussOpenDetail.shortUrlOpenPubDetail("${PUB_INDEX_URL?if_exists}",event)'>${EN_RES_NAME?if_exists}</a>
										<#else>
											<a onclick='DiscussOpenDetail.openPubDetail("${RES_ID?if_exists}",event)'>${EN_RES_NAME?if_exists}</a>
										</#if>
									</div>
