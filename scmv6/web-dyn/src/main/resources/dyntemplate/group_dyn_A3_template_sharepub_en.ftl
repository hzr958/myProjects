	<input type="hidden" name="dynId" value="${GROUP_DYN_ID ?if_exists}" des3ResId="${DES3_RES_ID?if_exists}" resType="${RES_TYPE?if_exists}" resId="${RES_ID?if_exists}" dynType="${DYN_TYPE?if_exists}"/>
      <div class="dynamic_header">
       ...
       <div class="author_information">
         <div class="action" >
          <div class="name" onclick='groupDynamic.openPsnDetail("${DES3_PSN_ID ?if_exists}",event)' style='cursor: pointer;'>${EN_AUTHOR_NAME?if_exists }</div>
          <div class="type"> shared a publication</div>
        </div>
   ...
   <div class="dynamic_content_container">
    ...
    <#if (RES_ID?exists&&GROUP_DYN_CONTENT?exists&&GROUP_DYN_CONTENT!='')>
    ...
    <div class="attachment">
      <div class="pub_whole">
        ...
        <div class="pub_information">
         	<div class="title">
         		<span onclick='groupDynamic.openPubDetail("${RES_ID?if_exists}",event)' style='cursor: pointer;'>${EN_RES_NAME?if_exists}</span>
         	</div>
        	<div class="author">${AUTHOR_NAMES?if_exists}</div>
         	<div class="source">${EN_RES_DESC?if_exists}</div>
       </div>	
     </div>
   </div>
   </div>
