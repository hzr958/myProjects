...
import com.smate.center.open.dao.publication.PubAssignLogDetailDao;
...
/**
 * 2018-04-13 获取个人所有待认领的成果，需要insid权限判断
 * 
 * @author aijiangbin
 *
 */
...
public class ObtainRcmdConfirmPubInfoServiceImpl extends ThirdDataTypeBase {
  ...
  public Map<String, Object> doHandler(Map<String, Object> paramet) {
    ...
    pubQueryDTO.setIsAll(1);// 查看全部
    pubQueryDTO.setIsPage(1);// 不分页查询
    pubQueryDTO.setServiceType(V8pubQueryPubConst.QUERY_PUB_UNCONFIRM_LIST_IGNORE_STATUS);
    pubQueryDTO.setPubType(com.smate.core.base.utils.string.StringUtils.IsNotBlankObject(paramet.get("pubType"))
        ? paramet.get("pubType").toString()
        : "");
      ...
      if (resultList != null && resultList.size() > 0) {
        for (Map<String, Object> pubMap : resultList) {
          try {
            ...
            map.put("serviceType", V8pubQueryPubConst.OPEN_PDWH_PUB);
            ...
            dataMap.put("pubConfirmId", pubMap.get("pubConfirmId"));
  ...
  private Map<String, Object> parseXmlToMap(PubDetailVO pubDetailVO, Person person) throws Exception {
    ...
    if (pubDetailVO != null) {
      ...
      ConstPubType constPubType = constPubTypeDao.get(pubDetailVO.getPubType());
      dataMap.put("pub_type", constPubType != null ? constPubType.getZhName() : "");
      dataMap.put("impactFactors", pubDetailVO.getImpactFactors());
      // 默认返回 3, "基准库导入" SCM-22078
      dataMap.put("record_from", 3);
   ...
   * @param pubDetailVO
   */
  public void buildPubMember(Map<String, Object> dataMap, PubDetailVO pubDetailVO, Person person) {
    ...
    if (pubMemberList != null && pubMemberList.size() > 0) {
      for (int j = 0; j < pubMemberList.size(); j++) {
        ...
        if (StringUtils.isNotBlank(psnName) && person != null) {
          psnName = psnName.trim();
          List<PubAssignLogDetail> detailList = pubAssignLogDetailDao.findByPubIdAndPsnId(pubDetailVO.getPubId(), person.getPersonId());
          if(CollectionUtils.isNotEmpty(detailList)){
            for(PubAssignLogDetail detail : detailList){
              if(detail.getMatchedName() != null && detail.getMatchedName().trim().equalsIgnoreCase(psnName)){
