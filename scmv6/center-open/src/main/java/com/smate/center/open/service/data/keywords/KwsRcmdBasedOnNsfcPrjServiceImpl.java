...
public class KwsRcmdBasedOnNsfcPrjServiceImpl extends ThirdDataTypeBase {
  ...
  public Map<String, Object> doHandler(Map<String, Object> parameter) {
    ...
    try {
      if (StringUtils.isEmpty(discode)) {
        dataList = this.nsfcKeywordsService.getRelatedKw(kwStr, null, null);
        msg = "无nsfc学科代码约束，推荐关键词成功";
      } else {
        dataList = this.nsfcKeywordsService.getRelatedKw(kwStr, discode, queryType);
        msg = "nsfc学科代码为" + discode + "，推荐关键词成功";
      }
    } catch (Exception e) {
      logger.error("nsfc关键词推荐出错", e);
      return errorMap("nsfc关键词推荐出错", parameter, "nsfc关键词推荐出错");
    }
