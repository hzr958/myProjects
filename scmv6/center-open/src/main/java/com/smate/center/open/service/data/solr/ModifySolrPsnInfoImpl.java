...
@Deprecated
public class ModifySolrPsnInfoImpl extends ThirdDataTypeBase {
  ...
  public Map<String, Object> doVerify(Map<String, Object> parameter) {
    ...
    if (parameter.get(SolrConstants.SOLR_PSN_ID) == null) {
      ...
      temp = super.errorMap(OpenMsgCodeConsts.SCM_295, parameter, "SCM_295  solr更新人员信息,服务参数  psn_id 不能为空");
      return temp;
    }
  ...
  public Map<String, Object> doHandler(Map<String, Object> parameter) {
    ...
    try {
      this.userService.updateSolrPsnInfo(psnId);
    } catch (Exception e) {
      // 返回结果list，为2时说明solr处理出错
      logger.error("通过open系统solr更新人员信息出错", e);
      data.put(SolrConstants.SOLR_RETURN_STATUS, 2);
      dataList.add(data);
      return successMap("通过open系统solr更新人员信息出错", dataList);
    }
    data.put(SolrConstants.SOLR_RETURN_STATUS, 1);
    dataList.add(data);
    return successMap("通过open系统solr更新人员信息成功", dataList);
  }
