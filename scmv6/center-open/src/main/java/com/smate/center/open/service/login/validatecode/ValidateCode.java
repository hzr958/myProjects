package com.smate.center.open.service.login.validatecode;
...
public class ValidateCode implements Filter {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    if (cxt != null && cxt.getBean(OpenCacheService.class) != null && openCacheService == null) {
      openCacheService = (OpenCacheService) cxt.getBean(OpenCacheService.class);
    }
    String sessionId = this.findServletRequestSessionId(request);
    openCacheService.put(SecurityConstants.OAUTH_VALIDATE_CODE, 5 * 60, sessionId, sRand);
    g.dispose();
  ...
  /**
   * 优先从cookie中获取sessionId，因为直接用ServletRequest强转成的HttpServletRequest获取到的sessionId会改变
   * 
   * @param request
   * @return
   */
