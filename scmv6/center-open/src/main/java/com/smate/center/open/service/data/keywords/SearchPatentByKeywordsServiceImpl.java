...
public class SearchPatentByKeywordsServiceImpl extends ThirdDataTypeBase {
  @Value("${domainscm}")
  ...
  public Map<String, Object> doVerify(Map<String, Object> parameter) {
    ...
    if (parameter.get(KeywordConstants.KEYWORDS) == null) {
      logger.error("根据关键词检索专利，关键词列表 keywordList 不能为空  ");
      temp = super.errorMap(OpenMsgCodeConsts.SCM_799, parameter, "SCM_799  根据关键词检索专利，关键词列表 keywordList 不能为空  ");
      return temp;
    ...
    if (parameter.get(KeywordConstants.START_YEAR) == null) {
      logger.error("根据关键词检索专利，分析周期开始年份 startYear 不能为空 ");
      temp =
          super.errorMap(OpenMsgCodeConsts.SCM_797, parameter, "SCM_797  根据关键词检索专利，服务参数     分析周期开始年份 startYear不能为空 ");
      return temp;
    ...
    if (parameter.get(KeywordConstants.END_YEAR) == null) {
      logger.error("根据关键词检索专利，分析周期结束年份 endyear 不能为空 ");
      temp = super.errorMap(OpenMsgCodeConsts.SCM_798, parameter, "SCM_798 根据关键词检索专利，服务参数    分析周期结束年份 endyear不能为空 ");
      return temp;
  ...
  public Map<String, Object> doHandler(Map<String, Object> parameter) {
    ...
    try {
      Map<String, Object> solrMap = solrIndexService.queryPatents(1, 3, queryFields);
      fillPatentData(solrMap, dataList);
    } catch (SolrServerException e) {
      logger.error("根据关键词检索专利数据出错", e);
      return errorMap("根据关键词检索专利数据出错", parameter, e.toString());
    }

    return successMap("根据关键词检索专利数据成功", dataList);
  }

  private void fillPatentData(Map<String, Object> solrMap, List<Map<String, Object>> dataList) {
    ...
    if (flag) {
      ...
      if (itemsList != null && itemsList.size() > 0) {
        for (Map<String, Object> map : itemsList) {
          ...
          if (map.get("patId") != null && NumberUtils.isNumber(map.get("patId").toString())) {
            dataMap.put("patId", NumberUtils.toLong(map.get("patId").toString()));
          }
          if (map.get("patTitle") != null) {
            dataMap.put("zhPatTitle", map.get("patTitle").toString());
          } else {
            dataMap.put("zhPatTitle", "");

          }
          if (map.get("patTitle") != null) {
            dataMap.put("enPatTitle", map.get("patTitle").toString());
          } else {
            dataMap.put("enPatTitle", "");
          }
          if (map.get("patAuthors") != null) {
            dataMap.put("patAuthorNames", map.get("patAuthors").toString());
          } else {
            dataMap.put("patAuthorNames", "");
          }
          if (map.get("patBrief") != null) {
            dataMap.put("zhPatBrief", map.get("patBrief").toString());
          } else {
            dataMap.put("zhPatBrief", "");
          }

          if (map.get("patBrief") != null) {
            dataMap.put("enPatBrief", map.get("patBrief").toString());
          } else {
            dataMap.put("enPatBrief", "");
          }
          ...
          pubQueryDTO.setSearchPubId(NumberUtils.toLong(map.get("patId").toString()));
