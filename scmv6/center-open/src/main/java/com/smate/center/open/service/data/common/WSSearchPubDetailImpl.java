...
public class WSSearchPubDetailImpl extends ThirdDataTypeBase {
  ...
  public Map<String, Object> doVerify(Map<String, Object> paramet) {
    ...
    if (paramet.get(OpenConsts.MAP_DATA) == null && StringUtils.isEmpty(paramet.get(OpenConsts.MAP_DATA).toString())) {
      logger.error("未关联账号，根据成果ID返回成果详情,服务码:a5c9df5a");
      temp = super.errorMap(OpenMsgCodeConsts.SCM_209, paramet, "未关联账号，根据成果ID返回成果详情,服务码:a5c9df5a");
      return temp;
    } else {
  ...
  public Map<String, Object> doHandler(Map<String, Object> paramet) {
    ...
    try {
      ...
      String pubListXml = pubInfoService.searchPubDetail(pubIDS, psnID);
      Map<String, Object> temp = new HashMap<String, Object>();
      temp.put(OpenConsts.RESULT_STATUS, OpenConsts.RESULT_STATUS_SUCCESS);
      temp.put(OpenConsts.RESULT_MSG, "scm-0000");// 响应成功
      temp.put(OpenConsts.RESULT_DATA, pubListXml);
      dataList.add(temp);
    } catch (Exception e) {
      e.printStackTrace();
      throw new OpenNsfcException("未关联账号，根据成果ID返回成果详情失败", e);
    }
