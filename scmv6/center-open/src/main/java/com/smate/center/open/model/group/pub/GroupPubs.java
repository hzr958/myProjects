...
public class GroupPubs implements Serializable {
   ...
   * 
   */
  private static final long serialVersionUID = -7886317297776714517L;
  // 群组成果ID
  ...
  private Long labeled = 0L; // 是否 标注 0没有标注
  private Long relevance = 0L; // 相关度
  private Date createDate = new Date();// 成果添加到群组的时间
  private Date updateDate;// 成果更新的时间
  ...
  public GroupPubs(Long pubId, Long labeled, Long relevance) {
    super();
    this.pubId = pubId;
  ...
  public GroupPubs(Publication pub, Long groupId, Integer nodeId) {

    this.groupId = groupId;
    this.pubId = pub.getPubId();
    this.ownerPsnId = pub.getOwnerPsnId();
    this.authorNames = pub.getAuthorNames();
    this.briefDesc = pub.getBriefDesc();
    this.publishYear = pub.getPublishYear();
    this.publishMonth = pub.getPublishMonth();
    this.publishDay = pub.getPublishDay();
    this.citedTimes = pub.getCitedTimes();
    this.impactFactors = pub.getImpactFactors();
    this.zhTitle = pub.getZhTitle();
    this.enTitle = pub.getEnTitle();
    this.nodeId = nodeId;
    this.typeId = pub.getPubType();
    this.fulltextFileid = pub.getFullTextField();
    this.fulltextUrl = pub.getFullTextUrl();
    this.citedList = pub.getCitedList();
    this.citedUrl = pub.getCitedUrl();
    this.citedDate = pub.getCitedDate();
    this.setFulltextExt(pub.getFullTextFileExt());
  }
