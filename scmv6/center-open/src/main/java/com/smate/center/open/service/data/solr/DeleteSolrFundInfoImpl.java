...
public class DeleteSolrFundInfoImpl extends ThirdDataTypeBase {
  ...
  public Map<String, Object> doVerify(Map<String, Object> parameter) {
    ...
    if (parameter.get(SolrConstants.SOLR_FUND_ID) == null) {
      logger.error("solr删除基金信息，服务参数  fund_id 不能为空 ");
      temp = super.errorMap(OpenMsgCodeConsts.SCM_2955, parameter, "SCM_2955  solr删除人员信息,服务参数  fund_id 不能为空");
      return temp;
    }
  ...
  public Map<String, Object> doHandler(Map<String, Object> parameter) {
    ...
    try {
      this.fundService.deleteSolrFundInfo(fundId);
    } catch (Exception e) {
      // 返回结果list，为2时说明solr处理出错
      logger.error("通过open系统solr删除基金信息出错", e);
      data.put(SolrConstants.SOLR_RETURN_STATUS, 2);
      dataList.add(data);
      return successMap("通过open系统solr删除基金信息出错", dataList);
    }
    data.put(SolrConstants.SOLR_RETURN_STATUS, 1);
    dataList.add(data);
    return successMap("通过open系统solr删除基金信息成功", dataList);
  }
