...
public class Config implements Serializable {
  ...
  public static void main(String[] args) {
    ...
    config.setBrief(item);

    String str = JSONObject.fromObject(config).toString();
    JSONObject jobj = JSONObject.fromObject(str);
    Map<String, Class> m = new HashMap<String, Class>();
    m.put("ConfigElements", ConfigElement.class);
    Config config2 = (Config) JSONObject.toBean(jobj, Config.class, m);
    System.out.println(config2.getBrief().getConfigElements().get(0).getElemCode());
  }
