...
public class Message implements Serializable {
  ...
  private String desResRecId;// 加密的resRecId
  /** 分享类型：1成果，2文献，3文件. */
  private Integer shareType;
  // 根据分享消息跳转标识 1:分享消息只有1条 2:分享消息有多条
  private Integer shareTarget = 0;
  private Long resId;// 个人资源Id，即：（成果，项目，文献ID或文件ID）
  private String des3ResId;// 加密的个人资源Id，即：（成果，项目，文献ID或文件ID）
  private List<StationFile> fileList;// 分享文件列表
  private List<Publication> pubList;// 分享文件列表
  private String ids;// 分享消息的多个资源id
  // 移动端==================================================end
  ...
  public void setShareTarget(Integer shareTarget) {
  ...
  public String getDesResRecId() {
    return desResRecId;
  }

  public void setDesResRecId(String desResRecId) {
    this.desResRecId = desResRecId;
  }

  public Long getResRecId() {
    if (resRecId == null && StringUtils.isNotBlank(desResRecId)) {
      String desResRecIdStr = ServiceUtil.decodeFromDes3(desResRecId);
      if (StringUtils.isBlank(desResRecIdStr)) {
        return resRecId;
      } else {
        return Long.parseLong(desResRecIdStr);
      }
    }
    return resRecId;
  }
