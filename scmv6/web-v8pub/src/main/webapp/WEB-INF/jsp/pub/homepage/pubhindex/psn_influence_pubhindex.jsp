...
<script type="text/javascript">
    ...
    function shareCallback(dynId, shareContent, resId, pubId, isB2T,
            receiverGrpId) {
      debugger;
      var count = $.trim($('.dev_pub_share_' + pubId).text());
      if(count.indexOf('(1k+)')<0){
         count = Number($('.dev_pub_share_' + pubId).text().replace(/[\D]/ig,"")) + 1;
         if(count>=1000){
           count = "1k+";  
         }
         $('.dev_pub_share_' + pubId).text(shareI18 + "(" + count + ")");
    ...
    function awardCallback(obj,isAward,awardCount){
      if (isAward == 1) {// 由取消赞变成赞
        $(obj).find(".fabulous_icon").removeClass("fabulous_icon-checked");
        if (awardCount == 0) {
          $(obj).find(".dev_pub_award_item").text(pubi18n.i18n_like);
          $(obj).find(".new-Standard_Function-bar_item-title").text(pubi18n.i18n_like);
        } else {
          if(awardCount>=1000){
            awardCount = "1k+";  
          }
          $(obj).find(".dev_pub_award_item").text(pubi18n.i18n_like + "(" + awardCount + ")");
        ...
        $(obj).find(".fabulous_icon").addClass("fabulous_icon-checked");
        $(obj).find(".dev_pub_award_item").text(pubi18n.i18n_unlike + "(" + awardCount + ")");
      }
    };
</script>
        ...
        <div class="container__effect-hindex_right-item_content">
          ...
          <div class="container__effect-hindex_right-item_inforbox">
            <div
              ...
              <div class="multipleline-ellipsis__content-box dev_inf_pub_title" style="white-space: normal;"
                title="<c:out value="${pub.title}"/>">
            ...
            <div class="container__effect-hindex_right-item_footer">
              <c:if test="${pubVo.self == 'no'}">
                <div class="dev_award container__effect-hindex_right-item_footer-item" isAward="${pub.isAward}"
                  ...
                  <c:if test="${pub.isAward == 0}">
                    ...
                    <span style="cursor: pointer;"
                      ...
                      <c:if test="${pub.awardCount > 0 && pub.awardCount<1000}">(${pub.awardCount})</c:if>
                      <c:if test="${pub.awardCount >= 1000}">(1k+)</c:if>
                    </span>
                  </c:if>
                ...
                <div  class="container__effect-hindex_right-item_footer-item" style="margin-left: 8px;" onclick="Pub.pubCite(this);"
                  pubMainListIsAnyUser="${pub.isAnyUser}" type="list" windowType="pub" owner="${pubVo.self}" resId="${pub.des3PubId}"
                  ...
                  <span style="cursor: pointer;"
                    ...
                    <c:if test="${pub.shareCount > 0 && pub.shareCount<1000}">(${pub.shareCount})</c:if>
                    <c:if test="${pub.shareCount >= 1000}">(1k+)</c:if>
                  </span>
                </div>
              </c:if>
              <c:if test="${pubVo.self == 'yes'}">
                <div class="container__effect-hindex_right-item_footer-item" onclick="Pub.pubCite(this);"
                pubMainListIsAnyUser="${pub.isAnyUser}" type="list" windowType="pub" owner="${pubVo.self}" resId="${pub.des3PubId}"
                  resType="1" pubId="${pub.pubId}">
                  <div style="cursor: pointer;" class="share_icon"></div>
                  <span style="cursor: pointer;"
