...
<input name="isCollected" type="hidden" value="${pubInfo.isCollected }" />
<input name="isAward" type="hidden" value="${pubInfo.isAward }" />
<div class="new-Standard_Function-bar">
  ...
  </a> <a href="${pubInfo.pubIndexUrl}<c:if test='${not empty pubInfo.pubIndexUrl}'>?type=comment</c:if>" class="manage-one mr20" target="_blank">
  ...
  </a> <a class="manage-one mr20 thickbox" onclick="Pub.showPdwhQuote('/pub/ajaxpdwhpubquote','${pubInfo.des3PubId}',this)"
    title="<spring:message code="pub.search.citations" />">
    <div class="new-Standard_Function-bar_item">
      <i class="new-Standard_function-icon new-Standard_Quote-icon"></i> <span
        class="new-Standard_Function-bar_item-title"><spring:message code="pub.search.citations" /> </span>
    </div>
  </a> <a href="javascript:;" class="manage-one mr20" collected="${pubInfo.isCollected }"
    onclick="Pub.dealCollectedPub('${pubInfo.des3PubId}','PDWH',this)"> <c:if test="${pubInfo.isCollected== 0}">
      <div class="new-Standard_Function-bar_item">
        <i class="new-Standard_function-icon new-Standard_Save-icon"></i> <span
          class="new-Standard_Function-bar_item-title"><spring:message code="pub.search.collect" /></span>
      </div>
    </c:if> <c:if test="${pubInfo.isCollected== 1 }">
      <div class="new-Standard_Function-bar_item new-Standard_Function-bar_selected">
        <i class="new-Standard_function-icon new-Standard_Save-icon"></i> <span
          class="new-Standard_Function-bar_item-title"><spring:message code="pub.search.unsave" /></span>
      </div>
    </c:if>
  </a>
</div>
