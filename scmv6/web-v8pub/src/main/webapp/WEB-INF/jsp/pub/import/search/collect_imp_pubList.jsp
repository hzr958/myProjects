  ...
  <div class="import_Achieve-item-body_content <c:if test='${pubInfo.authermatch !=1 }'>bg01</c:if>">
    ...
    <div class="item_content item_content-paper">
      <select name="pubtype" class="pubtype inp_text"
        ...
        onchange="checkPubTypeDup(this.value,'${pubInfo.index}','${pubInfo.pubId}');">
    ...
    <c:if test="${not empty pubInfo.dupPubId }">
      <div class="item_content item_content-same">
        <input type="hidden" id="dup_pubId_${pubInfo.pubId}" name="dup_pubId" value="<iris:des3 code='${pubInfo.dupPubId}'/>" />
        ...
        <div style="width: 100%; text-align: center; color: red;">
          <spring:message code='referencesearch.import.same' />
        </div>
        <select class="repeatSelect" onclick="selectTip(this)"
          ...
          <option value="1"><spring:message code='referencelist.select.update' /></option>
          <option value="2"><spring:message code='referencelist.select.notduplicated' /></option>
          <option value="0"><spring:message code='referencelist.select.skip' /></option>
        </select>
        <div style="width: 100%; text-align: center; color: gray;">
          <spring:message code='referencelist.label.overwrite' />
        </div>
      </div>
    </c:if>
  </div>
