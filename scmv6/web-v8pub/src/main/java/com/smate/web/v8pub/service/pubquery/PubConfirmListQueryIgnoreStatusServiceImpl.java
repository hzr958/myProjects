...
public class PubConfirmListQueryIgnoreStatusServiceImpl extends AbstractPubQueryService {
  ...
  public void queryPubs(PubQueryDTO pubQueryDTO) {
    pubAssignLogService.queryPubConfirmIdIgnoreStatusList(pubQueryDTO);
    if (!CollectionUtils.isEmpty(pubQueryDTO.getPubIds())) {
      if (StringUtils.isNotBlank(pubQueryDTO.getPubType())) {
        pubPdwhService.findByIdsforSie(pubQueryDTO);
      } else {
  ...
  public PubListResult assembleData(PubQueryDTO pubQueryDTO) {
    ...
    if (pubList != null && pubList.size() > 0) {
      ...
      if (pubQueryDTO.getIsPage() == 1) {
        buildpubConfirmId(list, pubQueryDTO);
      }
  ...
  private void buildpubConfirmId(List<PubInfo> list, PubQueryDTO pubQueryDTO) {
