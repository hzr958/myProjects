...
public class ASPsnStatisticsAddImpl implements PubHandlerAssemblyService {
    ...
    try {
      if (pub.pubGenre != PubGenreConstants.GROUP_PUB) {
        ...
        if (psnStatistics != null) {
          ...
          if (pub.pubHandlerName.equals(PubHandlerEnum.SAVE_SNS.name)) {
            int newCitedSum = psnStatistics.getCitedSum() + pub.citations;
      ...
      logger.error("更新人员统计数出错！psnId={}", pub.psnId, e);
