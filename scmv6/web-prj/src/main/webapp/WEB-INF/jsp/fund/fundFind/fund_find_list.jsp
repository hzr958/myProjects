...
<table border="0" cellpadding="0" cellspacing="0" class="tab-style">
  <s:if test="fundInfoList != null && fundInfoList.size() > 0">
    <s:iterator value="fundInfoList" id="fund" status="sta">
      <tr class="main-list__item" des3id="<iris:des3 code='${fund.fundAgencyId }'/>" style="padding: 0px; width: 940px!important;">
                ...
                <div class="file-rigth-box">
                  ...
                  <div class="p1 file-rigth-container">
                    <a title="${fund.fundAgencyName }"
                      ...
                      target="_black">${fund.fundAgencyName }</a>
                  </div>
                  <!-- 基金科技领域 -->
                   <div class="p1 file-rigth-container file-right_contion" style="color: #999 !important;">
                    <span style="cursor: default" title='${fund.scienceAreas }'>
                        ${fund.scienceAreas }</span>
                  </div>
                </div>
                <div class="new-Standard_Function-bar">
                  <a style="${fund.hasAward ? 'display:none;' : ''}"
                    ...
                    <div class="new-Standard_Function-bar_item" style="margin-left: 0px;">
                      <i class="new-Standard_function-icon new-Standard_Praise-icon"></i> <span
                        class="new-Standard_Function-bar_item-title"> <s:text
                          name='homepage.fundmain.recommend.like' /><span class="awardOperateCount"> 
                    ...
                    <div class="new-Standard_Function-bar_item new-Standard_Function-bar_selected"
                      ...
                      <i class="new-Standard_function-icon new-Standard_Praise-icon"></i> <span
                        class="new-Standard_Function-bar_item-title"> <s:text
                          name='homepage.fundmain.recommend.unlike' /><span class="awardOperateCount"> 
                          <c:if test="${!empty fund.awardCount && fund.awardCount > 0 && fund.awardCount < 1000}">
                           (${fund.awardCount})
                          </c:if>
                          <c:if test="${!empty fund.awardCount && fund.awardCount > 1000}">
                    ...
                    <div class="new-Standard_Function-bar_item">
                      <i class="new-Standard_function-icon new-Standard_Share-icon"></i> <span
                        class="new-Standard_Function-bar_item-title"> <s:text
                          name='homepage.fundmain.recommend.share' /><span class="shareCount_${fund.fundId }"> 
        ...
        <td align="center" width="25%"  <c:if test="${!fund.isStaleDated }">style="color:#000; height: 111px; line-height: 111px; "</c:if>>${fund.showDate }</td>
  ...
  <s:else>
    <div id="div_noMoreRecord" style="width: 100%;  text-align: center; min-height:500px; display: flex; align-items: center; justify-content: center; ">
          <div class="no_effort-tip_box">
               <i class="no_effort-tip_avator"></i>
               <span class="no_effort-tip_text"><s:text name='homepage.fundmain.recommend.noMoreRecord' /></span>
          </div>
           <!--  暂无基金机会符合你的推荐条件，请在左边栏设置更多条件。 -->
    </div>
  </s:else>
</table>
