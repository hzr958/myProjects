    ...
    <div class="main-box">
      <form name="mainform" id="mainform" action="/prjweb/prj/save" method="post">
        ...
        <input type="hidden" id="des3Id" name="des3Id" value="<iris:des3 code="${prjId }"/>" /> <input type="hidden"
          ...
          id="groupFolderId" value="${groupFolderId }" /> <input type="hidden" name="grpId" id="grpId"
          value="${grpId }" /> <input type="hidden" name="prjNodeId" id="prjNodeId" value="${prjNodeId}" /> <input
          type="hidden" name="from" id="from" value="${from}" /> <input type="hidden" name="backType" id="backType"
          value="${backType}" /> <input type="hidden" name="des3GrpId" id="des3GrpId" value="${des3GrpId}" />
        <div class="bar_btn">
          <span class="Fleft bar_prjenter_width"> <a class="uiButton uiButtonConfirm saveOutputButton"
            style="cursor: pointer;"> <s:text name="projectEdit.button.save" />
          </a> <s:if test="backType>=1">
              <a class="uiButton" style="cursor: pointer;" onclick="back();"> <s:text
                  name="projectEdit.button.back1" />
              </a>
            </s:if> <s:else>
              <a class="uiButton" style="cursor: pointer;" onclick="back();"> <s:text name="projectEdit.button.back" />
              </a>
            </s:else>
          </span> <span class="Fleft prjStateError" style="display: none;"> <label class="error"> <s:text
                name="project.state.error" />
          </label>
          </span>
          <c:if test="${!empty prjId}">
            <span class="Fright mtop5"> <span class="f666"> <s:text name="projectEdit.label.serialno" />
                <s:text name="colon.all" />${prjId }</span>
            </span>
          </c:if>
        </div>
        <c:if test="${!empty prjId}">
        ...
        <div class="Menubox">
          ...
          <span class="Fright"> <s:text name="projectEdit.label.mandatory" />
          </span>
        </div>
        ...
        <div class="bar_btn2">
          <span class="bar_width"> <a class="uiButton uiButtonConfirm saveOutputButton" style="cursor: pointer;">
              <s:text name="projectEdit.button.save" />
          </a> <a class="uiButton" style="cursor: pointer;" onclick="back();"> <s:text name="projectEdit.button.back" />
          </a>
          </span>
        </div>
      </form>
