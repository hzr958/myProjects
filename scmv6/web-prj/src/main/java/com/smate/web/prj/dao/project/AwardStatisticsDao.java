...
public class AwardStatisticsDao extends SnsHibernateDao<AwardStatistics, Long> {
   ...
   * 
   * @param psnId 赞的人的id
   * @param actionKey 被赞东西的主键
   * @param actionType 被赞东西的类型
   * @return
   */
  public AwardStatistics getAwardStatistics(Long psnId, Long actionKey, Integer actionType) {
    String hql =
        "from AwardStatistics t where t.psnId =:psnId and t.actionKey =:actionKey and t.actionType =:actionType";
    List<AwardStatistics> list = super.createQuery(hql).setParameter("psnId", psnId)
