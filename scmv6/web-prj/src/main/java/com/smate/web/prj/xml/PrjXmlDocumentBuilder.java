 ...
 */
public class PrjXmlDocumentBuilder {
  ...
  public static void mergeWhenImport(PrjXmlDocument newImportDoc, String oldSavedXml) throws Exception {
    if (XmlUtil.isEmpty(oldSavedXml)) {
      return;
    }
    PrjXmlDocument oldSavedDoc = new PrjXmlDocument(oldSavedXml);
    // 拷贝全文节点
    Node node = oldSavedDoc.getNode(PrjXmlConstants.PRJ_FULLTEXT_XPATH);
    if (node != null) {
      newImportDoc.copyPrjElement((Element) node);
    }
    // 拷贝附件节点
    Element newAttachs = newImportDoc.createElement(PrjXmlConstants.PRJ_ATTACHMENTS_XPATH);
    List attachs = oldSavedDoc.getNodes(PrjXmlConstants.PRJ_ATTACHMENTS_ATTACHMENT_XPATH);
    ...
    // 删除导入的项目参与者节点
    newImportDoc.removeNode(PrjXmlConstants.PRJ_MEMBERS_XPATH);
    // 拷贝项目参与者节点
    Element ele = (Element) oldSavedDoc.getNode(PrjXmlConstants.PRJ_MEMBERS_XPATH);
    Element newEle = (Element) newImportDoc.getNode(PrjXmlConstants.PRJ_MEMBERS_XPATH);
    if (ele != null && newEle == null) {
      newEle = newImportDoc.createElement(PrjXmlConstants.PRJ_MEMBERS_XPATH);
    ...
    // 保留旧的项目的公开权限信息
    String authority = oldSavedDoc.getXmlNodeAttribute(PrjXmlConstants.PRJ_META_XPATH, "authority");
    if (StringUtils.isBlank(authority) || !NumberUtils.isCreatable(authority)) {
      authority = PsnCnfConst.ALLOWS.toString();
    }
    newImportDoc.setXmlNodeAttribute(PrjXmlConstants.PRJ_META_XPATH, "authority", authority);
  }
   ...
   */
  @SuppressWarnings("unchecked")
  public static PrjXmlDocument build(Map data) throws Exception {
   ...
   * @throws Exception Exception
   */
  public static PrjXmlDocument createForR(Long currentUserId, String currentUserName, Long currentInsId,
    ...
    Element elem = null;
    // 默认作者.
    elem = xmlDoc.createElement(PrjXmlConstants.PRJ_MEMBERS_MEMBER_XPATH);
    ...
    elem = node != null ? (Element) node : xmlDoc.createElement(PrjXmlConstants.PRJ_META_XPATH);
   ...
   * @throws Exception Exception
   */
  public static PrjXmlDocument createForRO(long currentUserId, Long currentInsId, String formTmpl,
