...
public final class SystemUtils {
  ...
  public static void main(String[] args) {
    String s = "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:49.0) Gecko/20100101 Firefox/49.0";
    int indexOf = s.indexOf("Firefox");
  ...
  public static String getRequestBrowserInfo(HttpServletRequest request) {
    ...
    if (header.indexOf("MSIE") >= 0) {
      browserVersion = getStrBySplit(header, " ", "MSIE");
    } else if (header.indexOf("Firefox") >= 0) {
      browserVersion = getStrBySplit(header, " ", "Firefox");
    } else if (header.indexOf("Chrome") >= 0) {
      browserVersion = getStrBySplit(header, " ", "Chrome");
    } else if (header.indexOf("like Gecko") >= 0) {
      browserVersion = "MSIE 11";
    } else if (header.indexOf("Safari") >= 0) {
      browserVersion = getStrBySplit(header, " ", "Safari");
    } else if (header.indexOf("Camino") >= 0) {
      browserVersion = getStrBySplit(header, " ", "Camino");
    } else if (header.indexOf("Konqueror") >= 0) {
      browserVersion = getStrBySplit(header, " ", "Konqueror");
    }
    return browserVersion;
  }

  public static String getStrBySplit(String str, String splitStr, String matches) {
    if (str == null || str == "") {
      return "";
    } else {
      String[] split = str.split(splitStr);
      for (int i = 0; i < split.length; i++) {
        if (split[i].indexOf(matches) >= 0) {
          if ("MSIE".equals(matches)) {
