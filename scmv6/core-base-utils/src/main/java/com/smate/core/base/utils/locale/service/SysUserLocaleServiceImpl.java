...
public class SysUserLocaleServiceImpl implements SysUserLocaleService {
  ...
  public void saveOrUpdateSysUserLocale(Long psnId, String locale) throws Exception {
    try {
      SysUserLocale sysUserLocale = sysUserLocaleDao.get(psnId);
      if (sysUserLocale == null) {
        sysUserLocale = new SysUserLocale(psnId, locale);
      } else {
        sysUserLocale.setLocale(locale);
      }
      sysUserLocaleDao.save(sysUserLocale);
    } catch (Exception e) {
      logger.error("查询用户本地化时出错", e);
      throw new Exception(e);
    }
