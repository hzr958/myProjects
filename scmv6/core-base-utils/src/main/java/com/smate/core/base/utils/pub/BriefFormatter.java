package com.smate.core.base.utils.pub;
...
public class BriefFormatter {
   ...
   * 英文日期格式.
   */
  public static final String ENG_DATE_FORMATS = "yyyy-MM-dd";
  ...
  /*
   * 注意注意注意：以下所构造的字段，不要随意改动名字，不要随意改动名字，不要随意改动名字 实在要改，请在下面备注，问题单号，以及原因，改动哪里
   */
  // 书籍
  public static final String BOOKBRIEF_PATTERN =
      "<out>${publisher}</out><page>, ${total_pages}</page><date>, ${publish_date}</date><out>, ${language}</out><out></out>";
  // 书籍章节
  public static final String BOOKCHPATERBRIEF_PATTERN =
      "<out>${name}</out><out>, ${editors}</out><out>, ${publisher}</out><page>, ${page_number,page_number}</page><out>, ${country_name}</out><date>, ${publish_date}</date><out></out>";
  // 奖励
  public static final String AWARDBRIEF_PATTERN =
      "<out>${issuing_authority}</out><out>, ${category}</out><out>, ${grade}</out><date>, ${award_date}</date><out></out>";
  // 会议论文
  public static final String CONFPAPERBRIEF_PATTERN =
      "<out>${conf_name}</out><date_interval>, ${start_date,end_date}</date_interval><page>, ${page_number,page_number}</page><out>,${city}</out><date>, ${publish_date}</date><out>, ${confType}</out><out></out>";
  // 期刊论文
  public static final String JOURNALBRIEF_PATTERN =
      "<out>${name}</out><vol_issue>, ${volume_no,issue}</vol_issue><page>, ${page_number,page_number}</page><date>, ${publish_date}</date><out></out>";
  // 其他
  public static final String OTHERBRIEF_PATTERN = "<out>, ${country_name}</out><out>, ${publish_date}</out><out></out>";
  // 专利
  public static final String PATENTBRIEF_PATTERN =
      "<date>${start_date}</date><out>, ${country_name}</out><out>, ${issuing_authority}</out><out>, ${application_no}</out><out></out>";
  // 学位论文
  public static final String THEISBRIEF_PATTERN =
      "<out>${department}</out><out>, ${degree}</out><out>, ${issuing_authority}</out><out>, ${country_name}</out><date>, ${publish_date}</date><out></out>";
  // 标准
  public static final String STANDARDBRIEF_PATTERN = "<out>${standard_no}</out>";
  // 软件著作权
  public static final String SOFTWARECOPYRIGHTBRIEF_PATTERN = "<out>${register_no}</out>";
  ...
  public String formatPage(String totalPage) {
    ...
    if (XmlUtil.isEmpty(totalPage)) {
      ...
      return String.format("p.%s", totalPage);
    }
  }
   ...
   * 
   * @param startPage
   * @param endPage 总页数：xxx p 第x页：p x 第x-y页 pp x-y
   * @return String
   */
