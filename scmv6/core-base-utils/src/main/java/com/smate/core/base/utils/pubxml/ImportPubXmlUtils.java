...
public class ImportPubXmlUtils {
   ...
   * @return
   */
  public static List<String> parsePubKeywords2(String keywords) {
    String[] kws = null;
    if (StringUtils.isBlank(keywords)) {
  ...
  public static List<String> parsePubAuthorNames(String authorNames) {
    ...
    String htmlauthorNames = HtmlUtils.htmlUnescape(authorNames);
    kws = htmlauthorNames.split("[;；]");
    if (!htmlauthorNames.contains(";") && !htmlauthorNames.contains("；")) {
      kws = htmlauthorNames.split("[,，]");
    }
    List<String> list = new ArrayList<String>();
    for (String kw : kws) {
   ...
   * @return
   */
  public static List<Long> parsePubIds(String pubIds) {
    if (StringUtils.isBlank(pubIds)) {
      return null;
    }
    String[] ids = pubIds.split("[,，;；/\\\\:：\"。]");
    List<Long> list = new ArrayList<>();
    for (String pubId : ids) {
      if (StringUtils.isNumeric(pubId)) {
