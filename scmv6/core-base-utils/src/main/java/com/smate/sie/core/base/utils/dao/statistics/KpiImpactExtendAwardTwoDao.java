 ...
 */
@Repository
public class KpiImpactExtendAwardTwoDao extends SieHibernateDao<KpiImpactExtendAwardTwo, Long> {

  public void delTwoMonBeforeData(Long insId, String time) {
    String hql = "delete from KpiImpactExtendAwardTwo t where t.insId=? and to_number(t.time)<=to_number(?)";
  ...
  public Long getCountByDateTime(Long insId, String time) {
    String hql = "select count(t.id) from KpiImpactExtendAwardTwo t where t.insId=:insId and t.time = :time";
