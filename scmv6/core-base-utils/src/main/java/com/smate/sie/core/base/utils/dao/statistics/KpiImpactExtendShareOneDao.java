 ...
 */
@Repository
public class KpiImpactExtendShareOneDao extends SieHibernateDao<KpiImpactExtendShareOne, Long> {


  public void delTwoMonBeforeData(Long insId, Date beginTime) {
    String hql = "delete from KpiImpactExtendShareOne t where t.insId=:insId and t.stDate<:beginTime";
  ...
  public Long getCountByDateTime(Long insId, Date startDate, Date endDate) {
    String hql =
        "select count(t.id) from KpiImpactExtendShareOne t where t.insId=:insId and t.stDate >= :startDate and t.stDate < :endDate";
