    ...
    this.each(function(index) {
      ...
      var result = {
        ...
        "put" : function(val, text) {// 添加word
          ...
          var width = label.width() > defaults["show_max"]
              ? defaults["show_max"]
              : (($.browser.msie || (/Trident\/7\./).test(navigator.userAgent)) ? label.width() + 2 : label.width());
          label.width(width);
        ...
        "putAll" : function(words) {// 添加多个word
          var _result = this;
          $.each(words, function() {
            if (defaults["isEncoder"]) {
              if (typeof this.des3Val == "undefined") {
                _result.put(this.val, this.text);
              } else {
                _result.put(this.des3Val, this.text);
              }
            } else {
              _result.put(this.val, this.text);
            }
        ...
        "_input_width" : function() {
          ...
          /*
           * var maxchar = 50;//最多50字母，34中文 var iCount = text.length; if (iCount <= maxchar)
           * {//自动适应中英文长度 _input.attr("maxlength", maxchar); } else { _input.attr("maxlength",
           * (maxchar - (iCount - maxchar))); }
           */
          _input.css({
            "width" : newWidth + "px"
          });
      ...
      // 设置是否要触发ajax全局事件
      _input.attr("global", defaults["global"]);
      if (defaults["watermark_flag"] == true) {
        if (result.inputCount() < defaults["words_max"]) {
          _this.find(".auto_word_watermark").text(defaults["watermark_tip"]);
          _this.find(".auto_word_watermark").show();
      ...
      _input.keyup(function(event) {
        ...
        if (text == '' || text.length == 0||event.keyCode==32) {//输入空格不触发
          return;
        }
        if (text.length > 0) {
          ...
          function(event) {// 失去焦点
            ...
            if (!$.isFunction(defaults["blur"]) && defaults["blur"]) {
              blurKeyMark = setTimeout(function() {
                ...
                var isOut = true;
                /*
                 * $("#en_auto_word_outer_div_1").each(function(){ if ($(this).is(":visible"))
                 * {//关联内容显示，不自动完成单词 isOut = false; return false; } });
                 */
                ...
                if (isOut && text.length > 0) {// 关联内容隐藏，自动完成单词
                  ...
                  if (defaults["email"] && !re.test(text)) {
                    ...
                    // 点击输入自动下拉框提示的关键词不做处理
                    if (typeof $.autoword.config == "undefined"
                        || ($.autoword.config != "undefined" && !$.autoword.config.mouseDownOnSelect)) {
      ...
      if (defaults["select"] != null && $.isFunction(defaults["select"])) {// 自动完成下拉框
        ...
        select["extend"](function(data) {
          if (defaults["enter"]) {
            // 如果需要验证输入内容的正确性，则不清除上一输入内容(兼容选择好友和匹配关键词业务需求的功能实现)_MJG_2013-03-04_SCM-1932).
            // if(!$.isFunction(defaults["blur"]) && defaults["blur"])
            /*
             * var _validate_url=defaults["validateUrl"]; if(_validate_url!=null &&
             * typeof(_validate_url)!="undefined"){
             * 
             * }else{
             *  }
             */
      ...
      if (!$.isFunction(defaults["delClick"]) && defaults["delClick"]) {
        _this.find(".auto_word_div_del").live("click", function(e) {// 鼠标事件，删除word
          $(this).parent().remove();
          if (typeof $.autoword.friendId_clear != "undefined") {
            $.autoword.friendId_clear();
          }
          defaults["isblur"] = false;
