...
var ScmMaint =
{
		"searchSomeOneBind":function(){//检索人员标签bind事件.
			...
			$("#home_search_items").attr("style","display:none");
			...
			var title = $.trim($("#search_some_one").attr("title"));
			if(userInfo == "" || userInfo == title){
				return;
			}
			var a=$.trim($("#search_some_one").val());
			$("#search_some_one").val(a);
		...
		"logoutSys":function(){//退出登录.
			MsgBoxUtil.delCookie("msgBoxIsClose");
			Cookie.delCookie("CasAuth");
			Cookie.delCookie("pwd");
			document.location.href = domainOauth+"/oauth/logout?sys=SIE&sysDomain="+window.document.location.host;
//			document.location.href=domCas+"logout?locale="+locale+"&url="+logoutindex;
		...
		"changeLanguage":function(language){//已登录系统的语言切换响应事件(直接跳转到“个人中心”页面并同时切换语言).
			document.location.href=ctxpath+"/main?locale="+language;
		},
	...
	"onlineService":function(){ //在线QQ服务
		
		$("#onlineservice").find("iframe").first().document.getElementById("launchBtn").click(); 
	...
	"initMainLabelStatus":function(){
		...
		//绑定在线客服点击事件.
//		var onlinehelp=$(".Online-Service");
//		onlinehelp.bind("click",function(){
//			window.open(online_help_url);
//		});
