...
//自动匹配人员列表
function init_auhorname(){
	$("#authorName,#psnName").autocomplete(ctxpath+"/ac/rolPsnUnitIns.action", {
		width: 220,
		...
		scroll: false,
		cacheLength: 0,
		extraParams: {'deptId':function(){
			$("#psnId").val("");			
			var deptId = $("#queryUnitId").attr('unitId');
			if(deptId == "0"){//rol-2747 选择other部门后，控制人员只取other下的
				return "-1";
			}
			return deptId;
		}},
		formatItem: function(data, i, n, value, term) {
			if(data.unitName != ""){
				return data.psnName + "("+data.unitName+")";
			}
			return data.psnName;
		},
		parse: function(data){// 转化数据
			...
			for(var i=0;i<rows.length;i++){
				var item = rows[i];
				parsed.push({data:item,value:item.cpPsnId.toString(),result:item.psnName});
			}
			return parsed;
		}
	}).result(function(event, data, formatted) {// 返回result
		$("#psnId").val(data.cpPsnId);
	});
}
...
checkUserLogin=function(){
	...
	$.ajax({
  	  	async: false,
  	  	url : ctxpath+"/archiveFiles/ajaxCheckUserLogin",
  	  	type : 'post',
  	  	dataType:'json',
  	  	success : function(data) {
  	  	  	if(data.result == "fail" ){
  	  	  		flag = false;
  	  			jConfirm(project_view.noLogin, project_view.prompt,function(result){
					if(result)
