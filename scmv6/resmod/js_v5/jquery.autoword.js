...
//           var clickflag = 0;
            ...
            var result = {
                ...
                "put": function(val, text){//添加word
                    ...
                    var width = label.width()>defaults["show_max"]?defaults["show_max"]:($.browser.msie?label.width()+2:label.width());
                    label.width(width);
...
//					   _input.val(data["name"]);
             ...
             if (!$.isFunction(defaults["delClick"]) && defaults["delClick"]) {
	            _this.find(".auto_word_div_del").live("click", function(e){//鼠标事件，删除word
            	 ...
            	 _this.find(".auto_word_div_del").live("click", function(e){//鼠标事件，删除word
 	                $(this).parent().remove();
 	                result._input_show();//显示输入框
 	                defaults["delClick"]($(this).parent().attr("val"),$(this).parent().find("label").text());
 	                return false;//防止事件冒泡
 	            });
             }
    ...
    var _create_word_input = function(){
    		  return '<div class="auto_word_input_div" style=""><input name="input" type="text" class="auto_word_input" size="1" maxlength="40" /></div><label class="auto_word_watermark" />';
    };
