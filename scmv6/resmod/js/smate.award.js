...
var smate = smate ? smate : {};
 smate.award = smate.award ? smate.award
		: {
...
smate.award.awardLockFlag = "";
// 初始化赞操作
smate.award.iniAward = function(resType) {
	var jsonParam = "{\"resType\":" + resType + ",\"resDetails\":"
			+ smate.award.collectResData(resType) + "}";
	var resDataJsonObj = JSON.parse(smate.award.collectResData(resType))[0];
	var resNode;
	...
	$.ajax( {
		type : "post",
		url : snsctx + "/dynamic/ajaxIniAward",
		data : {
...
smate.award.collectResData = function(resType) {
	var json = [];
 ...
 * 赞人员列表.
 */
smate.award.showAwardDetail = function(obj) {
	var awardId = $(obj).attr("awardId");
	if (awardId == 0) {
		$.smate.scmtips.show("warn", smate.award.noAward);
	} else {
		$("#hidden-awardLink").attr(
				"alt",
				snsctx + "/dynamic/ajaxShowAwardDetails?awardId=" + awardId
						+ "&TB_iframe=true&height=384&width=672");
		$("#hidden-awardLink").attr("title", smate.award.personalList);
 ...
 * 赞.
 */
smate.award.ajaxAward = function(resType, resNode, resId) {
	if (smate.award.awardLockFlag == ""
		|| smate.award.awardLockFlag != (resType + "_" + resNode + "_" + resId)){
		smate.award.awardLockFlag = resType + "_" + resNode + "_" + resId;
		var jsonParam = "";
		if ($("#hidden_groupIdABC").length == 0) {
		...
		$.ajax( {
			...
			success : function(data) {
				if (data.result == "success") {
					...
					smate.award.awardLockFlag = "";
					jConfirm(smate.award.loginCfmTip, smate.award.reminder, function(result) {
						if (result)
							location.href = snsctx + "?service="
 ...
 * 取消赞.
 */
smate.award.ajaxCancelAward = function(resType, resNode, resId) {
	if (smate.award.awardLockFlag == ""
		|| smate.award.awardLockFlag != (resType + "_" + resNode + "_" + resId)){
		smate.award.awardLockFlag = resType + "_" + resNode + "_" + resId;
