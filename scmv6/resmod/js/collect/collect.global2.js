...
var db_code_to_search_id={'WanFangYX':-1,'ROLDB':0,'Scopus':1,'ScienceDirect':2,'ISIWebOfScience':4,'IEEEXplore':135171,'ChinaJournal':16,'CqVip':32,'WanFang':64,'CNIPR':1024,'ITF':512,'Nsfc':4096,'CnkiFund':8192,'EI':16384,'SCIE':32768,'SSCI':65536,'ISTP':131072,"PubMed":135168,"Baidu":135169,"GoogleScholar":128,"Cnkipat":135170};
var search_id_to_db_code={'-1':'WanFangYX',0:'ROLDB',1:'Scopus',2:'ScienceDirect',4:'ISIWebOfScience',135171:'IEEEXplore',16:'ChinaJournal',32:'CqVip',64:'WanFang',1024:'CNIPR',512:'ITF',4096:'Nsfc',8192:'CnkiFund',16384:'EI',131072:'ISTP',65536:'SSCI',32768:'SCIE',135168:'PubMed',135169:"Baidu",128:"GoogleScholar",135170:"Cnkipat"};
var default_iframe_hegiht={"WanFangYX":1200,"Scopus":2400,"ChinaJournal":1600,'CnkiFund':1000,"EI":2700,"SCI":2300,"SCIE":2300,"SSCI":2200,"ISTP":2900,'ISIWebOfScience':2250,'CNIPR':3085,'ITF':1300,'ScienceDirect':2200,'IEEEXplore':2200,'CqVip':1000,'Nsfc':2000,'WanFang':1500};
var default_batch_page_count={'SCI':20,'SCIE':20,'SSCI':20,'ISTP':20,'EI':25,'Scopus':20,'WanFang':10,'ChinaJournal':20,'CNIPR':10};
...
var allowDbcode = ['SCI','SCIE','SSCI','ISTP'];
...
var logCriteriaStr=""; // 检索条件Criteria（可以把它看做一个类）中的参数值拼接成的字符串
...
var isIEBrowser = true; // 是否是IE浏览器
var importPubLimitMaxsum = 20; // 限制的最多一次导入的成果数
...
function checkLastest(oldVersion,newVersion){
   try{
       ...
       if(client.browser.name == "IE" || client.browser.name == "Chrome" || client.browser.name == "Trident" ){
...
// 页面点击检索按钮, 进行检索
function searchData(){
  var timeoutReqUrl = "/pub/ajaxtimeout";
  if(isProjectCollect){
    timeoutReqUrl = "/prjweb/ajaxtimeout";
  }
    BaseUtils.checkCurrentSysTimeout(timeoutReqUrl, function(){
        // 先确定检索的文献库类型
        ...
        // 删除上次查询操作时, 检索结果页面左边栏的文献库列表, 原先用的是hiddenDBs方法
        removeDBUI();
        // 在全局变量中记录一个type值,后面方法可以调用
        var type = getSearchType();
        globalType=type;
        // 分IE浏览器和chrome内核浏览器中检索
        if(isIEBrowser){
            searchData_ie(type);
        }else{
            searchData_chrome(type);
        }
    }, 0);
}
...
function getSearch_DB_Code(){
    ...
    if (multDB){
        $(".selected-author_single.selected-author_confirm").each(function(){
...
function ajaxInitPsnAliasJsonFRDB(type,tmpOrgName,callback,db_code){
   ...
   psnName =$.trim(CtoH($("#lname").val()))+" "+$.trim(replaceStr(CtoH($("#fname").val()),"-"," "));    
    data={'dbCode':allowDbcode.join(','),'name':psnName};
    $.ajax({
        url:'/psnweb/psnalias/initjson',
        type:'post',
        async:false,
        data:data,
        dataType:'json',
            ...
            if(json.result=='success'){
                for(var i = 0 ;i<allowDbcode.length;i++){
                    if(json[allowDbcode[i]]!=undefined||json[allowDbcode[i]]!=""){
                        ...
                        if(allowDbcode[i] == "SCI" || allowDbcode[i] == "SCIE"){
                          SCIPsnAliasJsonFRDB=currentData;
                        }
                        if(allowDbcode[i] == "SSCI"){
                          SSCIPsnAliasJsonFRDB=currentData;
                        }
                        if(allowDbcode[i] == "ISTP"){
                          ISTPPsnAliasJsonFRDB=currentData;
                        }
                    }
                }
                isFirstToSearch++;
...
function removeDBUI(){
    ...
    $("#db_list_ul").find("div.search_result_db_item").each(function(){ 
        $(this).remove(); 
    });
}
...
function validSearchInput(type){
    ...
    if (multDB && search_DB_Code.length==0){
      scmpublictoast(referencesearch_msg_needselectdatabase, 1000);
        return false;
    }   
    var pattern=/^(([1-9][0-9][0-9][0-9])|([1-9][0-9][0-9][0-9]-[1-9][0-9][0-9][0-9]))$/;
    if(publicationArticleType=='1'){
        // 将全角转成半角, 发表年份不为空的话校验年份格式
        var pub_year=CtoH($("#publicyear").val());
        if ( pub_year!="" && !pattern.test(pub_year)){
            smate.showTips._showNewTips(referencesearch_label_pubyeardesc,referencesearch_msg_alert,undefined,undefined,referencesearch_btn_sure,referencesearch_btn_cancel);
            return false;           
        }
    }
    // 获取人员姓名
    ...
    for(var j=0;j<search_DB_Code.length;j++){
        if(search_DB_Code[j]=="SCI" || search_DB_Code[j]=="SCIE" || search_DB_Code[j]=="SSCI" || search_DB_Code[j]=="ISTP" ||
    ...
    // 如果选择了中文库
    if(isSelectedChineseDB){
        if($.trim(txtFname) != "" && $.trim(txtTitle)=="" && $.trim(txtDoi)=="" && $.trim(txtInsName)=="" && $.trim(txtLname)==""){
            smate.showTips._showNewTips(globalTips.needMoreCondition,referencesearch_msg_alert,undefined,undefined,referencesearch_btn_sure,referencesearch_btn_cancel);  
            return false;
        }
    } 
    // 如果选择了英文库
    if(isSelectedEnglishDB){
    ...
    if (txtTitle=="" && txtDoi=="" && txtInsName=="" && txtCname=="" && txtFname=="" && txtLname=="" && name_else==""){
        smate.showTips._showNewTips(referencesearch_msg_needenteritem2,referencesearch_msg_alert,undefined,undefined,referencesearch_btn_sure,referencesearch_btn_cancel);
        return false;
    }
    // 有些库不支持doi检索, 如果检索条件中doi不为空, 则校验是否有选中不支持doi检索的文献库, 有的话要提示
    if($.trim(txtDoi) != "" && search_DB_Code.length>0){
        var notDoiSearchDB = "";
        var splitCode = ", ";
        for(var k=0;k<search_DB_Code.length;k++){
            if(search_DB_Code[k]!="SCI" && search_DB_Code[k]!="SCIE" && search_DB_Code[k]!="Baidu"&&search_DB_Code[k]!="SSCI" &&
        ...
        if($.trim(notDoiSearchDB)!=""){
            smate.showTips._showNewTips(notDoiSearchDB + referenceSearch_msg_notdoisearch,referencesearch_msg_alert,undefined,undefined,referencesearch_btn_sure,referencesearch_btn_cancel);
            return false;
        }
    }
...
function isSelectedEnglishDatabase(){
    var chineseDatabase = new Array("SCI","SCIE","SSCI","ISTP","EI","Scopus","ScienceDirect","IEEEXplore","ISIWebOfScience","ITF","PubMed");
    for(var i=0;i<search_DB_Code.length;i++){
        for(var j=0;j<chineseDatabase.length;j++){
            if(search_DB_Code[i].toUpperCase()==chineseDatabase[j].toUpperCase()){
                return true;
...
// 根据db_code获取文献库名称, paramName是因为成果和项目在json中的名称的key不一样
function getShowDBname(db_code, paramName){
    if(paramName == null || paramName == "" || typeof(paramName) == "undefined"){
        paramName = "dbName";
    }
    var db_name='';
    ...
    for(var i=0;i<dbLen;i++){
        ...
        if(db_code==code){
            db_name = ins_url[search_DB_Code[i]][paramName];
        }   
    }
    return db_name;
}
...
function getDBName(db_code){
    ...
    return getShowDBname(db_code, param);
}
...
function checkLanguage_DB(lang,db_name,criteria_len){
    ...
    if (lang =="en_US" || lang =="en"){
        if (criteria_len==0){
            // 提示需要其它英文条件以进行检索
            smate.showTips._showNewTips(replaceStr(referencesearch_msg_term_onlysupport,'@database@',db_name),referencesearch_msg_alert,undefined,undefined,referencesearch_btn_sure,referencesearch_btn_cancel);
            return false;
        }else if ((isChineseStr(oname) || isChineseStr(fname) || isChineseStr(lname)) ||(cname!='' && isChineseStr(cname))){
            // 提示文献数据库只支持英文人名
            smate.showTips._showNewTips(replaceStr(referencesearch_msg_name_onlysupport,'@database@',db_name),referencesearch_msg_alert,undefined,undefined,referencesearch_btn_sure,referencesearch_btn_cancel);
            return false;   
        }else if ( title !="" && isChineseStr(title) ){
            // 提示文献数据库只支持英文标题
            smate.showTips._showNewTips(replaceStr(referencesearch_msg_topic_onlysupport,'@database@',db_name),referencesearch_msg_alert,undefined,undefined,referencesearch_btn_sure,referencesearch_btn_cancel);
            return false;   
        }
        // 提示文献数据库只支持英文条件
        if (criteria.field_count>0){
            var flag = true;
            // 原先的逻辑不适用现在的页面, TODO 待确认是否保留该逻辑
            return flag;
        }
    }
    return true;
...
// console.log(taskInfo);
    if ($("#divMessage").length>0){
        ...
        if (taskInfo["getxml"]["db_id"] !=""){
            ...
            var tmpProgInfo=referencesearch_msg_collecting;// "正在从@DB_NAME@获取数据...";
            var tmpDBName="";
            // 检索结果页面点击导入时出正在加载弹框
            var currentSelectedDBName = new Array();
            for(var i=0; i<search_DB_Code.length; i++){
                ...
                if(cbxs.size() > 0){
                    for( var j=0; j<cbxs.length; j++ ){
                        if( cbxs[j].checked ){
                            currentSelectedDBName.push(getDBName(search_DB_Code[i]));
                            break;
                        }
                    }
                }
            }
            showProgress("imp_res","<span>" + tmpProgInfo.replace("@DB_NAME@",currentSelectedDBName.join(", ")) + "</span>");
// if(multDB){
...
// }
            ...
            // 隐藏加载提示框
            $("#getDataFromTips").hide();
        }
        if (taskInfo["search"]["db_id"]!=""){
            var searchingDBName=[];
            var job_db_id=taskInfo["search"]["db_id"].split(",");
            for (var i=1;i<=4 && i<job_db_id.length;i++){               
                searchingDBName.push(getDBName(getDbCodeByBitCode(job_db_id[i])));          
            }
            searchingInfo = searchingDBName.join(", ");
            var len = job_db_id.length - 1;
            if(len > 4){
                searchingInfo += "等" + len + "个库";
            }
        }
        // 隐藏或显示进度条
        if (taskInfo["count"]=="0" || $('.loadding_div_box').is(':visible') || $('#getDataFromTips').is(':visible')){
            $("#divMessage").hide();
            $("#search_ref_db_enum").html("");
        }else{
            $("#search_ref_db_enum").html(searchingInfo);
            $("#divMessage").show();
        }
        // 定位到进度条
        scrollShow($("#divMessage"),$(".header__main"),10);
        // document.getElementById("divMessage").scrollIntoView();
    }
}

/**
 * 定位到某一个元素，当指定元素被遮盖时，自动调节滚动栏，显示该元素
 * 
 * @param element
 *          指定的要定位的元素对象
 * @param fixedElement
 *          页面上某些布局为fixed样式的元素，固定元素
 * @param customAddHeight
 *          自己可以进行调节的高度。进行微调
 * @returns
 */
function scrollShow(element,fixedElement,customAddHeight){
  var fixedHeight=$(fixedElement).outerHeight()||0;
  if(($(window).scrollTop()>($(element).offset().top+$(element).outerHeight()-fixedHeight))){// 元素在上方被遮挡
    var h1=$(element).offset().top-fixedHeight-customAddHeight;
    $("html,body").animate({scrollTop:h1},100);
  }else if((($(window).scrollTop()+$(window).height())<$(element).offset().top)){// 元素在下方被遮挡
    var h2=$(element).offset().top-$(window).height()+$(element).outerHeight()+$(window).scrollTop()+customAddHeight;
    $("html,body").animate({scrollTop:h2},100);
  }
}


// 显示检索的进度条 TODO 重新调整进度条的样式
function showProgress(name,content){
    $("#get_data_from_content").html(content);
    $("#getDataFromTips").show();
    if(document.getElementsByClassName("new-getdata_tip-container").length>0){
...
function getTaskInfo(){
    // 组件会返回如“JobCount@SiteID;CmdID@SiteID;CmdID...."的信息 ,比如用户勾选了cnki检索库和万方检索库：则可能返回 16;1@64;1
    tmp_JobInfo = getJobInfo();
// console.log(tmp_JobInfo);
...
function checkOrgNames(orgNames){
    ...
    if (errorDb_Name.length>0){
        smate.showTips._showNewTips(replaceStr(referencesearch_msg_aff_onlysupport_me,'@database@',errorDb_Name),referencesearch_msg_alert,undefined,undefined,referencesearch_btn_sure,referencesearch_btn_cancel);
        return false;
    }
    return true;
}
...
function showSearchCriteria(type){
    ...
    var tmpStr_RT=contextLang =="en"?"<p style = 'color:#999; display: inline;'>)</p>":"<p style = 'color:#999;display: inline;'>)</p>";
    // 根据语言选择分隔符,并拼接好当前查询的文献库名称字符串
    var splitCode= ", ";
    for(var i=0;i<search_DB_Code.length;i++){
        tmpDBName=tmpDBName + splitCode + getDBName(search_DB_Code[i]);
    }
    var newTmpDBName = tmpDBName.substring(1);
    tmpStr=tmpStr + newTmpDBName.substring(1,newTmpDBName.length);
    /*
     * if (contextLang =="en") { tmpStr=tmpStr + newTmpDBName.substring(1,newTmpDBName.length); }
     * else { tmpStr=tmpStr + tmpDBName.substring(1); }
     */
    // 拼接各个查询条件
...
function bulidCurrentRetrieval (db_code){
    ...
    if("me"==globalType && ("SCI"==db_code || "SCIE"==db_code || "SSCI"==db_code || "ISTP"==db_code) && flag){
...
function onUpdateLog(db_code,varCmdId,varRetInt,varRetStr){
    ...
    if ( varCmdId!=14){
        if(varCmdId==5){    
            ...
            // scm-6608
            // varRetStr=replaceStr('<br/>'+referencesearch_msg_search_error,'@database@',getDBName(db_code))
            // + varRetStr;
            var logContent=logInsSearch+"\ndbCode=" + db_code + ";cmdID=" + varCmdId + ";\nvarRetStr="+ varRetStr;
            writeLog(4,varRetInt,logContent);
...
// }
    resetSelectedPubSum();
    var selectedNum = parseInt($("#selected_to_import_num").text());
    if(selectedNum > 20){
      scmpublictoast(globalTips.select_import_maxnum_tips, 2000);
      return false;
    }
    getDataXML_All_chrome(impType);
}
...
// 限制一次性导入的成果数
function limitImportMaxnum(obj, selected){
    var selectedNum = parseInt($("#selected_to_import_num").text());
    // 全选时，如果选中全选
    if(selected || selected == "checked"){
        selectedNum++;
        if(selectedNum > importPubLimitMaxsum){
            scmpublictoast(globalTips.select_import_maxnum_tips, 2000);
    ...
    $("#selected_to_import_num").html(selectedNum);
    if(selectedNum >= importPubLimitMaxsum){
        $("#select_maxnum_limit_span").attr("style", "padding-right:10px;color: red !important;");
    }else{
        $("#select_maxnum_limit_span").attr("style", "padding-right:10px;");
    }
    return true;
}
...
function resetSelectedPubSum(){
    ...
    $("#selected_to_import_num").html(selectedSum);
    if(selectedSum >= importPubLimitMaxsum){
        $("#select_maxnum_limit_span").attr("style", "padding-right:10px;color: red !important;");
    }else{
        $("#select_maxnum_limit_span").attr("style", "padding-right:10px;");
    }
...
function selectAll(objdoc,selected){
    try{
        ...
        var optDoc = $(objdoc);
        var optSelectAllCheckbox = optDoc.find("#cbxSelectAll");
        var chkItems = optDoc.find(":checkbox[name='iris_id']");
        // IE的双击checkbox和其他浏览器处理方式不一样
        optSelectAllCheckbox.attr("ondblclick", "this.click()");
        // 全选时选中的成果数不能超过限制的20个
        $(chkItems).each(function(){
            var itemCheckStatus = $(this).attr("checked");
            var noNeedDeal = (selected || selected == "checked") && (itemCheckStatus || itemCheckStatus == "checked");
            // 如果是选中全选操作，已选中的就不用处理，跳过
            if(!noNeedDeal){
                if(!limitImportMaxnum($(this), selected)){
                  optSelectAllCheckbox.attr("checked",false);
                  return false;
                }
            }
        });
...
// 重新设置检索结果列表中的每条成果前的复选框点击事件
function resetCheckBoxClickEvent(db_code){
    var objdoc=getFrameDoc("if_" + db_code);
    if(db_code == "CNIPR"){// 中国知识产权网的是单选框
        chkItems = $(objdoc).find(":radio[name='iris_id']");
    }else{
        chkItems = $(objdoc).find(":checkbox[name='iris_id']");
    }
    if(chkItems.size() > 0){
        var items = $(chkItems);
        // 重新注册事件时注意解绑点击事件，不然多次注册点击事件点击后会多次触发
        items.off("click");
        // 绑定click去处理选中数量的话，多次快速连续点击还是会有问题
        items.change(function(ev){
          searchResultSelect($(objdoc), $(this), ev);
        });
        // IE中双击checkbox和其他浏览器中处理逻辑不一样
        items.attr('ondblclick', 'this.click()');
    }
}

// 检索结果复选框点击事件
function searchResultSelect(docObj, obj, ev){
    var checked = $(obj).attr("checked");
    limitImportMaxnum($(obj), checked);
    if($(docObj).find("#cbxSelectAll").attr("checked")=="checked" && !checked){
        $(docObj).find("#cbxSelectAll").attr("checked",false);
    } else {
        var flag = true;
        chkItems = $(docObj).find(":checkbox[name='iris_id']");
        if(chkItems.size() == 0){
          chkItems = $(docObj).find(":radio[name='iris_id']");
        }
        for (var i = 0;i < chkItems.length;i++) {
            if (!$(chkItems[i]).attr("checked")) {
                flag = false;
                break;
            }
        }
        if (flag) {
            $(docObj).find("#cbxSelectAll").attr("checked",true);
        }
    }
    ev.stopPropagation();
}
...
// 校验浏览器是否是隐身模式
function checkBrowserIsHideStyle(){
  ...
  if (!fs) {
    ...
    fs(window.TEMPORARY,100,function(){
      ...
      smate.showTips._showNewTips("为正常使用功能，请关掉无痕浏览模式", referenceSearch_propmt, "detectChromePlugin('search');smate.showTips._closeTips(this);", "");
	...
	if(client.browser.name == "Firefox"){
		...
		try{
	    	...
	    	if(flag){
	    		...
	    		getIrisOctopus().SetDownLoadFileUrl(pageContext_request_serverName + "/resmod/js/search/searchplugin/iris_Octopus.html?a="+100*Math.random());
...
function splitNamePY(input) { 
	 var pattern = /[^aoeiuv]?h?[iuv]?(ai|ei|ao|ou|er|ang?|eng?|ong|a|o|e|i|u|ng|n)?/gi; 
	     ...
	     while((result = pattern.exec(input))!=null && result.index != pattern.lastIndex){ 
	         // alert("Match:" + result[0] + ", " + "Postion:" + result.index +", next start
            // postion:" + pattern.lastIndex);
...
function switchDIV(showResult){
	if (showResult && multDB){
		...
		$("#step_one_div").removeClass("Federal-retrieval_header-item_first");
		$("#step_one_div").addClass("Federal-retrieval_header-item_fifth");
		$("#step_two_div").addClass("Federal-retrieval_header-item_first");
		$("#step_two_div").removeClass("Federal-retrieval_header-item_second");
	}
	if(!showResult && multDB){
		stepSwitchShow(1);
		...
		$("#box_main_pub").show();
		$("#step_two_div").removeClass("Federal-retrieval_header-item_first");
        $("#step_two_div").addClass("Federal-retrieval_header-item_second");
        $("#step_one_div").addClass("Federal-retrieval_header-item_first");
        $("#step_one_div").removeClass("Federal-retrieval_header-item_fifth");
	}
	if (showResult && !multDB){
		stepSwitchShow(2);
...
function clearInput(){ 
	...
	if(type=="friend"){
		...
		$("#insId").val("");
		$("#friendPsnId").val("");
		$.autoword["div_plugin_shoose_friends"].clear();// 增加好友选择框条件清除 date:3-4 by:Lrl
...
function unCheckedAll(){
	...
	for(var i=0;i< db_code.length;i++)
	{
		try
		{
			...
			$(objdoc).find("#cbxSelectAll").attr("checked", false);
...
function resizeIframe(iframeID,varRetInt) {
	...
	try{
	    var iframeDOM = getFrameDoc(iframeID).getElementById(iframeID);
		nPageHeight=Math.max(iframeDOM.scrollHeight,iframeDOM.offsetHeight);
	...
	var nMaxSize = parseInt(varRetInt) / 10 * 1430 ;
...
function WriteText (doc,sText,id,type,db_code,varRetInt) {
	...
	if(db_code=="Scopus" ||db_code=="WanFang" || db_code=="SCI" || db_code=="SCIE" || db_code=="SSCI" || db_code=="ISTP" || db_code=="CNIPR")
	...
	frameHtml += "<body onload=\"parent.resize('" +id+ "','"+varRetInt+"','200');\">\n";
  	...
  	}else if(!multDB|| (client.browser.name != "IE" && client.browser.name != "Trident")){
  	    sText=sText.replace(/<myFlag>([\s\S]*)<\/myFlag>/,"");
  	}
  	// 如果是写入html, 则加全选框
  	if (type && type=="1" && db_code != "CNIPR")// 中国知识产权网的不要加全选功能
  	{
  		// scm-6689
  		if(db_code == "BaiTen"){
  			frameHtml +="<div style='padding-left:15px;padding-top:10px; margin-top:15px;margin-bottom: 10px;'>";
  		}else if(db_code == "PubMed"){
  			frameHtml +="<div style='padding-top:10px; margin-top:15px;margin-bottom: 10px;'>&nbsp;&nbsp;";
  		}else{
  			frameHtml +="<div style='padding-top:10px; margin-top:15px;margin-bottom: 10px;'>";
  		}
	  	frameHtml +="<input id=\"cbxSelectAll\" onchange=\"parent.selectAll(document,this.checked);\" type=\"checkbox\" style=\"position: static;\"/>";
...
function dbshow(db_code,varRetInt){
	...
	if(!checkisDB(db_code)){
		if(initDBResutlCount==0){
			$('#db_list_ul').append("<div class='Federal-retrieval_Resultbox-left_item search_result_db_item Federal-retrieval_Resultbox-left_item-slected' onclick='dbtocontent(this)' id='"+db_code+"'><span class='Federal-retrieval_Resultbox-left_item-detail' title='"+db_name+"("+varRetInt+")'><span class='DB_name'>"+db_name+"</span>("+varRetInt+")</span></div>");
		}else{
		    $('#db_list_ul').append("<div class='Federal-retrieval_Resultbox-left_item search_result_db_item' onclick='dbtocontent(this)' id='"+db_code+"'><span class='Federal-retrieval_Resultbox-left_item-detail' title='"+db_name+"("+varRetInt+")'><span class='DB_name'>"+db_name+"</span>("+varRetInt+")</span></div>");
		}
	}
	initDBResutlCount++;
}
...
function checkisDB(db_code){
	var flag = false;
	$("#db_list_ul").find(".search_result_db_item").each(function(){ 
...
function leftDBshow(db_code){
	$("#db_list_ul div.search_result_db_item ").each(function(){ 
		if($(this).attr('id')==db_code){
			$(this).addClass("Federal-retrieval_Resultbox-left_item-slected");
		}else{
			$(this).removeClass("Federal-retrieval_Resultbox-left_item-slected");
		}
	});
}
...
function setDbSerarchResCount(db_code,varRetInt){
	$("#db_list_ul").find("div").each(function(){ 
		if($(this).attr('id')==db_code){
			 $("#"+db_code).html("<span class='Federal-retrieval_Resultbox-left_item-detail' title='"+db_code+"("+varRetInt+")'><span class='DB_name'>"+db_code+"</span>("+varRetInt+")</span>");
		}
	});
}
...
function select_ShDB_Code(obj){
	// status
	if(!obj.checked){
	    // 如果在检索时，不许改变检索条件,也不提示 2018-11-16-ajb
        if($.trim($("#search_ref_db_enum").html()) !== ""){
            return false;
        }
		$("#box_main_ref").find("#dbCB_All").attr("checked",false);
		var selectedNum = $(".selected-author.selected-author_single.selected-author_confirm").size();
		var totalNum = $(".selected-author_single").size();
		// 判断选中与总数
		if(selectedNum == totalNum || (selectedNum+1) == totalNum){
			if($(obj).attr("class") == "selected-author selected-author_single"){
				$("#selected-author_all").addClass("selected-author_confirm");
			}else{
				$("#selected-author_all").removeClass("selected-author_confirm");
			}
		}else{
			$("#selected-author_all").removeClass("selected-author_confirm");
		}
...
function reSearchAction(db_code,isCheckSelect){
  if($("#ifClick").val() != 'true'){
   ...
   var data={'dbcode':db_code,'psnName':psnName,'psnAliasNames':JSON.stringify(jsonData)};
	$.ajax({
		url:'/psnweb/psnalias/ajaxupdate',
		type:'post',
		data:data,
		dataType:'json',
		success:function(json){	
			...
			// 将currentData更新至最新的各文献库排除数据中
			for ( var i = 0; i < search_DB_Code.length; i++) {
			  if("SCI"==search_DB_Code[i] || "SCIE"==search_DB_Code[i]){
			    SCIPsnAliasJsonFRDB = currentData;
			  }
			  if("SSCI"==search_DB_Code[i]){
			    SSCIPsnAliasJsonFRDB = currentData;
			  }
			  if("ISTP"==search_DB_Code[i]){
			    ISTPPsnAliasJsonFRDB = currentData;
			  }
			}
			if(isCheckSelect){
			...
			for ( var i = 0; i < search_DB_Code.length; i++) {
				if("SCI"==search_DB_Code[i] || "SCIE"==search_DB_Code[i] || "SSCI"==search_DB_Code[i] || "ISTP"==search_DB_Code[i]){
...
function showPsnAliasBox(db_code){
	var backgroundDivId = "#boxDiv_"+db_code+"_background";
	var html = "";
	var currentData = switchPsnAliasData(db_code);
	for(var i= 0 ;i<currentData.length;i++){
		var id =currentData[i].ID+"_"+db_code;
		html += "<div code='"+db_code+"' name ='"+currentData[i].NAME+"' class='new-delete_alias-body_item'><div class='new-delete_alias-body_item-detail'>"+currentData[i].NAME+"</div><i class='new-delete_alias-body_item-close' itemId='"+id+"'></i></div>";
	}
	var box = $(backgroundDivId);
	$ul = box.find("div.label-list").attr("alt",db_code).attr("count",0);
	$ul.empty();
	$ul.html(html);
	box.show();
	resizePsnAliasBox(db_code);
}

// 调整排除人名弹框显示
function resizePsnAliasBox(db_code){
    var backgroundDivId = "#boxDiv_"+db_code+"_background";
    var boxDivId = "#boxDiv_"+db_code;
    var parentbox = $(boxDivId)[0];
    parentbox.style.left = (window.innerWidth - parentbox.offsetWidth)/2 + "px";
    parentbox.style.bottom = (window.innerHeight - parentbox.offsetHeight)/2 + "px";
    window.onresize = function(){
        parentbox.style.left = (window.innerWidth - parentbox.offsetWidth)/2 + "px";
        parentbox.style.bottom = (window.innerHeight - parentbox.offsetHeight)/2 + "px";
    }
    var closebtn = document.getElementsByClassName("new-delete_alias-close");
    var closelist = document.getElementsByClassName("new-delete_alias-body_item-close");
    var saveBtn = document.getElementsByClassName("save_psn_alias_btn");
    for(var j = 0; j < closebtn.length; j++){
        closebtn[j].onclick = function(){
            this.closest(".new-delete_alias-container").style.bottom = - 500 + "px";
            $(backgroundDivId).hide();
        }
    }
    for(var i = 0; i < closelist.length; i++){
        closelist[i].onclick = function(){
            this.closest(".new-delete_alias-body").removeChild(this.closest(".new-delete_alias-body_item"));
            reductionPsnAlias($(this).attr("itemId"), db_code);
        }
    }
    for(var j = 0; j < saveBtn.length; j++){
      saveBtn[j].onclick = function(){
          this.closest(".new-delete_alias-container").style.bottom = - 500 + "px";
          $(backgroundDivId).hide();
          reSearchAction(db_code, true);
      }
    }
}
