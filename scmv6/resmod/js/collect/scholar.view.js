...
ScholarView.gotoEdit = function(tab,pubId)
{
	...
	if(window.opener)
	{
		var url = ScholarView.getWebContextPath()+"/publication.do?action=preModify&pub_Id="+pubId+"&tab="+tab+"&pubfly=1";
...
ScholarView.viewPubDetail = function(des3Id,obj,Id){
	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.result==1 || data.ajaxSessionTimeOut=='yes'){
				//ScholarView.forceOpen(ScholarView.getWebContextPath()+"/publication/view?"+params+"&currentDomain="+"/pubweb");
				 newwindow.location.href=ScholarView.getWebContextPath()+"/publication/view?"+params+"&currentDomain="+"/pubweb"+"&pubFlag=1";  
	             newwindow.focus();  
			}else if(data.result==0){
			//window.location.href="/groupweb/grouppub/view?"+params+"&gpubId="+Id;
			//window.open("/groupweb/grouppub/view?"+params+"&gpubId="+Id);
...
{
//	var url = "/pub/view/tmpUrl?tmpUrl="+encodeURIComponent(tmpUrl);
	tmpUrl = encodeURI(tmpUrl);
	var win = window.open(tmpUrl);
	if(win.focus)
	{
		win.focus();
	}
