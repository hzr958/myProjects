...
(function($){
    ...
    $.fn.autoword = function(options){
        var defaults = {
        	...
        	"global" : true, //ajax请求是否触发全局ajax事件
        	"word_block_html":""
        };
...
//           var clickflag = 0;
            ...
            var result = {
                ...
                "put": function(val, text){//添加word
                    ...
                    if(defaults["word_block_html"] != ""){
                    	var blockHtml = defaults["word_block_html"].replace("wordText", text).replace("wordVal", val);
                    	_this.find(".auto_word_input").parent().before(blockHtml);//word添加到DIV
                    }else{
                    ...
                    if(count>0){
                    	_this.find(".auto_word_input").parent().before(	_create_word_AndCount_html(val, text,count));//word添加到DIV
                    		  ...
                    		  if(defaults["word_block_html"] != ""){
                    			  var wordHtml = defaults["word_block_html"].replace("wordText", text).replace("wordVal", val);
                    			  _this.find(".auto_word_input").parent().before(wordHtml);//word添加到DIV
                    		  }else{
                    			  _this.find(".auto_word_input").parent().before(_create_word_html(val, text));//word添加到DIV
                    		  }
                    	  }
...
//                            		defaults["isblur"] = false;
             ...
             "_input_split_char":function(_inputStr){//将输入、复制的关键拆分 by zk 2013/08/13
                  var _splitArray="";
            	   var reE = /[;；]/g;//原先的分割符为多个逗号，冒号现在去除了/[,;，；:：]/g   这里的分隔符是可以在同时输入如：冒；软件额外；复位；变为三个块
            		var repText = "";
            	   ...
            	   if(_splitArray.length>0){
            		   for(var i=0;i<_splitArray.length;i++){
            			   if(_splitArray[i]==''){
            				   _splitArray.splice(i,1);//如果输入的是 ：任务；而非为；任v电风扇； 则会把它分割成四个块，最后一个为空，则需要把最后一个为空的块去掉，不要显示出来
            				   i=i-1;
            			   }
            		   }
	            		for (var i=0; i<_splitArray.length;i++){
...
*/
            ...
            _input.keyup(function(event){
                ...
                if (text.length > 0) {
					...
					if(event.keyCode==13 ){//按键；=(ie:186,ff:59)，=(ie,ff=188) enter:13式完成输入
						if (text.length > 1) {
                ...
                if (!$.isFunction(defaults["blur"]) && defaults["blur"]) {
                    ...
                    if (isOut && text.length > 0 ) {//关联内容隐藏，自动完成单词
                    	...
                    	if(defaults["email"] && !re.test(text)) {
                    		...
                    		// 点击输入自动下拉框提示的关键词不做处理
                    		if(typeof $.autoword.config == "undefined" || ($.autoword.config != "undefined" && !$.autoword.config.mouseDownOnSelect)){
