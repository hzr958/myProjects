...
(function ($) {
     ...
     * @param  success     (function, optional) success function to be called after script is correct terminated
     */
    $.i18n.properties = function (settings) {
        ...
        */
        ...
        var file = settings.name;
        if( file && file.length > 0){
	        var longFileName = settings.path + file + '_' + settings.language + '.properties';
	        if (settings.language.length >= 5) {
	            var longCode = settings.language.substring(0, 5);
	            longFileName = settings.path + file + '_' + longCode + '.properties';
	        }
	        var  fileNames = [longFileName];
	        loadAndParseFiles(fileNames, settings);
	        // call success
	        if (settings.success && !settings.async) {
	            settings.success();
	        }
        }
    }; // properties
                ...
                if (pair.length > 0) {
                    ...
                    if (settings.mode == 'map' || settings.mode == 'both') {
                        ...
                        if (unicodeMatches) {
                        	...
                        	$.each(unicodeMatches, function (i, match) {
    ...
    function unescapeUnicode(str) {
         ...
         * reduce函数在ie 
         */
        return reduce(codes, function (acc, val) { return acc + String.fromCharCode(val); }, '');
    }
    function reduce(array, acc, initValue){
    	var total = initValue;
    	for(var i=0; i<array.length; i++){
    		total = acc(total, array[i]);
    	}
    	return total;
    }
}) (jQuery);
