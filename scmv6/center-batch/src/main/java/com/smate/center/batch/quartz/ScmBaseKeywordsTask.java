...
public class ScmBaseKeywordsTask {
  ...
  public void run() throws BatchTaskException {
    logger.debug("====================================ScmBaseKeywordsTask===开始运行");
    if (isRun() == false) {
      logger.debug("====================================ScmBaseKeywordsTask===开关关闭");
      return;
    } else {
      try {
        doRun();
      } catch (BatchTaskException e) {
        logger.error("ScmBaseKeywordsTask,运行异常", e);
      }
    }
  }

  public void doRun() throws BatchTaskException {
    List<String> toHandleList = this.nsfcKeywordsService.getNsfcCategoryToHandleKwList(0, SIZE);

    if (toHandleList == null || toHandleList.size() == 0) {
      logger.info("=====ScmBaseKeywordsTask处理完毕=====");
      return;
    }
