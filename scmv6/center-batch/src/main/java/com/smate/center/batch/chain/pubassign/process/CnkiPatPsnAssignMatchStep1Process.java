...
/**
 * 单位人员匹配单位所有CnkiPat成果任务第一步.
 * 
 * @author liqinghua
 * 
 */
public class CnkiPatPsnAssignMatchStep1Process implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = 2190935493140472091L;
    ...
    for (int index = 0; index < this.tasks.size(); index++) {
      ...
      if (flag) {
        try {
          ...
          logger.error("CnkiPatPsnAssignMatchStep1Process成果匹配任务: task=" + task.getName(), e);
          throw e;
        } catch (Exception e) {
          logger.error("CnkiPatPsnAssignMatchStep1Process成果匹配任务: task=" + task.getName(), e);
          throw e;
