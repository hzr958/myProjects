...
public class PubToPubsimpleTask22 {
  private final Logger logger = LoggerFactory.getLogger(getClass());

  private final static Integer SIZE = 500; // 每次刷新获取的个数
  private Long startPubId = 25118575L;
  private Long endPubId = 19909669437L;
  ...
  public void run() throws BatchTaskException {
    logger.debug("====================================pubToPubsimpleTask__22===开始运行");
    if (isRun() == false) {
      logger.debug("====================================pubToPubsimpleTask__22===开关关闭");
      return;
    } else {
      try {
        doRun();
      } catch (BatchTaskException e) {
        logger.error("pubToPubsimpleTask__22,运行异常", e);
  ...
  public void doRun() throws BatchTaskException {
    if (taskMarkerService.getApplicationQuartzSettingValue("PubToPubsimpleTask_removePubCache22") == 1) {

      taskMarkerService.closeQuartzApplication("PubToPubsimpleTask_removePubCache22");
    }
    Long count = 0L;
    while (true) {
      count++;
      ...
      if (dataList == null || dataList.size() == 0 || count.equals(3999999L)) {
        logger.info("PubToPubsimpleTask22已经完成");
        break;
      }
      for (Object obj : dataList) {
        ...
        try {
          ...
          resStatus = 2;
          logger.error("PubToPubsimpleTask22 -- 关键词组合出错  pubid ：" + pubId, e);
          resMsg = "--PubToPubsimpleTask22--pubId=" + pubId + "," + e.getMessage();
  ...
  public boolean isRun() throws BatchTaskException {
    return taskMarkerService.getApplicationQuartzSettingValue("PubToPubsimpleTask22") == 1;
  }
}
