 ...
 */
@Repository
public class IsiPubAssignDao extends PdwhHibernateDao<IsiPubAssign, Long> {
  ...
  public IsiPubAssign getIsiPubAssign(Long pubId, Long insId) {
    ...
    String hql = "from IsiPubAssign t where t.pubId = ? and t.insId = ? ";
  ...
  public List<IsiPubAssign> getIsiPubAssignList(Long insId) {
    String hql = "from IsiPubAssign t where t.result=1 and t.insId = ? ";
  ...
  public void pubStatistics(List<Long> insIds, Page<Object> page) {
    ...
    // 查询HQL
    StringBuilder countHql = new StringBuilder("select count(distinct insId) from IsiPubAssign t  where 1 = 1 ");
    StringBuilder queryHql = new StringBuilder("select t.insId,count(t.assignId) from IsiPubAssign t where 1 = 1 ");
    if (insIds != null) {
  ...
  private void pubStatistics(Map<Long, Map> insMap, final int result, final String key) {
    ...
    String hql =
        "select t.insId,count(t.insId) from IsiPubAssign t where  t.result = :result and t.insId in(:insIds) group by t.insId";
  ...
  public List<Long> getIsiPubAssignIdList(Long insId) {
    String hql = "select pubId from IsiPubAssign t where t.result=1 and t.insId = ? ";
    return super.find(hql, insId);
  }
  ...
  public void loadInsPubAssign(Integer result, Long insId, Page<Object> page) {
    ...
    String countHql = "select count(t.insId) from IsiPubAssign t where t.result = :result and t.insId = :insId";
    Long count =
    ...
    String hql = "select pubId from IsiPubAssign t where t.result in(:result) and t.insId = :insId order by assignId ";
    // 获取实际数据
   ...
   * @return
   */
  public List<IsiPubAssign> getNeedMatchPub(Long insId) {

    String hql = "from IsiPubAssign t where t.insId = ? and t.result in(2,3)";
    return super.find(hql, insId);
  }
   ...
   */
  @SuppressWarnings("unchecked")
  public List<IsiPubAssign> getRematchMatchPub(Long startId) {

    String hql = "from IsiPubAssign t where t.status = 0 and t.assignId > ?  order by assignId asc ";
   ...
   */
  @SuppressWarnings("unchecked")
  public List<IsiPubAssign> getNeedSendPub(Long startId, int size) {

    String hql =
        "from IsiPubAssign t where t.assignId > ? and ((t.result = 1 and t.isSend = 0) or t.isSend = 9)  order by assignId asc ";
  ...
  public List<Long> getIsiPubAssList(Long insId, List<Long> pubIdList, int size) {
    ...
    String hql = "select pubId from IsiPubAssign t where t.result=1 and t.insId = :insId and t.pubId in (:pubId) ";
   ...
   * @return
   */
  public IsiPubAssign getPubAssignByAssignId(Long assignId) {

    String hql = "from IsiPubAssign t where t.assignId = ? and ((t.result = 1 and t.isSend = 0) or t.isSend = 9) ";
  ...
  public List<Long> getIsiPubId(Long insId) {
    ...
    String hql = "select t.pubId from IsiPubAssign t where t.insId = ? and t.status = 3";
