 ...
 * 
 */
public class ExtractCniprAuthorNameTask implements IPubAssignXmlTask {

  private final String name = "ExtractCniprAuthorNameTask";
  @Autowired
  ...
  public boolean run(PubXmlDocument xmlDocument, PubXmlProcessContext context) throws Exception {
    ...
    if (authorList != null && authorList.size() > 0) {
      for (int i = 0; i < authorList.size(); i++) {
        ...
        this.pubAssignCniprXmlService.savePubAuthor(context.getCurrentPubId(), context.getCurrentInsId(), auname, insId,
            seqNo);
      }
    } else {
      return false;
