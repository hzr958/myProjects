 ...
 * 
 */
public class IsiPubAssignMatchProcess implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = 89600712656075369L;
    ...
    for (int index = 0; index < this.tasks.size(); index++) {
      ...
      if (flag) {
        try {
          ...
          e.printStackTrace();
          logger.error("IsiPubAssignMatchProcessISI成果匹配任务: task=" + task.getName(), e);
          throw e;
        } catch (Exception e) {
          e.printStackTrace();
          logger.error("IsiPubAssignMatchProcessISI成果匹配任务: task=" + task.getName(), e);
