...
public class PdwhPubKeywordsTask4 {
  ...
  public void run() throws BatchTaskException {
    logger.info("====================================PdwhPubKeywordsTask4===开始运行");
    if (isRun() == false) {
      logger.info("====================================PdwhPubKeywordsTask4===开关关闭");
      return;
    } else {
      try {
        taskMarkerService.closeQuartzApplication("PdwhPubKeywordsTask4");
        doRun();
      } catch (BatchTaskException e) {
        logger.error("PdwhPubKeywordsTask4,运行异常", e);
  ...
  private void doRun() throws BatchTaskException {
    while (true) {
      List<Map<String, Object>> pubIdList = this.nsfcKeywordsService.getEncodedPubIdList(5000, 5L);
  ...
  public boolean isRun() throws BatchTaskException {
    // 任务开关控制逻辑
    // return true;
    return taskMarkerService.getApplicationQuartzSettingValue("PdwhPubKeywordsTask4") == 1;
  }
}
