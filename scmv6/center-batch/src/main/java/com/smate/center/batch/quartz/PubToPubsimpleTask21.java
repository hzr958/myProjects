...
public class PubToPubsimpleTask21 {
  private final Logger logger = LoggerFactory.getLogger(getClass());

  private final static Integer SIZE = 500; // 每次刷新获取的个数
  private Long startPubId = 17398573L;
  private Long endPubId = 25118575L;
  ...
  public void run() throws BatchTaskException {
    logger.debug("====================================pubToPubsimpleTask__21===开始运行");
    if (isRun() == false) {
      logger.debug("====================================pubToPubsimpleTask__21===开关关闭");
      return;
    } else {
      try {
        doRun();
      } catch (BatchTaskException e) {
        logger.error("pubToPubsimpleTask__21,运行异常", e);
  ...
  public void doRun() throws BatchTaskException {
    if (taskMarkerService.getApplicationQuartzSettingValue("PubToPubsimpleTask_removePubCache21") == 1) {

      taskMarkerService.closeQuartzApplication("PubToPubsimpleTask_removePubCache21");
    }
    Long count = 0L;
    while (true) {
      count++;
      ...
      for (Object obj : dataList) {
        ...
        try {
          ...
          resStatus = 2;
          logger.error("PubToPubsimpleTask21 -- 关键词组合出错  pubid ：" + pubId, e);
          resMsg = "--PubToPubsimpleTask21--pubId=" + pubId + "," + e.getMessage();
  ...
  public boolean isRun() throws BatchTaskException {
    return taskMarkerService.getApplicationQuartzSettingValue("PubToPubsimpleTask21") == 1;
  }
}
