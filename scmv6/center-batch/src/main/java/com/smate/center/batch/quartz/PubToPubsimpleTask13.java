...
public class PubToPubsimpleTask13 {
  ...
  public void run() throws BatchTaskException {
    logger.debug("====================================pubToPubsimpleTask__13===开始运行");
    if (isRun() == false) {
      logger.debug("====================================pubToPubsimpleTask__13===开关关闭");
      return;
    } else {
      try {
        doRun();
      } catch (BatchTaskException e) {
        logger.error("pubToPubsimpleTask__13,运行异常", e);
  ...
  public void doRun() throws BatchTaskException {
    while (true) {
      List<BigDecimal> psnList = nsfcKeywordsService.getPsnToHandleKwList(SIZE, 13);
  ...
  public boolean isRun() throws BatchTaskException {
    // 任务开关控制逻辑
    return true;
    // return taskMarkerService.getApplicationQuartzSettingValue("PubToPubsimpleTask13") == 1;
  }
}
