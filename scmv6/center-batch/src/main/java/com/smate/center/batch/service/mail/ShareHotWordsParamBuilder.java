 ...
 * 
 */
@Service("shareHotWordsParamBuilder")
public class ShareHotWordsParamBuilder extends CommonInsideMailParamBuilder implements InsideMailParamBuilder {
  ...
  public Map<String, Object> builderParam(Message message, List<Long> receiverIdList) throws ServiceException {
    try {
      ...
      map.put("msgType", String.valueOf(InsideMessageConstants.MSG_TYPE_SHARE_HOTWORDS));
      map.put("notSysUserTemplate", "Email_Share_HotWords_NotSysUser_Template_${locale}.ftl");
      map.put("recommendUrl", message.getRecommendUrl());

      return map;
    } catch (Exception e) {
      logger.error("短信－构造分享热点领域所需参数出现异常：", e);
