 ...
 * 
 */
public class CnkiPatPubAssignMatchProcess implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = 2367731311065698017L;
    ...
    for (int index = 0; index < this.tasks.size(); index++) {
      ...
      if (flag) {
        try {
          ...
          e.printStackTrace();
          logger.error("CnkiPatPubAssignMatchProcess成果匹配任务: task=" + task.getName(), e);
          throw e;
        } catch (Exception e) {
          e.printStackTrace();
          logger.error("CnkiPatPubAssignMatchProcess成果匹配任务: task=" + task.getName(), e);
