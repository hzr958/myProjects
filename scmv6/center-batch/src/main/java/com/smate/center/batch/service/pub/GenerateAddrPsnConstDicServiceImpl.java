...
import com.smate.center.batch.dao.pdwh.prj.NsfcKwDisciplineDao;
import com.smate.center.batch.dao.pdwh.prj.NsfcKwForSortingDao;
import com.smate.center.batch.dao.rcmd.pub.NsfcKwsRcmdDao;
...
public class GenerateAddrPsnConstDicServiceImpl implements GenerateAddrPsnConstDicService {
  ...
  @Autowired
  private NsfcKwsRcmdDao nsfcKwsRcmdDao;
  @Autowired
  private KeywordsHighAndNewTechDao keywordsHighAndNewTechDao;
  @Autowired
  private NsfcKwDisciplineDao nsfcKwDisciplineDao;
  ...
  public void generateNsfcKwsDic() {
    List<String> kwlist = nsfcKwDisciplineDao.getScmKwDic();
    Integer k = 0;
    for (String string : kwlist) {
      try {
        ...
        if (XmlUtil.isChinese(string)) {
          ...
          DicLibrary.insert(DicLibrary.DEFAULT, replaceCharsZH(string.toLowerCase().trim()), "nsfc_kw_discipline",
              DicLibrary.DEFAULT_FREQ);
        } else {
          // this.writeToDicFile(kwDic, cleanAuthorChars(string));
          DicLibrary.insert(DicLibrary.DEFAULT, cleanAuthorChars(string.toLowerCase().trim()), "nsfc_kw_discipline",
              DicLibrary.DEFAULT_FREQ);
        }
        k++;
      } catch (Exception e) {
        logger.error("nsfc关键词词典写入出错，name:" + string, e);

      }
    }
    System.out.println("nsfc关键词词典写入关键词数量为：" + k);
  }

  @Override
  public void generateHighTechKwsDic() {
    List<String> kwlist = keywordsHighAndNewTechDao.getAllKwStr();
    Integer k = 0;
    for (String string : kwlist) {
      try {
        if (StringUtils.isEmpty(string)) {
        ...
        if (XmlUtil.isChinese(string)) {
          // this.writeToDicFile(kwDic, replaceCharsZH(string));
          DicLibrary.insert(DicLibrary.DEFAULT, replaceCharsZH(string.toLowerCase().trim()), "hightech_kw",
              DicLibrary.DEFAULT_FREQ);
        } else {
          // this.writeToDicFile(kwDic, cleanAuthorChars(string));
          DicLibrary.insert(DicLibrary.DEFAULT, cleanAuthorChars(string.toLowerCase().trim()), "hightech_kw",
              DicLibrary.DEFAULT_FREQ);
        }
        k++;
      } catch (Exception e) {
        logger.error("高新技术关键词词典写入出错，name:" + string, e);
      }
    }
    System.out.println("高新技术关键词词典写入关键词数量为：" + k);
  }

  @Override
  public void generateNsfcKwsDicByCategory(String category) {

    List<String> kwlist = nsfcKwsRcmdDao.getKwByCategory(category);
    Integer k = 0;
    for (String string : kwlist) {
      try {
        if (StringUtils.isEmpty(string)) {
  ...
  public void writeToDicFile(String fileName, String content)
      throws IOException {/*
                           ...
                           * 可检测多种类型，并剔除bom BOMInputStream bomIn = new BOMInputStream(InputStringStream, false,
                           * ByteOrderMark.UTF_8, ByteOrderMark.UTF_16LE, ByteOrderMark.UTF_16BE); String charset =
                           * "utf-8"; // 若检测到bom，则使用bom对应的编码 if (bomIn.hasBOM()) { charset = bomIn.getBOMCharsetName();
