...
public class PubAssignCnkiConferenceDao extends RolHibernateDao<PubAssignCnkiConference, Long> {
  ...
  public PubAssignCnkiConference getPubAssignConferenceByPubId(Long pubId) {
    String hql = "from PubAssignCnkiConference where pubId = ? ";
    List<PubAssignCnkiConference> list = super.find(hql, pubId);
    if (list.size() > 0) {
      return list.get(0);
    }
    return null;
  ...
  public List<PsnPmConference> getPcMatchPubPc(Long pubId, Set<Long> psnIds) {
    String hql =
        "select pp from PubAssignCnkiConference pc ,PsnPmConference pp where pc.pubId = :pubId and pp.psnId in(:psnIds) and pc.nameHash = pp.nameHash ";
