...
public class PubAssignPubMedKeywordsDao extends RolHibernateDao<PubAssignPubMedKeywords, Long> {
  ...
  public boolean isExitKeyWords(Long pubId, Integer keyHash) {
    ...
    String hql = "select count(id) from PubAssignPubMedKeywords where pubId = ? and keyHash = ? ";
  ...
  public Map<Long, Long> getKwMatchPubKw(Long pubId, Set<Long> psnIds) {
    ...
    String kwhql = "select pa.keyHash from PubAssignPubMedKeywords pa where pa.pubId = ? ";
