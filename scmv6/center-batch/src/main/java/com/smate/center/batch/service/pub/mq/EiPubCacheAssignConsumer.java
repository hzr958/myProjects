 ...
 */
@Deprecated
@Component("EiPubCacheAssignConsumer")
public class EiPubCacheAssignConsumer {
  ...
  public void receive(EiPubCacheAssignMessage message) throws ServiceException {

    try {

      EiPubCacheAssignMessage msg = (EiPubCacheAssignMessage) message;
      logger.debug("#####EI成果批量抓取指派给单位数据xmlId:" + msg.getXmlId());
      // 是否是导入isi文献库的文献
      if (msg.getType() == 1 && StringUtils.isNotBlank(msg.getXmlData())) {
        Long pubId = rolPublicationXmlManager.backgroundImportEiPubXml(msg);
   ...
   * @throws Exception
   */
  private void savePdwhId(EiPubCacheAssignMessage msg, Long pubId) throws Exception {
