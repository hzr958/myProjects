 ...
 */
@Repository
public class PubAssignEiKeywordsDao extends RolHibernateDao<PubAssignEiKeywords, Long> {
  ...
  public boolean isExitKeyWords(Long pubId, Integer keyHash) {
    ...
    String hql = "select count(id) from PubAssignEiKeywords where pubId = ? and keyHash = ? ";
  ...
  public Map<Long, Long> getKwMatchPubKw(Long pubId, Set<Long> psnIds) {
    ...
    String kwhql = "select pa.keyHash from PubAssignEiKeywords pa where pa.pubId = ? ";
    ...
    String hql =
        "select pkw.psnId,count(pkw.id) from PsnPmKeyWordRol pkw where pkw.psnId in(:psnIds) and pkw.kwhash in(:kwhash) group by pkw.psnId ";
   ...
   * @return
   */
  public List<PubAssignEiKeywords> getKwByPubId(Long pubId) {
    String hql = "from PubAssignEiKeywords where pubId = ? ";
