 ...
 */
@Repository
public class PubAssignCniprKeywordsDao extends RolHibernateDao<PubAssignCniprKeywords, Long> {
  ...
  public boolean isExitKeyWords(Long pubId, Integer keyHash) {
    ...
    String hql = "select count(id) from PubAssignCniprKeywords where pubId = ? and keyHash = ? ";
  ...
  public Map<Long, Long> getKwMatchPubKw(Long pubId, Set<Long> psnIds) {
    ...
    String kwhql = "select pa.keyHash from PubAssignCniprKeywords pa where pa.pubId = ? ";
   ...
   * @return
   */
  public List<PubAssignCniprKeywords> getKwByPubId(Long pubId) {
    String hql = "from PubAssignCniprKeywords where pubId = ? ";
