...
public class PdwhPubClassificationByKwsCotfTask {
  private final Logger logger = LoggerFactory.getLogger(getClass());

  private final static Integer SIZE = 500; // 每次刷新获取的个数
  private Long startPubId = 10862967L;
  private Long endPubId = 17398573L;
  ...
  public void run() throws BatchTaskException {
    logger.debug("====================================PdwhPubClassificationByKwsCotfTask===开始运行");
    if (isRun() == false) {
      logger.debug("====================================PdwhPubClassificationByKwsCotfTask===开关关闭");
      return;
    } else {
      try {
        doRun();
      } catch (BatchTaskException e) {
        logger.error("pubToPubsimpleTask__20,运行异常", e);
  ...
  public void doRun() throws BatchTaskException {
    if (taskMarkerService.getApplicationQuartzSettingValue("PubToPubsimpleTask_removePubCache20") == 1) {

      taskMarkerService.closeQuartzApplication("PubToPubsimpleTask_removePubCache20");
    }
    Long count = 0L;
    while (true) {
      count++;
      ...
      if (dataList == null || dataList.size() == 0 || count.equals(3999999L)) {
        logger.info("PdwhPubClassificationByKwsCotfTask已经完成");
        break;
      }
      for (Object obj : dataList) {
        ...
        try {
          ...
          resStatus = 2;
          logger.error("PdwhPubClassificationByKwsCotfTask -- 关键词组合出错  pubid ：" + pubId, e);
          resMsg = "--PdwhPubClassificationByKwsCotfTask--pubId=" + pubId + "," + e.getMessage();
  ...
  public boolean isRun() throws BatchTaskException {
    return taskMarkerService.getApplicationQuartzSettingValue("PubToPubsimpleTask20") == 1;
  }
}
