 ...
 * 
 */
public class PubMedPsnAssignMatchStep2Process implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = 4725003097692847831L;
      ...
      for (int index = 0; index < this.tasks.size(); index++) {
        ...
        if (flag) {
          try {
            ...
            logger.error("PubMedPsnAssignMatchStep2Process成果匹配任务: task=" + task.getName(), e);
            throw e;
          } catch (Exception e) {
            logger.error("PubMedPsnAssignMatchStep2Process成果匹配任务: task=" + task.getName(), e);
            throw e;
  ...
  private PubAssginMatchContext buildPubAssginMatchContext(Long pubId, Map<Integer, PubAssignScoreMap> scoreMap)
    ...
    pubContext.setSettingPubAssignMatchScore(
        pubAssignPubMedMatchService.getSettingPubAssignScore(pub.getSourceDbId().longValue()));
