...
/**
 * scopus单位人员匹配单位所有成果任务第二步.
 * 
 * @author liqinghua
 * 
 */
public class SpsPsnAssignMatchStep2Process implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = -4735041625469505605L;
      ...
      for (int index = 0; index < this.tasks.size(); index++) {
        ...
        if (flag) {
          try {
            ...
            logger.error("SpsPsnAssignMatchStep2Process成果匹配任务: task=" + task.getName(), e);
            throw e;
          } catch (Exception e) {
            logger.error("SpsPsnAssignMatchStep2Process成果匹配任务: task=" + task.getName(), e);
            throw e;
  ...
  private PubAssginMatchContext buildPubAssginMatchContext(Long pubId, Map<Integer, PubAssignScoreMap> scoreMap)
    ...
    pubContext.setSettingPubAssignMatchScore(
        pubAssignSpsMatchService.getSettingPubAssignScore(pub.getSourceDbId().longValue()));
