...
import com.smate.center.batch.model.sns.pub.ConstDictionaryForm;
import com.smate.center.batch.model.sns.pub.ConstDictionaryKey;
...
public class ConstDictionaryManageCbImpl implements ConstDictionaryManageCb, Serializable {
  ...
  @Autowired
  private ConstCategoryService constCategoryService;
  ...
  private List<ConstDictionary> doSaveAddConstDictionary(ConstDictionaryForm form) throws ServiceException {
    ...
    try {
      ...
      // 保存常量类别
      ConstCategory constCategory = constCategoryService.saveConstCategory(new ConstCategory(category, description));
      String strDictionary = form.getDictionary();
      String[] strDictionarys = strDictionary.split("_\\|_");
      List<ConstDictionary> list = new ArrayList<ConstDictionary>();
      for (String dic : strDictionarys) {
        // 解析 JSON数据
        HashMap<String, String> mapDic =
            (HashMap<String, String>) JSONObject.toBean(JSONObject.fromObject(dic), HashMap.class);
        String code = mapDic.get("code");
        String enDesc = mapDic.get("enDesc");
        String cnDesc = mapDic.get("cnDesc");
        String twDesc = mapDic.get("twDesc");
        String seqNo = mapDic.get("seqNo");
        ConstDictionary dictionary = new ConstDictionary(category, code, enDesc, cnDesc, twDesc, seqNo);
        // 保存常量
        this.constDictionaryDao.save(dictionary);
        dictionary.setConstCategory(constCategory);
        list.add(dictionary);
      }
      return list;
    } catch (Exception e) {
      String jobj = JSONObject.fromObject(form).toString();
      logger.error("保存新添加常量错误form:{}", jobj, e);
      throw new ServiceException(e);
    }
  }

  @Transactional(rollbackFor = Exception.class, propagation = Propagation.REQUIRES_NEW)
  private ConstDictionary doSaveEditConstDictionary(ConstDictionaryForm form) throws ServiceException {

    String category = form.getCategory();
    ConstCategory constCategory = constCategoryService.getConstCategory(category);
    if (constCategory == null) {
      throw new ServiceException(String.format("常量不存在code:%s,category:%s", form.getCode(), form.getCategory()));
    }
    ConstDictionary dictionary = this.constDictionaryDao.findConstByCategoryAndCode(category, form.getCode());
    if (dictionary == null) {
      dictionary = new ConstDictionary();
      ConstDictionaryKey key = new ConstDictionaryKey(category, form.getCode());
      dictionary.setKey(key);
      dictionary.setEditFlag(false);
    } else {
      dictionary.setEditFlag(true);
    }
    dictionary.setEnUsName(form.getEnName());
    dictionary.setZhCnName(form.getCnName());
    dictionary.setZhTwName(form.getTwName());
    dictionary.setSeqNo(form.getSeqNo());
    this.constDictionaryDao.save(dictionary);
    dictionary.setConstCategory(constCategory);
    return dictionary;
  }
