 ...
 * 
 */
public class ExtractEiAuthorNameTask implements IPubAssignXmlTask {

  private final String name = "ExtractEiAuthorNameTask";
  @Autowired
  ...
  public boolean can(PubXmlDocument xmlDocument, PubXmlProcessContext context) {
    return context.isImport() && context.isEiImport();
  }
  ...
  public boolean run(PubXmlDocument xmlDocument, PubXmlProcessContext context) throws Exception {
    ...
    if (authorList != null && authorList.size() > 0) {
      for (int i = 0; i < authorList.size(); i++) {
        ...
        if (fullName.equalsIgnoreCase(abbrName)) {
          abbrName = null;
        } else if (StringUtils.isNotBlank(abbrName)) {
          // 去除所有空格，简称如果等于全称，则设置全称为空
          String wordAbbrName = abbrName.replaceAll("\\s+", "");
          String wordFullName = fullName.replaceAll("\\s+", "");
          if (wordFullName.equalsIgnoreCase(wordAbbrName)) {
            abbrName = null;
          }
        }
        // 查找是否有本单位，如果没有，随机取一个单位
        Long insId = ExtractEiAuthorNameTask.getAuthorInsId(author, context);
