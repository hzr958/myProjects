 ...
 * 
 */
public class SpsPsnAssignMatchStep1Process implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = 2495847420557011095L;
    ...
    for (int index = 0; index < this.tasks.size(); index++) {
      ...
      if (flag) {
        try {
          ...
          logger.error("SpsPsnAssignMatchStep1Process成果匹配任务: task=" + task.getName(), e);
          throw e;
        } catch (Exception e) {
          logger.error("SpsPsnAssignMatchStep1Process成果匹配任务: task=" + task.getName(), e);
          throw e;
