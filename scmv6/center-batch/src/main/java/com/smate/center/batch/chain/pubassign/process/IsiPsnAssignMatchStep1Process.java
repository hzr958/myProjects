 ...
 * 
 */
public class IsiPsnAssignMatchStep1Process implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = -719773213894301319L;
    ...
    for (int index = 0; index < this.tasks.size(); index++) {
      ...
      if (flag) {
        try {
          ...
          logger.error("IsiPsnAssignMatchStep1Process成果匹配任务: task=" + task.getName(), e);
          throw e;
        } catch (Exception e) {
          logger.error("IsiPsnAssignMatchStep1Process成果匹配任务: task=" + task.getName(), e);
          throw e;
