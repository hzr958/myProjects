 ...
 * 
 */
public class CnkiPatPubAssignXmlProcess implements IPubAssignXmlProcess {

  /**
   * 
   */
  private static final long serialVersionUID = 6031336107461090450L;
  // logger.
  ...
  public void setTasks(List<IPubAssignXmlTask> tasks) {
    ...
    logger.info("初始化CnkiPatPubAssignXmlProcess Tasks成功");
  }

  @Override
  public void start(PubXmlDocument xmlDocument, PubXmlProcessContext context) throws Exception {

    if (context.isImport() && context.isCnkiPatImport()) {
      ...
      for (int index = 0; index < this.tasks.size(); index++) {
        ...
        if (flag) {
          try {
            ...
            e.printStackTrace();
            logger.error("CnkiPatPubAssignXmlProcess终止处理该XML: task=" + task.getName(), e);
            throw e;
          } catch (Exception e) {
            e.printStackTrace();
            logger.error("CnkiPatPubAssignXmlProcess终止处理该XML: task=" + task.getName(), e);
