...
import com.smate.center.batch.chain.pub.MultiSubsetsFromPrpAndPubTask;
...
/**
 * 计算nsfc prj子集，写入txt文件用于hadoop任务
 * 
 */
public class KGSubsetsByNsfcPrjTask {
    ...
    public void run() throws BatchTaskException {
        logger.debug("====================================KGSubsetsByNsfcPrjTask===开始运行");
        if (isRun() == false) {
            logger.debug("====================================KGSubsetsByNsfcPrjTask===开关关闭");
            return;
        } else {
            try {
                doRun();
            } catch (BatchTaskException e) {
                logger.error("KGSubsetsByNsfcPrjTask,运行异常", e);
    ...
    public void doRun() throws BatchTaskException {
        ...
        // type =1 成果
        List<BigDecimal> pdwhPubIdList = nsfcKeywordsService.getSubsetsToHandleKwList(SIZE, 2);
        if (pdwhPubIdList == null || pdwhPubIdList.size() == 0) {
            logger.info("====================================KGSubsetsByNsfcPrjTask===运行完毕");
            return;
        }
        ...
        while (count <= sum) {
            ...
            for (Integer i = start; i < batch; i++) {
                ...
                MultiSubsetsFromPrpAndPubTask mst =
                        new MultiSubsetsFromPrpAndPubTask(pdwhPubIdList.get(i).longValue(), sb, latch, null);
                tpe.execute(mst);
            }
    ...
    public static void main(String[] args) {
        KGSubsetsByNsfcPrjTask ks = new KGSubsetsByNsfcPrjTask();
        File file = ks.getFileName("C:/Users/Administrator/Desktop/20181015open/pubtestfile/");
        File file1 = new File("C:/Users/Administrator/Desktop/20181015open/pubtestfile/subset_pub_1562361533186.txt");
