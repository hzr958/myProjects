...
public class PubToPubsimpleTask19 {
  private final Logger logger = LoggerFactory.getLogger(getClass());

  private final static Integer SIZE = 500; // 每次刷新获取的个数
  private Long startPubId = 5063498L;
  private Long endPubId = 10862967L;
  ...
  public void run() throws BatchTaskException {
    logger.debug("====================================PubToPubsimpleTask19===开始运行");
    if (isRun() == false) {
      logger.debug("====================================PubToPubsimpleTask19===开关关闭");
      return;
    } else {
      try {
        doRun();
      } catch (BatchTaskException e) {
        logger.error("PubToPubsimpleTask19,运行异常", e);
  ...
  public void doRun() throws BatchTaskException {
    if (taskMarkerService.getApplicationQuartzSettingValue("PubToPubsimpleTask_removePubCache19") == 1) {

      taskMarkerService.closeQuartzApplication("PubToPubsimpleTask_removePubCache19");
    }
    Long count = 0L;
    while (true) {
      count++;
      ...
      if (dataList == null || dataList.size() == 0 || count.equals(3999999L)) {
        logger.info("PubToPubsimpleTask19已经完成");
        break;
      }
      for (Object obj : dataList) {
        ...
        try {
          ...
          resStatus = 2;
          logger.error("PubToPubsimpleTask19 -- 关键词组合出错  pubid ：" + pubId, e);
          resMsg = "--PubToPubsimpleTask19--pubId=" + pubId + "," + e.getMessage();
  ...
  public boolean isRun() throws BatchTaskException {
    return taskMarkerService.getApplicationQuartzSettingValue("PubToPubsimpleTask19") == 1;
  }
}
