...
/**
 * CnkiPat成果批量抓取指派给单位接收端.
 * 
 * @author liqinghua
 * 
 */
@Deprecated
@Component("cnkiPatPubCacheAssignConsumer")
public class CnkiPatPubCacheAssignConsumer {
  ...
  public void receive(CnkiPatPubCacheAssignMessage msg) throws ServiceException {

    try {
      logger.info("#####CnkiPat成果批量抓取指派给单位数据xmlId:" + msg.getXmlId());
      // 是否是CNIPR文献库的文献
      if (StringUtils.isNotBlank(msg.getXmlData())) {
        Long pubId = rolPublicationXmlManager.backgroundImportCnkiPatPubXml(msg);
        this.savePdwhId(msg, pubId);
      }
    } catch (Exception e) {
      logger.error("CnkiPatPubCacheAssignConsumer导入XML错误", e);
   ...
   * @throws Exception
   */
  private void savePdwhId(CnkiPatPubCacheAssignMessage msg, Long pubId) throws Exception {
