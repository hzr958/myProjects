 ...
 * 
 */
@Service("shareFundParamBuilder")
public class ShareFundParamBuilder extends CommonInsideMailParamBuilder implements InsideMailParamBuilder {
  ...
  public Map<String, Object> builderParam(Message message, List<Long> receiverIdList) throws ServiceException {
    try {
      ...
      map.put("msgType", String.valueOf(InsideMessageConstants.MSG_TYPE_SHARE_FUND));
      map.put("notSysUserTemplate", "Email_Share_Fund_NotSysUser_Template_${locale}.ftl");
      map.put("recommendUrl", message.getRecommendUrl());

      return map;
    } catch (Exception e) {
      logger.error("短信－构造分享基金推荐所需参数出现异常：", e);
