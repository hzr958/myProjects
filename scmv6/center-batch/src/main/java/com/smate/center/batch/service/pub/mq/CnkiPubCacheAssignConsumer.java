 ...
 */
@Deprecated
@Component("cnkiPubCacheAssignConsumer")
public class CnkiPubCacheAssignConsumer {
  ...
  public void receive(CnkiPubCacheAssignMessage msg) throws ServiceException {

    try {
      logger.info("#####CNKI成果批量抓取指派给单位数据xmlId:" + msg.getXmlId());
      // 是否是CNKI文献库的文献
      if (StringUtils.isNotBlank(msg.getXmlData())) {
        Long pubId = rolPublicationXmlManager.backgroundImportCnkiPubXml(msg);
        this.savePdwhId(msg, pubId);
      }
    } catch (Exception e) {
      logger.error("CnkiPubCacheAssignConsumer导入XML错误", e);
   ...
   * @throws Exception
   */
  private void savePdwhId(CnkiPubCacheAssignMessage msg, Long pubId) throws Exception {
