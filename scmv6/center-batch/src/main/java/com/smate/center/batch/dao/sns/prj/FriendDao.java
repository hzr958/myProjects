...
import com.smate.center.batch.model.sns.prj.Friend;
...
public class FriendDao extends SnsHibernateDao<Friend, Long> {
  ...
  // param = "%" + param.replace(" ", "") + "%";
  // String hql =
  // "select distinct p.psnName,p.psnId from SyncPerson p,Friend f where p.psnId=f.friendPsnId and
  // (replace(p.psnName,' ','') like ? or replace(p.psnLastName,' ','')||replace(p.psnFirstName,'
  // ','') like ?) and f.psnId=? ";
