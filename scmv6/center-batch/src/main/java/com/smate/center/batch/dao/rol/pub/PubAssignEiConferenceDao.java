 ...
 */
@Repository
public class PubAssignEiConferenceDao extends RolHibernateDao<PubAssignEiConference, Long> {
   ...
   * @return
   */
  public PubAssignEiConference getPubAssignConferenceByPubId(Long pubId) {
    String hql = "from PubAssignEiConference where pubId = ? ";
    List<PubAssignEiConference> list = super.find(hql, pubId);
    if (list.size() > 0) {
      return list.get(0);
    }
    return null;
  ...
  public List<PsnPmConference> getPcMatchPubPc(Long pubId, Set<Long> psnIds) {
    String hql =
        "select pp from PubAssignEiConference pc ,PsnPmConference pp where pc.pubId = :pubId and pp.psnId in(:psnIds) and pc.nameHash = pp.nameHash ";
