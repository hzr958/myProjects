 ...
 * 
 */
public class KGPrjKwsExtractTask {
  ...
  public void doRun() throws BatchTaskException {
    ...
    List<BigDecimal> pdwhPubIdList = nsfcKeywordsService.getNsfcPrjToHandleKwList(SIZE);
    ...
    for (BigDecimal id : pdwhPubIdList) {
      ...
      Integer rs = 0;
      try {
        // rs = this.nsfcKeywordsService.extractPrjKwsFromTitleAndAbsByTfEn(prjId);
        rs = this.nsfcKeywordsService.extractPrjKwsFromTitleAndAbsByTfForGaoxiao(prjId);
        this.nsfcKeywordsService.updateNsfcPrjStatus(prjId, rs);
      } catch (Exception e) {
