...
public class PubToPubsimpleTask26 {
  ...
  public void run() throws BatchTaskException {
    logger.debug("====================================PubToPubsimpleTask26===开始运行");
    if (isRun() == false) {
      logger.debug("====================================PubToPubsimpleTask26===开关关闭");
      return;
    } else {
      try {
        doRun();
      } catch (BatchTaskException e) {
        logger.error("PubToPubsimpleTask26,运行异常", e);
  ...
  public void doRun() throws BatchTaskException {
    // SIZE
    List<String> pdwhPubIdList = nsfcKeywordsService.getNsfcCategoryToHandleKwList(11, SIZE);
