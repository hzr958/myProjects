...
/**
 * 单位人员匹配单位所有Cnki成果任务第一步.
 * 
 * @author liqinghua
 * 
 */
public class CnkiPsnAssignMatchStep1Process implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = 8184622884375217786L;
    ...
    for (int index = 0; index < this.tasks.size(); index++) {
      ...
      if (flag) {
        try {
          ...
          logger.error("CnkiPsnAssignMatchStep1Process成果匹配任务: task=" + task.getName(), e);
          throw e;
        } catch (Exception e) {
          logger.error("CnkiPsnAssignMatchStep1Process成果匹配任务: task=" + task.getName(), e);
          throw e;
