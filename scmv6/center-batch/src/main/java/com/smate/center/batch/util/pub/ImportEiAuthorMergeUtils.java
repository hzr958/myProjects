 ...
 * 
 */
public class ImportEiAuthorMergeUtils {
  ...
  public static List<Map<String, String>> mergeNamesToOldAuthor(List<String> pubAddrs, List<String> namesList,
    ...
    for (int i = 0; namesList != null && i < namesList.size(); i++) {
      ...
      for (int j = 0; mapPsns != null && j < mapPsns.size(); j++) {
        ...
        // 匹配是否相同，相同则合并
        if (XmlUtil.compareAuthorName(full, authorName)) {
    ...
    for (int j = 0; mapPsns != null && j < mapPsns.size(); j++) {
      ...
      newAuthor.put("full_name", authorName);
      newAuthor.put("abbr_name", "");
      newAuthor.put("seq_no", String.valueOf(seqNo));
  ...
  /**
   * 解析抓取数据publication下面用户名称数据.已经屏蔽abbr_name，但是为了和isi的分配过程保持一致，仍然在最后加入abbr_name节点
   * 
   * @param doc
   * @return
   */
  private static List<String> parseNameList(PubXmlDocument doc) {
    ...
    // String authorNamesAbbrStr = doc.getXmlNodeAttribute(PubXmlConstants.PUBLICATION_XPATH,
    // "author_names_abbr");
    ...
    if (pubAuthors != null && pubAuthors.size() > 0) {
      for (int k = 0; k < pubAuthors.size(); k++) {
        ...
        if (StringUtils.isBlank(au)) {
          ...
          // 重复，则合并
          if (dup != null) {
            ImportEiAuthorMergeUtils.mergeSameAu(dup, map);
          } else {
            mapPsn.add(map);
          }
        }
