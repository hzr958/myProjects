  ...
  public boolean run(PubXmlDocument xmlDocument, PubXmlProcessContext context) throws Exception {
    ...
    if (xmlDocument.isPatent()) {// 专利
      ...
      if (xmlDocument.existsNodeAttribute(PubXmlConstants.PUB_PATENT_XPATH, "start_date")) {
        // 生效开始日期
        dates = XmlFragmentCleanerHelper.splitDateYearMonth(xmlDocument, PubXmlConstants.PUB_PATENT_XPATH, "start_date",
            pattern);
      ...
      // if
      // (xmlDocument.existsNodeAttribute(PubXmlConstants.PUB_CONF_PAPER_XPATH,
      // "start_date")) {
      // String startDate =
      // xmlDocument.getXmlNodeAttribute(PubXmlConstants.PUB_CONF_PAPER_XPATH,
      // "start_date");
      // String endDate =
      // xmlDocument.getXmlNodeAttribute(PubXmlConstants.PUB_CONF_PAPER_XPATH,
      // "end_date");
      ...
      if (xmlDocument.existsNodeAttribute(PubXmlConstants.PUB_OTHER_XPATH, "pub_date")) {
        ...
        dates = XmlFragmentCleanerHelper.splitDateYearMonth(xmlDocument, PubXmlConstants.PUB_OTHER_XPATH, "start_date",
            pattern);
      ...
      if (!StringUtils.isBlank(programme)) {
        ConstDictionary cd = context.getXmlServiceFactory().getConstDictionaryManage()
            .findConstByCategoryAndCode("pub_thesis_degree", programme);
        if (cd != null) {
    ...
    dates = XmlFragmentCleanerHelper.splitDateYearMonth(xmlDocument, PubXmlConstants.PUBLICATION_XPATH, "publish_date",
        pattern);
    if (!"".equals(dates[0])) {
      publishYear = dates[0];
