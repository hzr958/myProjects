...
public class PubAssignPubMedJournalDao extends RolHibernateDao<PubAssignPubMedJournal, Long> {
  ...
  public PubAssignPubMedJournal getPubAssignPubMedJournalByPubId(Long pubId) {
    String hql = "from PubAssignPubMedJournal where pubId = ? ";
    List<PubAssignPubMedJournal> list = super.find(hql, pubId);
    if (list.size() > 0) {
      return list.get(0);
    }
    return null;
  ...
  public List<PsnPmJournalRol> getPJMatchPubPJ(Long pubId, Set<Long> psnIds) {
    String hql =
        "select pp from PubAssignPubMedJournal pj ,PsnPmJournalRol pp where pj.pubId = :pubId and pp.psnId in(:psnIds) and pj.jid = pp.jid ";
