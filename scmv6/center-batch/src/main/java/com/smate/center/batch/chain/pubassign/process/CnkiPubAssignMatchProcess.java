 ...
 * 
 */
public class CnkiPubAssignMatchProcess implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = -7490967325550845833L;
    ...
    for (int index = 0; index < this.tasks.size(); index++) {
      ...
      if (flag) {
        try {
          ...
          e.printStackTrace();
          logger.error("CnkiPubAssignMatchProcess成果匹配任务: task=" + task.getName(), e);
          throw e;
        } catch (Exception e) {
          e.printStackTrace();
          logger.error("CnkiPubAssignMatchProcess成果匹配任务: task=" + task.getName(), e);
