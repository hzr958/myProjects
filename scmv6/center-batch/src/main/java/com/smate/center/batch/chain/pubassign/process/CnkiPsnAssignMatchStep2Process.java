...
/**
 * 单位人员匹配单位所有Cnki成果任务第二步.
 * 
 * @author liqinghua
 * 
 */
public class CnkiPsnAssignMatchStep2Process implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = 1778259895272659841L;
      ...
      for (int index = 0; index < this.tasks.size(); index++) {
        ...
        if (flag) {
          try {
            ...
            logger.error("CnkiPsnAssignMatchStep2Process成果匹配任务: task=" + task.getName(), e);
            throw e;
          } catch (Exception e) {
            logger.error("CnkiPsnAssignMatchStep2Process成果匹配任务: task=" + task.getName(), e);
            throw e;
  ...
  private PubAssginMatchContext buildPubAssginMatchContext(Long pubId, Map<Integer, PubAssignScoreMap> scoreMap)
    ...
    pubContext.setSettingPubAssignMatchScore(
        pubAssignCnkiMatchService.getSettingPubAssignScore(pub.getSourceDbId().longValue()));
