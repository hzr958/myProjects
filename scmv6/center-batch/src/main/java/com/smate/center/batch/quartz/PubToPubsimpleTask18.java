...
public class PubToPubsimpleTask18 {
  private final Logger logger = LoggerFactory.getLogger(getClass());

  private final static Integer SIZE = 500; // 每次刷新获取的个数
  private Long startPubId = 19909096248L;
  private Long endPubId = 19912862542L;
  ...
  public void run() throws BatchTaskException {
    logger.debug("====================================pubToPubsimpleTask__18===开始运行");
    if (isRun() == false) {
      logger.debug("====================================pubToPubsimpleTask__18===开关关闭");
      return;
    } else {
      try {
        doRun();
      } catch (BatchTaskException e) {
        logger.error("pubToPubsimpleTask__18,运行异常", e);
  ...
  public void doRun() throws BatchTaskException {
    if (taskMarkerService.getApplicationQuartzSettingValue("PubToPubsimpleTask_removePubCache18") == 1) {

      taskMarkerService.closeQuartzApplication("PubToPubsimpleTask_removePubCache18");
    }
    Long count = 0L;
    while (true) {
      count++;
      ...
      for (Object obj : dataList) {
        ...
        try {
          ...
          resStatus = 2;
          logger.error("PubToPubsimpleTask18 -- 关键词组合出错  pubid ：" + pubId, e);
          resMsg = "--PubToPubsimpleTask18--pubId=" + pubId + "," + e.getMessage();
