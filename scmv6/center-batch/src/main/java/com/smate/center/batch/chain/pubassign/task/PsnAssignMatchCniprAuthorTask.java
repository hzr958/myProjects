...
/**
 * cnipr成果匹配-人员匹配成果用户名称匹配.
 * 
 * @author liqinghua
 * 
 */
public class PsnAssignMatchCniprAuthorTask implements IPubAssignMatchTask {

  /**
   * 
   */
  private static final long serialVersionUID = -2718266188573360669L;
  private final String name = "PsnAssignMatchCniprAuthorTask";
  @Autowired
  ...
  public boolean run(PubAssginMatchContext context) throws Exception {
    ...
    List<Object[]> matchedPubs =
        pubAssignCniprMatchService.getCniprPubAuthorMatchPsn(context.getPsnId(), context.getInsId());
    // 如果未匹配到一条成果，直接返回
  ...
  public SettingPubAssignScoreWrap getSettingPubAssignScore(Long dbId) throws ServiceException {
    ...
    SettingPubAssignScoreWrap setting = this.pubAssignCniprMatchService.getSettingPubAssignScore(dbId);
    if (setting == null) {
