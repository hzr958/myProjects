...
public class GrpPubRcmdDao extends SnsHibernateDao<GrpPubRcmd, Long> {
  ...
  public Long getPendingConfirmedCount(Long grpId) {
    // TODO 待确认的条件没有加
    String hql = "select count(t.id) from GrpPubRcmd t where  t.grpId=:grpId and t.status!=8";
   ...
   * @throws Exception
   */
  public GrpPubRcmd getGrpPubRcmdWithoutStatus(Long pubId, Long grpId) throws Exception {
    String hql = "from GrpPubRcmd t where t.grpId=:grpId and t.pubId=:pubId  and t.status!=8";
    Object obj = super.createQuery(hql).setMaxResults(1).setParameter("grpId", grpId).setParameter("pubId", pubId)
        .uniqueResult();
    if (obj != null) {
      return (GrpPubRcmd) obj;
  ...
  public List<GrpPubRcmd> findGrpPubsRcmdByGroupId(Long grpId, Page page) {
    String hql = "from  GrpPubRcmd t  where t.grpId =:grpId  and t.status =0 order by t.createDate desc";
