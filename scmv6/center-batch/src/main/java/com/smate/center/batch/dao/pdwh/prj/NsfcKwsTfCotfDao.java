...
public class NsfcKwsTfCotfDao extends PdwhHibernateDao<NsfcKwsTfCotf, Long> {
    ...
    public Long getTaskId() {
        String sql = "select seq_nsfc_cotf_task_id.nextval from dual";
        return ((BigDecimal) this.getSession().createSQLQuery(sql).uniqueResult()).longValue();
    }

    public void insertIntoNsfcTaskStatus(Long taskId, String dataSection, Date startTime) {
        // java.sql.Date sTime = new java.sql.Date(startTime.getTime());
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String sTime = sdf.format(startTime);
        String sql =
                "insert into nsfc_cotf_task_record(task_id, task_data_section, task_start_time)  values (?,?,to_date(?,'YYYY-MM-DD HH24:mi:ss'))";
        super.update(sql, new Object[] {taskId, dataSection, sTime});
    }
    ...
    public void updateNsfcTaskStatus(Long taskId, Date endTime) {
        ...
        String sql =
                "update nsfc_cotf_task_record t set t.task_end_time = to_date(:endTime,'YYYY-MM-DD HH24:mi:ss') where t.task_id =:taskId";
        this.getSession().createSQLQuery(sql).setParameter("taskId", taskId).setParameter("endTime", eTime)
                .executeUpdate();
    }

    public Integer getTaskStatusByDataSection(String name) {
        String sql = "select count(1) from nsfc_cotf_task_record t where t.task_data_section =:name";
        BigDecimal count = (BigDecimal) this.getSession().createSQLQuery(sql).setParameter("name", name).uniqueResult();
        return count.intValue();
    }

    public void insertIntoNsfcKwsTranslation(Long pId, String zhKw, String enKw, String discode) {
        String sql = "insert into nsfc_kws_translate_by_seq values (seq_nsfc_kw_wtf.nextval, ?,?,?,?)";
        super.update(sql, new Object[] {pId, discode, zhKw, enKw});
    }
