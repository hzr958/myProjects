...
public class PubToPubsimpleTask20 {
  ...
  public void run() throws BatchTaskException {
    logger.debug("====================================pubToPubsimpleTask__20===开始运行");
    if (isRun() == false) {
      logger.debug("====================================pubToPubsimpleTask__20===开关关闭");
      return;
    } else {
      try {
        doRun();
  ...
  public void doRun() throws BatchTaskException {
    ...
    while (true) {
      ...
      if (dataList == null || dataList.size() == 0 || count.equals(3999999L)) {
        logger.info("PubToPubsimpleTask20已经完成");
        break;
      }
      for (Object obj : dataList) {
        ...
        try {
          ...
          resStatus = 2;
          logger.error("PubToPubsimpleTask20 -- 关键词组合出错  pubid ：" + pubId, e);
          resMsg = "--PubToPubsimpleTask20--pubId=" + pubId + "," + e.getMessage();
