 ...
 * 
 */
public class CniprPubAssignXmlProcess implements IPubAssignXmlProcess {

  /**
   * 
   */
  private static final long serialVersionUID = -4639624028857488111L;
  // logger.
      ...
      for (int index = 0; index < this.tasks.size(); index++) {
        ...
        if (flag) {
          try {
            ...
            e.printStackTrace();
            logger.error("CniprPubAssignXmlProcess终止处理该XML: task=" + task.getName(), e);
            throw e;
          } catch (Exception e) {
            e.printStackTrace();
            logger.error("CniprPubAssignXmlProcess终止处理该XML: task=" + task.getName(), e);
