 ...
 * 
 */
public class EiPubAssignMatchProcess implements IPubAssignMatchProcess {


  /**
   * 
   */
  private static final long serialVersionUID = -3728184560334658959L;
    ...
    for (int index = 0; index < this.tasks.size(); index++) {
      ...
      if (flag) {
        try {
          ...
          e.printStackTrace();
          logger.error("EiPubAssignMatchProcessEI成果匹配任务: task=" + task.getName(), e);
          throw e;
        } catch (Exception e) {
          e.printStackTrace();
          logger.error("EiPubAssignMatchProcessEI成果匹配任务: task=" + task.getName(), e);
