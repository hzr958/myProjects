...
/**
 * CnkiPat成果匹配-人员匹配成果用户名称匹配.
 * 
 * @author liqinghua
 * 
 */
public class PsnAssignMatchCnkiPatAuthorTask implements IPubAssignMatchTask {

  /**
   * 
   */
  private static final long serialVersionUID = 6551700258415900245L;
  private final String name = "PsnAssignMatchCnkiPatAuthorTask";
  @Autowired
  ...
  public boolean run(PubAssginMatchContext context) throws Exception {
    ...
    List<Object[]> matchedPubs =
        pubAssignCnkiPatMatchService.getCnkiPatPubAuthorMatchPsn(context.getPsnId(), context.getInsId());
    // 如果未匹配到一条成果，直接返回
  ...
  public SettingPubAssignScoreWrap getSettingPubAssignScore(Long dbId) throws ServiceException {
    ...
    SettingPubAssignScoreWrap setting = this.pubAssignCnkiPatMatchService.getSettingPubAssignScore(dbId);
    if (setting == null) {
