...
public class PdwhBriefUtils {
  ...
  // 如果匹配到base_journal表中的标准期刊，则优先使用
  public static String getBrief(PubXmlDocument doc, String language, String journalNameZh, String journalNameEn)
    ...
    try {
      ...
      switch (typeId) {
        ...
        case 3:// 会议论文
          ...
          break;
        case 4:// 期刊论文
          brief = getBriefJournal(doc, language, journalNameZh, journalNameEn);
          break;
        case 5:// 专利
          brief = getBriefPatent(doc, language);
          break;
   ...
   * @throws ServiceException
   */
  private static String getBriefJournal(PubXmlDocument doc, String language, String journalNameZh, String journalNameEn)
      throws ServiceException {
    String brief = "";
    try {
      List<String> params = new ArrayList<String>();
      String original = "";
      if (StringUtils.isNotEmpty(journalNameZh) || StringUtils.isNotEmpty(journalNameEn)) {
        if ("zh".equalsIgnoreCase(language)) {
          original = StringUtils.isNotEmpty(journalNameZh) ? journalNameZh : journalNameEn;
        } else {
          original = StringUtils.isNotEmpty(journalNameEn) ? journalNameEn : journalNameZh;
        }
      } else {
        original = StringUtils.trimToEmpty(doc.getXmlNodeAttribute(PubXmlConstants.PUBLICATION_XPATH, "original"));
      }
      String volume = StringUtils.trimToEmpty(doc.getXmlNodeAttribute(PubXmlConstants.PUBLICATION_XPATH, "volume"));
      String issue = StringUtils.trimToEmpty(doc.getXmlNodeAttribute(PubXmlConstants.PUBLICATION_XPATH, "issue"));
      String start_page =
      ...
      String end_page = StringUtils.trimToEmpty(doc.getXmlNodeAttribute(PubXmlConstants.PUBLICATION_XPATH, "end_page"));
      String pubyear = StringUtils.trimToEmpty(doc.getXmlNodeAttribute(PubXmlConstants.PUBLICATION_XPATH, "pubyear"));
      // 基准库中保持xml抓取时的journal形式，不再修改，20180823与强哥确认
      // params.add(XmlUtil.formatJnlTitle(original));
      params.add(original);
      if (StringUtils.isNotBlank(volume) && StringUtils.isNotBlank(issue)) {
        // params.add(volume);
  ...
  private static String rebuildBrief(List<String> params, String typeName) {
    ...
    // newbrief += "." + typeName;
    if (StringUtils.isNotBlank(newbrief)) {
