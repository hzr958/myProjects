...
/**
 * 单位人员匹配单位所有CnkiPat成果任务第二步.
 * 
 * @author liqinghua
 * 
 */
public class CnkiPatPsnAssignMatchStep2Process implements IPubAssignMatchProcess {
      ...
      for (int index = 0; index < this.tasks.size(); index++) {
        ...
        if (flag) {
          try {
            ...
            logger.error("CnkiPatPsnAssignMatchStep2Process成果匹配任务: task=" + task.getName(), e);
            throw e;
          } catch (Exception e) {
            logger.error("CnkiPatPsnAssignMatchStep2Process成果匹配任务: task=" + task.getName(), e);
            throw e;
  ...
  private PubAssginMatchContext buildPubAssginMatchContext(Long pubId, Map<Integer, PubAssignScoreMap> scoreMap)
    ...
    pubContext.setSettingPubAssignMatchScore(
        pubAssignCnkiPatMatchService.getSettingPubAssignScore(pub.getSourceDbId().longValue()));
