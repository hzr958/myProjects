 ...
 * 
 */
public class PubMedPsnAssignMatchStep1Process implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = 8319452498784916837L;
    ...
    for (int index = 0; index < this.tasks.size(); index++) {
      ...
      if (flag) {
        try {
          ...
          logger.error("PubMedPsnAssignMatchStep1Process成果匹配任务: task=" + task.getName(), e);
          throw e;
        } catch (Exception e) {
          logger.error("PubMedPsnAssignMatchStep1Process成果匹配任务: task=" + task.getName(), e);
          throw e;
