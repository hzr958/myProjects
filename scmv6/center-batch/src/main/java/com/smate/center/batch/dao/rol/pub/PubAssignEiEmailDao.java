 ...
 */
@Repository
public class PubAssignEiEmailDao extends RolHibernateDao<PubAssignEiEmail, Long> {
  ...
  public boolean isExists(Long pubId, String email) {
    String hql = "select count(id) from PubAssignEiEmail where pubId = ? and email = ? ";
  ...
  public List<Object[]> getEmailMatchPubEmail(Long pubId, Long insId) {
    ...
    // 用户必须在职,作者机构ID为空(=2)
    String hql =
        "select pe.seqNo,rp.pk.psnId,pe.insId from PubAssignEiEmail pe,PsnPmEmailRol pp,RolPsnIns rp where rp.pk.insId = ? and rp.pk.psnId = pp.psnId  and rp.status = 1 and rp.isIns = 0 and pe.pubId = ? and pe.email = pp.email and (pe.insId = ? or pe.insId =2 )";
  ...
  public List<Object[]> getPsnEmailMatchEiPubEmail(Long psnId, Long insId) {
    StringBuilder hql = new StringBuilder();
    hql.append("select pe.pubId,pe.seqNo,pe.insId from PubAssignEiEmail pe,PsnPmEmail pp ");
    // 单位，PSN_ID过滤
  ...
  public List<String> getNotExistsCoEmail(Long psnId, Long pubId) {
    ...
    StringBuilder hql = new StringBuilder();
    hql.append("select email from PubAssignEiEmail t where ");
  ...
  public List<Object[]> getEiCoeMatchPubAuthor(Long pubId, Set<Long> psnIds) {
    String hql =
        "select pp.psnId,count(pp.psnId) from PubAssignEiEmail pa ,PsnPmCoemail pp where pa.pubId =:pubId and pp.psnId in(:psnIds) and pp.email = pa.email group by pp.psnId ";
