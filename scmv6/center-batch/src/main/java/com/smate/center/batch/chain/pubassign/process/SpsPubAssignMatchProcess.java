 ...
 * 
 */
public class SpsPubAssignMatchProcess implements IPubAssignMatchProcess {

  /**
   * 
   */
  private static final long serialVersionUID = -317383848776065374L;
    ...
    for (int index = 0; index < this.tasks.size(); index++) {
      ...
      if (flag) {
        try {
          ...
          e.printStackTrace();
          logger.error("SpsPubAssignMatchProcess成果匹配任务: task=" + task.getName(), e);
          throw e;
        } catch (Exception e) {
          e.printStackTrace();
          logger.error("SpsPubAssignMatchProcess成果匹配任务: task=" + task.getName(), e);
