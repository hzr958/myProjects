...
public class PubAssignPubMedEmailDao extends RolHibernateDao<PubAssignPubMedEmail, Long> {
  ...
  public List<Object[]> getEmailMatchPubEmail(Long pubId, Long insId) {
    // String hql =
    // "select distinct rp.pk.psnId,pe.seqNo,pe.insId from PubAssignPubMedEmail pe,PsnPmEmail
    // pp,RolPsnIns rp where rp.pk.insId = ? and rp.pk.psnId = pp.psnId and rp.status = 1 and pe.pubId =
    // ? and pe.email = pp.email ";
    // 用户必须在职,作者机构ID为空(=2)
    String hql =
        "select pe.seqNo,rp.pk.psnId,pe.insId from PubAssignPubMedEmail pe,PsnPmEmail pp,RolPsnIns rp where rp.pk.insId = ? and rp.pk.psnId = pp.psnId  and rp.status = 1 and rp.isIns = 0 and pe.pubId = ? and pe.email = pp.email and (pe.insId = ? or pe.insId =2 )";
    return super.find(hql, insId, pubId, insId);
  }

  /**
   * 获取人员匹配上PUBMED成果作者email的成果列表.
   * 
   * @param psnId
   * @param insId
   * @return
  ...
  public List<Object[]> getPsnEmailMatchPubMedPubEmail(Long psnId, Long insId) {
    StringBuilder hql = new StringBuilder();
    hql.append("select pe.pubId,pe.seqNo,pe.insId from PubAssignPubMedEmail pe,PsnPmEmail pp ");
    // 单位，PSN_ID过滤
