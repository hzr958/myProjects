 ...
 * 
 */
public class ExtractPubMedAuthorNameTask implements IPubAssignXmlTask {

  private final String name = "ExtractPubMedAuthorNameTask";
  @Autowired
  ...
  public boolean can(PubXmlDocument xmlDocument, PubXmlProcessContext context) {
    return context.isImport() && context.isPubMedImport();
  }
  ...
  public boolean run(PubXmlDocument xmlDocument, PubXmlProcessContext context) throws Exception {
    ...
    if (authorList != null && authorList.size() > 0) {
      for (int i = 0; i < authorList.size(); i++) {
        ...
        Long insId = ExtractPubMedAuthorNameTask.getAuthorInsId(author, context);
        this.pubAssignPubMedXmlService.savePubAuthor(context.getCurrentPubId(), context.getCurrentInsId(), abbrName,
            fullName, insId, seqNo);
      }
    } else {
      return false;
    }
