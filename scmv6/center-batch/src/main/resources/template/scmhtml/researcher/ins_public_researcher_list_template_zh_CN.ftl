...
<td align="left">
    ...
    <p>
        ${positionZh!''}<#if (positionEn?exists) && (unitEname?exists)>; </#if> ${unitZhame!''}
    </p>
	...
	<b class="public_pulldown">
	    ...
	    <a class="share_pull" style="display:none" resId="${psnId?c }" resTitle="${zhName!''}<#if (positionEn?exists) || (unitEname?exists)>&nbsp;</#if>${positionZh!''}<#if (positionEn?exists) && (unitEname?exists)>; </#if>${unitZhame!''}" resPic="${avatars!'#'}" resUrl="${psnUrlOutside!'#'}"></a>
    </b>
</td>
