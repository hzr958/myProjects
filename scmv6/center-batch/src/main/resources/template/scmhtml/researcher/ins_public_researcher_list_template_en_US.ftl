...
<td align="left">
    ...
    <p>
        ${positionEn!''}<#if (positionEn?exists) && (unitEname?exists)>; </#if> ${unitEname!''}
    </p>
	...
	<b class="public_pulldown">
	    ...
	    <a class="share_pull" style="display:none" resId="${psnId?c }" resTitle="${firstName!''}  ${lastName!''}<#if (positionEn?exists) || (unitEname?exists)>&nbsp;</#if>${positionEn!''}<#if (positionEn?exists) && (unitEname?exists)>; </#if>${unitEname!''}" resPic="${avatars!'#'}" resUrl="${psnUrlOutside!'#'}"></a>
    </b>
</td>
