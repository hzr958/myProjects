<li onmouseover="dynMsgUtil.showDynOperation(this)"  onmouseout="dynMsgUtil.hideDynOperation(this)">
    ...
    <div class="publish-nr">
      <div class="moving-contat" id="shareTheme_${dynId?c}_${resId?c}" style="word-wrap:break-word;" >
         <a href="/scmwebsns/resume/psnView?des3PsnId=${des3PsnId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" class="Blue">${psnName}</a>分享：${sayContent} &nbsp;&nbsp;<#if (linkUrl?exists)><a href="${linkUrl}" class="Blue" target="_blank">${linkUrl}</a></#if>
	  </div>
      ...
      <div class="appraisa-choose">
        ...
        <span class="f888 date01"> ${dynDateVar}</span>&nbsp;&nbsp;&nbsp;&nbsp;
	        <div class="praise_box" style="margin-left:20px;">
		        ...
		        <#if (hasAward==0)>
		        <a onclick="dynMsgUtil.ajaxAward(this, '${resType}','${resNode}','${resId?c}','${permission}')" style="cursor:pointer" class="mright10 f888 award_link${resType}_${resNode}_${resId?c}"><i class="dt_icon zan_icon"></i>赞</a>
		        ...
		        <#else>
		        <a onclick="dynMsgUtil.ajaxAward(this, '${resType}','${resNode}','${resId?c}','${permission}')" style="cursor:pointer;display:none" class="mright10 f888 award_link${resType}_${resNode}_${resId?c}"><i class="dt_icon zan_icon"></i>赞</a>
    ...
    <div class="drop_down">
        ...
        <div class="drop_jt_zh_CN"></div>
           <ul>
             <#if (isMine==1)>
               <li><a style="cursor:pointer; <#if (isGroupFlag==1)>border-width:0;</#if>" onclick="dynMsgUtil.ajaxDeleteDyn('${dynId?c}')"><i class="img_nochoose"></i>删除此动态</a></li>
               <#if (isGroupFlag==0)>
               <li><a style="cursor:pointer; border-width:0;" onclick="dynMsgUtil.ajaxShieldDynByType('09','${dynId?c}')"><i class="img_nochoose"></i>屏蔽此类动态</a></li>
             	</#if>
             <#else>
             	<#if (isGroupManager == '1')>
               	<li><a style="cursor:pointer; <#if (isGroupFlag==1)>border-width:0;</#if>" onclick="dynMsgUtil.ajaxDeleteDyn('${dynId?c}')"><i class="img_nochoose"></i>删除此动态</a></li>
