    ...
    <div class="publish-nr">
      ...
      <div class="appraisa-choose">
        <p class="t_detail">
          ...
          <#if (groupPage!='')&&(queryType!=4)><a href="${groupPage}&menuId=1100" class="Blue" target="_blank">查看群组</a>&nbsp;</#if>
          <#if (joinGroupFlag==0)&&(queryType!=4)><a onclick="dynMsgUtil.ajaxJoinGroup(this,'${groupId?c}','${groupNode}')" style="cursor:pointer;" class="Blue apply_joingroup" dynId="${dynId?c}" groupId="${groupId?c}" groupNode="${groupNode}">申请加入群组</a></#if>
        </p>
      </div>
    </div>
