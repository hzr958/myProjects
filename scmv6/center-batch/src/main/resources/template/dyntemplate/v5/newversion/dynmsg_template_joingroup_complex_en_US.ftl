<li onmouseover="dynMsgUtil.showDynOperation(this)"  onmouseout="dynMsgUtil.hideDynOperation(this)">
    ...
    <div class="publish-nr">
      <div class="moving-contat">
       ...
       <#list joinerDetails as joiner><#if (!joiner_has_next) && (joiner_index!=0)> and <#else>${splitChar}</#if><a href="/scmwebsns/resume/psnView?des3PsnId=${joiner.joiner_des3PsnId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" class="Blue">${joiner.joiner_psnEnName}</a><#if joiner_has_next><#assign splitChar>, </#assign></#if></#list>
       <#if queryType==4>
        joined group
       <#else>
       joined a group
      ...
      <div class="appraisa-choose">
        <p class="t_detail">
           ...
           <#if (groupPage?exists)&&(groupPage!='')&&(queryType!=4)><a href="${groupPage}&menuId=1100" class="Blue" target="_blank">View Group</a>&nbsp;</#if>
           <#if (joinGroupFlag==0)&&(queryType!=4)&&(groupNode?exists)><a onclick="dynMsgUtil.ajaxJoinGroup(this,'${groupId?c}','${groupNode}')" style="cursor:pointer;" class="Blue apply_joingroup" dynId="${dynId?c}" groupId="${groupId?c}" groupNode="${groupNode}">Request to join</a></#if>
