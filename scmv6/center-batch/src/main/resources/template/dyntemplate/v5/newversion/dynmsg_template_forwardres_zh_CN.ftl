    ...
    <div class="publish-nr">
      ...
      <#if (forwardPsnDetails?is_sequence)>
       <#list forwardPsnDetails as forwardPsn>
        <a href="/scmwebsns/resume/psnView?des3PsnId=${forwardPsn.forwarder_des3PsnId}&menuId=<#if (isGroupFlag==1)>3<#else>1100</#if>" target="_blank" class="Blue">${forwardPsn.forwarder_psnName}</a>分享了一条动态：${forwardPsn.sayContent}<#if forwardPsn_has_next>&nbsp;||&nbsp;</#if>
       </#list>
       <#else>
