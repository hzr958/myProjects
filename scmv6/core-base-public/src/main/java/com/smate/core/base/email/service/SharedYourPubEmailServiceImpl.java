 ...
 *
 */
@Service("sharedYourPubEmailService")
@Transactional(rollbackOn = Exception.class)
public class SharedYourPubEmailServiceImpl extends AbstractEmailSend {
  ...
  public Map<String, String> invoke(Object... params) throws PublicException {
    ...
    if (!(ctxMap.containsKey("psnId") && ctxMap.containsKey("sharedPsnId") && ctxMap.containsKey("pubId"))) {
      logger.error("map对象缺少传入的参数！");
      throw new PublicException();
    }
    Long psnId = (Long) ctxMap.get("psnId");
    Long sharedPsnId = (Long) ctxMap.get("sharedPsnId");
    // 定义接口接收的参数
    ...
    try {
      ...
      sharedPerson = personProfileDao.getPersonForEmail(sharedPsnId);
    } catch (Exception e) {
      logger.error(" [人名] 分享了你的论文: [论文名]邮件，查询人员信息出错.psnId=" + psnId + ",likedPsnId=" + sharedPsnId, e);
      throw new PublicException(" [人名] 分享了你的论文: [论文名]邮件，查询人员信息出错.psnId=" + psnId + ",likedPsnId=" + sharedPsnId);
    }
    this.generateMailData(ctxMap, person, sharedPerson, paramData);
    return paramData;
  }

  // 获取邮件数据
  private void generateMailData(Map<String, Object> ctxMap, Person person, Person sharedPerson,
    ...
    // 分享人员姓名
    mailData.put("sharedPsnName",
        emailCommonService.getPsnNameByEmailLangage(sharedPerson, person.getEmailLanguageVersion()));
    // 成果url
    String pubShortUrl = "";
  ...
  private void generateSubjectParam(Person person, Map<String, String> paramData, Map<String, String> mailData) {
    ...
    subjectParamLinkList.add(mailData.get("sharedPsnName"));
