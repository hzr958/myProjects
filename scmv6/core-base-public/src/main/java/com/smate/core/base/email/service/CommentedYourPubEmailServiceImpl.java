 ...
 *
 */
@Service("commentedYourPubEmailService")
@Transactional(rollbackOn = Exception.class)
public class CommentedYourPubEmailServiceImpl extends AbstractEmailSend {
  ...
  public Map<String, String> invoke(Object... params) throws PublicException {
    ...
    if (!(ctxMap.containsKey("psnId") && ctxMap.containsKey("commentedPsnId") && ctxMap.containsKey("pubId"))) {
      logger.error("map对象缺少传入的参数！");
      throw new PublicException();
    }
    Long psnId = (Long) ctxMap.get("psnId");
    Long commentedPsnId = (Long) ctxMap.get("commentedPsnId");
    // 定义接口接收的参数
    Map<String, String> paramData = new HashMap<String, String>();
    Person person = null;
    Person commentedPerson = null;
    try {
      person = personProfileDao.getPersonForEmail(psnId);
      commentedPerson = personProfileDao.getPersonForEmail(commentedPsnId);
    } catch (Exception e) {
      logger.error(" [人名] 评论了你的论文: [论文名]邮件，查询人员信息出错.psnId=" + psnId + ",commentedPsnId=" + commentedPsnId, e);
      throw new PublicException(" [人名] 评论了您的论文: [论文名]邮件，查询人员信息出错.psnId=" + psnId + ",commentedPsnId=" + commentedPsnId);
    }
    this.generateMailData(ctxMap, person, commentedPerson, paramData);
    return paramData;

  }

  private void generateMailData(Map<String, Object> ctxMap, Person person, Person commentedPerson,
      Map<String, String> paramData) throws PublicException {
    // 定义构造邮件模版参数集
    Map<String, String> mailData = new HashMap<String, String>();
    // 获取用户设置接收邮件的语言.
    if (person.getEmailLanguageVersion() == null || StringUtils.isBlank(person.getEmailLanguageVersion())) {
    ...
    // 评论人员姓名
    mailData.put("commentedPsnName",
        emailCommonService.getPsnNameByEmailLangage(commentedPerson, person.getEmailLanguageVersion()));
    // 成果url
    String pubShortUrl = "";
  ...
  private void generateSubjectParam(Person person, Map<String, String> paramData, Map<String, String> mailData) {
    ...
    subjectParamLinkList.add(mailData.get("commentedPsnName"));
  ...
  private void buildMailIink(Person person, Map<String, String> mailData, String pubShortUrl) throws PublicException {
    ...
    l5.setKey("mobileImpactsUrl");
    l5.setUrl(domainscm + "/psnweb/outside/mobile/influence");
    l5.setUrlDesc("移动端影响力页面");
