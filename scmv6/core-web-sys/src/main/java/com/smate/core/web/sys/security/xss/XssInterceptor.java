...
public class XssInterceptor implements Interceptor {
  ...
  private static final List<String> excludeField = new ArrayList<>();  // 文本编辑器的内容不要过滤掉 标签
  static {
    excludeField.add("newsShowInfo.content");
  }
  ...
  public String intercept(ActionInvocation invocation) throws Exception {
    ...
    if (paramMap != null && paramMap.size() > 0) {
      ...
      while (iterator.hasNext()) {
        ...
        if(excludeField.contains(key.toString())){
          continue;
        }
