package com.smate.center.oauth.service.profile.psncnf;

import javax.annotation.Resource;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import com.smate.core.base.psn.enums.PsnCnfEnum;
import com.smate.core.base.psn.model.psncnf.PsnConfigMoudle;
import com.smate.core.base.psn.service.psncnf.PsnCnfService;
import com.smate.core.base.psn.utils.PsnCnfUtils;

@Service("clean02Moudle")
@Transactional(rollbackFor = Exception.class)
public class ComponentClean02Moudle implements ComponentClean {
  @Autowired
  private PsnCnfService psnCnfService;

  @Resource(name = "clean03List")
  private ComponentClean next;

  @Override
  public void clean(Long runs, Long cnfId) throws Exception {
    try {
      if (PsnCnfUtils.canRun(runs, PsnCnfEnum.CLEAN, PsnCnfEnum.ALL)) {
        // 删除模块、排序
        PsnConfigMoudle psnConfigMoudle = new PsnConfigMoudle(cnfId);
        psnCnfService.del(psnConfigMoudle);
      }
    } catch (Exception e) {
      throw new Exception("个人配置：clean02Moudle失败", e);
    }
    // 下个清理操作
    next.clean(runs, cnfId);
  }
}
