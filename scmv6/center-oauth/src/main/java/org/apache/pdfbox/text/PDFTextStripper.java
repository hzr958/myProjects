...
import org.apache.pdfbox.pdmodel.PDFReadForm;
...
import com.smate.core.base.utils.tosdbc.ToSDBCUtils;
...
public class PDFTextStripper extends LegacyPDFStreamEngine {
  ...
  /**
   * The charactersByArticle is used to extract text by article divisions. For example a PDF that has
   * two columns like a newspaper, we want to extract the first column and then the second column. In
   * this example the PDF would have 2 beads(or articles), one for each column. The size of the
   * charactersByArticle would be 5, because not all text on the screen will fall into one of the
   * articles. The five divisions are shown below
   *
   * Text before first article first article text text between first article and second article second
   * article text text after second article
   *
  ...
  protected void processPages(PDPageTree pages) throws IOException {
    ...
    PDFReadForm readForm = new PDFReadForm();
    for (PDPage page : pages) {
      page.setReadForm(readForm);
      currentPageNo++;
      if (page.hasContents()) {
        processPage(page);
      }
    }
    // JacksonUtils.mapToJsonStr(this.pdfMatchInfo.get());
    writeString(JacksonUtils.mapToJsonStr(readForm.getPdfMatchInfo()));
  }

  /**
   * This method is available for subclasses of this class. It will be called before processing of the
   * document start.
   *
  ...
  /**
   * This method is available for subclasses of this class. It will be called after processing of the
   * document finishes.
   *
   * @param document The PDF document that is being processed.
   * @throws IOException If an IO error occurs.
  ...
  /**
   * Start a new article, which is typically defined as a column on a single page (also referred to as
   * a bead). This assumes that the primary direction of text is left to right. Default implementation
   * is to do nothing. Subclasses may provide additional information.
   *
  ...
  /**
   * Start a new article, which is typically defined as a column on a single page (also referred to as
   * a bead). Default implementation is to do nothing. Subclasses may provide additional information.
   *
   ...
   */
  protected void writePage(PDFReadForm readForm) throws IOException {
    ...
    for (List<TextPosition> textList : charactersByArticle) {
      ...
      while (textIter.hasNext()) {
        ...
        if (position.getFontSizeInPt() > 10) {
          // System.out.print(position.toString());
        }
        // System.out.print(position.getFontSize());
        ...
        if (lastPosition != null) {
          ...
          if (!overlap(positionY, positionHeight, maxYForLine, maxHeightForLine)) {
            writeLine(normalize(line), readForm);
            line.clear();
  ...
  private void matchInfo(List<WordWithTextPositions> line, PDFReadForm readForm) {


    try {
      Map<String, Set<String>> temp = readForm.getPdfMatchInfo();
      if (temp == null) {
        temp = new HashMap<String, Set<String>>();
        temp.put("dois", new HashSet<String>());
        temp.put("titles", new HashSet<String>());
        readForm.setPdfMatchInfo(temp);
      }
      Set<String> titles = readForm.getPdfMatchInfo().get("titles");
      if (titles == null) {
        titles = new HashSet<String>();
        temp.put("titles", titles);
      }
      Set<String> dois = readForm.getPdfMatchInfo().get("dois");
      if (dois == null) {
      ...
      for (int i = 0; i < numberOfStrings; i++) {
        List<TextPosition> textPList = line.get(0).getTextPositions();
        if (CollectionUtils.isNotEmpty(textPList) && textPList.get(0).getFontSizeInPt() > 10) {
          contains = "title";
          output = true;
        }
        if (PDFDataUtils.hasDOI(ToSDBCUtils.ToDBC(line.get(i).text))) {
          contains = "doi";
          output = true;
        }
        /*
         * if (PDFDataUtils.hasEmail(line.get(i).text)) { contains = "email"; output = true; } if
         * (PDFDataUtils.hasAbstract(line.get(i).text)) { contains = "abstract"; output = true; } if
         * (PDFDataUtils.hasKeywords(line.get(i).text)) { contains = "keywords"; output = true; }
         */
      }
      // 通过一些判断 判断下一行 内容 是否跟上一行内容是相同内容
      if (true) {
        for (int i = 0; i < numberOfStrings; i++) {
          WordWithTextPositions word = line.get(i);
          result.append(ToSDBCUtils.ToDBC(word.getText()));
          result.append(" ");
        }
        // System.out.println(result);
        List<?> preLine = readForm.getPdfReadPreContext();
        if (StringUtils.isNotBlank(result)) {
          switch (contains) {
            case "title":
              if (CollectionUtils.isNotEmpty(preLine)) {
                List<TextPosition> textPList = line.get(0).getTextPositions();
                List<TextPosition> preTextPList = ((WordWithTextPositions) preLine.get(0)).getTextPositions();
                if (CollectionUtils.isNotEmpty(textPList) && CollectionUtils.isNotEmpty(preTextPList)
                    && textPList.get(0).getFontSizeInPt() == preTextPList.get(0).getFontSizeInPt()) {
                  result.insert(0, readForm.getPdfReadPreContent());
                }
              }
              titles.add(result.toString());
              break;
            case "doi":
              dois.add(PDFDataUtils.matchDOI(result.toString()));
              break;
            default:
          }
        }
      ...
      readForm.setPdfMatchInfo(temp);
      readForm.setPdfReadPreContent(result.toString());
      // 保存 当前行数据 以便下一行查询
      readForm.setPdfReadPreContext(line);
    } catch (Exception e) {
      // 有异常不做任何处理
      LOG.error("出现异常", e);
      return;
  ...
  /**
   * This is the page that the text extraction will start on. The pages start at page 1. For example
   * in a 5 page PDF document, if the start page is 1 then all pages will be extracted. If the start
   * page is 4 then pages 4 and 5 will be extracted. The default value is 1.
   *
  ...
  /**
   * This will get the last page that will be extracted. This is inclusive, for example if a 5 page
   * PDF an endPage value of 5 would extract the entire document, an end page of 2 would extract pages
   * 1 and 2. This defaults to Integer.MAX_VALUE such that all pages of the pdf will be extracted.
   *
  ...
  /**
   * Set the desired line separator for output text. The line.separator system property is used if the
   * line separator preference is not set explicitly using this method.
   *
  ...
  /**
   * Set the desired word separator for output text. The PDFBox text extraction algorithm will output
   * a space character if there is enough space between two words. By default a space character is
   * used. If you need and accurate count of characters that are found in a PDF document then you
  ...
  /**
   * Character strings are grouped by articles. It is quite common that there will only be a single
   * article. This returns a List that contains List objects, the inner lists will contain
  ...
  /**
   * By default the text stripper will attempt to remove text that overlapps each other. Word paints
   * the same character several times in order to make it look bold. By setting this to false all text
   * will be extracted, which means that certain sections will be duplicated, but better performance
   * will be noticed.
   *
  ...
  /**
   * Set if the text stripper should group the text output by a list of beads. The default value is
   * true!
   *
  ...
  /**
   * There will some additional text formatting be added if addMoreFormatting is set to true. Default
   * is false.
   * 
  ...
  /**
   * The order of the text tokens in a PDF file may not be in the same as they appear visually on the
   * screen. For example, a PDF writer may write out all text by font, so all bold or larger text,
   * then make a second pass and write out the normal text.<br>
   * The default is to <b>not</b> sort by position.<br>
   * <br>
   * A PDF writer could choose to write each character in a different order. By default PDFBox does
   * <b>not</b> sort the text tokens before processing them due to performance reasons.
   *
  ...
  /**
   * Get the current space width-based tolerance value that is being used to estimate where spaces in
   * text should be added. Note that the default value for this has been determined from trial and
   * error.
   * 
  ...
  /**
   * Set the space width-based tolerance value that is used to estimate where spaces in text should be
   * added. Note that the default value for this has been determined from trial and error. Setting
   * this value larger will reduce the number of spaces added.
   * 
  ...
  /**
   * Get the current character width-based tolerance value that is being used to estimate where spaces
   * in text should be added. Note that the default value for this has been determined from trial and
   * error.
   * 
   * @return The current tolerance / scaling factor
   */
   ...
   * should be added. Note that the default value for this has been determined from trial and error.
  ...
  /**
   * returns the multiple of whitespace character widths for the current text which the current line
  ...
  /**
   * the minimum whitespace, as a multiple of the max height of the current characters beyond which
   * the current line start is considered to be a paragraph start.
   * 
   ...
   * which the current line start is considered to be a paragraph start. The default value is 2.5.
   *
   * @param dropThresholdValue the character height multiple for max allowed whitespace between lines
   ...
   * separation. This should <i>only</i> be called for consecutive text positions that first pass the
   ...
   * </p>
   *
   * @param position the current text position. This may have its isParagraphStart or isHangingIndent
   *        flags set upon return.
  ...
  /**
   * returns the list item Pattern object that matches the text at the specified PositionWrapper or
   * null if the text does not match such a pattern. The list of Patterns tested against is given by
   * the {@link #getListItemPatterns()} method. To add to the list, simply override that method (if
  ...
  /**
   * a list of regular expressions that match commonly used list item formats, i.e. bullets, numbers,
   * letters, Roman numerals, etc. Not meant to be comprehensive.
   */
   ...
   * For example numbered items of form:
   * <ol>
   * <li>some text</li>
   * <li>more text</li>
   * </ol>
   ...
   * <li>more text</li>
   * </ul>
   * etc., all begin with some character pattern. The pattern "\\d+\." (matches "1.", "2.", ...) or
   * "\[\\d+\]" (matches "[1]", "[2]", ...).
   * <p>
   ...
   * <p>
   * Order of the supplied list of patterns is important as most common patterns should come first.
   * Patterns should be strict in general, and all will be used with case sensitivity on.
   * </p>
   * 
   ...
   * @throws IOException if something went wrong
   */
  private void writeLine(List<WordWithTextPositions> line, PDFReadForm readForm) throws IOException {
    int numberOfStrings = line.size();
    if (numberOfStrings > 0) {
      matchInfo(line, readForm);
    }


    /*
     * for (int i = 0; i < numberOfStrings; i++) { WordWithTextPositions word = line.get(i);
     * writeString(word.getText(), word.getTextPositions()); if (i < numberOfStrings - 1) {
     * writeWordSeparator(); } }
     */
  }
  ...
  /**
   * Handles the LTR and RTL direction of the given words. The whole implementation stands and falls
   * with the given word. If the word is a full line, the results will be the best. If the word
   * contains of single words or characters, the order of the characters in a word or words in a line
   * may wrong, due to RTL and LTR marks and characters!
   * 
  ...
  /**
   * Normalize certain Unicode characters. For example, convert the single "fi" ligature to "f" and
   * "i". Also normalises Arabic and Hebrew presentation forms.
   *
  ...
  /**
   * Internal class that maps strings to lists of {@link TextPosition} arrays. Note that the number of
   * entries in that list may differ from the number of characters in the string due to normalization.
   *
   * @author Axel Dörfler
   */
   ...
   * positions.
   * <p>
   * This is implemented as a wrapper since the TextPosition class doesn't provide complete access to
   * its state fields to subclasses. Also, conceptually TextPosition is immutable while these flags
   * need to be set post-creation so it makes sense to put these flags in this separate class.
   * </p>
   * 
